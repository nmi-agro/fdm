---
title: Fosfaatgebruiksnorm 2026
sidebar_label: "Fosfaatgebruiksnorm"
---

This document provides a detailed explanation of the Dutch legal usage norm (`gebruiksnorm`) for phosphate in 2026. It covers how the norm is calculated and how the amount of applied phosphate counts towards this norm (`opvulling`), including the reduced counting percentages for organic-rich fertilizers.

:::note
The phosphate norm values, the classification thresholds, and the rules for organic-rich fertilizers are the same in 2026 as in 2025.
:::
...
**Official Source**: [RVO — Fosfaatdifferentiatie](https://www.rvo.nl/onderwerpen/mest/gebruiken-en-uitrijden/fosfaat-landbouwgrond/differentiatie)

### How the FDM Calculator Determines the Norm

The `fdm-calculator` uses the `calculateNL2026FosfaatGebruiksNorm` function, located in `fdm-calculator/src/norms/nl/2026/value/fosfaatgebruiksnorm.ts`.
...
The core logic uses `fosfaatgebruiksnorm-data.ts`, which contains the norm values for both grassland and arable land.
...
- **75% of phosphate counts for:**
  ...
  - Vaste strorijke mest van varkens (only for organic farms)
...
- **100% of phosphate counts for:**
  - All other fertilizers, including mineral fertilizers, Renure products, and other organic fertilizers not listed above.
...
The `fdm-calculator` uses the `calculateNL2026FertilizerApplicationFillingForFosfaatGebruiksNorm` function, located in `fdm-calculator/src/norms/nl/2026/filling/fosfaatgebruiksnorm.ts`.

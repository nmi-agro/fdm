...
*   In the rotation and field-level fertilizer-application detail tables, equivalent fertilizer applications are now displayed on separate rows rather than collapsed.
...
*   **Advanced cultivation history view:** The field-detail page in the Atlas now offers a spatial flow diagram that visualises how a field's parcel boundaries changed over time — splits, merges, expansions, and shrinkage. Users can switch between the existing simple timeline ("Eenvoudig") and the new advanced view ("Uitgebreid") via a tab control. The selected tab is persisted in session storage.
...
*   The "show productive only" toggle on rotation tables works again. Search terms and the "show productive only" selection are now shared between the field and rotation tables, and the shared state is reflected consistently in the UI.
*   The rotation-table harvest-date display sorting has been corrected.

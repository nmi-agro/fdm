...
export function registerDerogationRoutes(
    ...
    const listDerogationsHandler: RouteHandler<
        typeof listDerogationsRoute
    > = async (c) => {
    ...
    const createDerogationHandler: RouteHandler<
        typeof createDerogationRoute
    > = async (c) => {
        const principal = c.get("principal") as unknown as ApiPrincipalContext
        // @ts-expect-error: @hono/zod-openapi type inference is broken with TypeScript 6 + Zod v4
        const { b_id_farm } = c.req.valid("param") as { b_id_farm: string }
        ...
        const body = c.req.valid("json") as z.infer<
            typeof CreateDerogationBodySchema
        >
        ...
        c.header(
            "Location",
            `${new URL(c.req.url).origin}/derogations/${b_id_derogation}`,
        )
        return c.json(
            {
                b_id_derogation,
    ...
    const deleteDerogationHandler: RouteHandler<
        typeof deleteDerogationRoute
    > = async (c) => {
        const principal = c.get("principal") as unknown as ApiPrincipalContext
        // @ts-expect-error: @hono/zod-openapi type inference is broken with TypeScript 6 + Zod v4
        ...
        await services.removeDerogation(
            fdm,
            principal.effectivePrincipalId,
            b_id_derogation,
        )

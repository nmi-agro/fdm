...
function serialiseFertilizerCatalogueBase(
    fertilizer: Fertilizer | FertilizerCatalogue,
) {
...
export function registerFertilizerRoutes(
    ...
    const listFertilizersHandler: RouteHandler<
        typeof listFertilizersRoute
    > = async (c) => {
        ...
        return c.json(
            paginatedResponse(
                fertilizers.map(serialiseFertilizer),
                limit,
                offset,
            ),
            200,
    ...
    const createFertilizerHandler: RouteHandler<
        typeof createFertilizerRoute
    > = async (c) => {
        const principal = c.get("principal") as unknown as ApiPrincipalContext
        // @ts-expect-error: @hono/zod-openapi type inference is broken with TypeScript 6 + Zod v4
        const { b_id_farm } = c.req.valid("param") as { b_id_farm: string }
        ...
        const body = c.req.valid("json") as z.infer<
            typeof CreateFertilizerBodySchema
        >
    ...
    const listFertilizerCatalogueHandler: RouteHandler<
        typeof listFertilizerCatalogueRoute
    > = async (c) => {
        const principal = c.get("principal") as unknown as ApiPrincipalContext
        // @ts-expect-error: @hono/zod-openapi type inference is broken with TypeScript 6 + Zod v4
        const { b_id_farm } = c.req.valid("param") as { b_id_farm: string }
        ...
        return c.json(
            paginatedResponse(
                fertilizers.map(serialiseFertilizerCatalogue),
                limit,
                offset,
            ),
            200,

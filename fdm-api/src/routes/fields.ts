...
const CreateFieldBodySchema = z
  .object({
    ...
    b_id_source: z.string().nullable().optional().describe("External source identifier."),
...
const deleteFieldRoute = createRoute({
  ...
  description: "Permanently deletes a field and all its associated cultivations and soil analyses.",
  security: [{ ApiKeyHeader: [] }, { BearerAuth: [] }],
  request: { params: z.object({ b_id: z.string() }) },
  responses: {
...
export function registerFieldRoutes(
  ...
  const createFieldHandler: RouteHandler<typeof createFieldRoute> = async (c) => {
    ...
    const { b_id_farm } = c.req.valid("param") as { b_id_farm: string }
    // @ts-expect-error: @hono/zod-openapi type inference is broken with TypeScript 6 + Zod v4
    const body = c.req.valid("json") as z.infer<typeof CreateFieldBodySchema>
    ...
    const field = await services.getField(fdm, principal.effectivePrincipalId, b_id)
    if (!field?.b_id) {
      throw new ApiError(500, "internal-error", "Field created but could not be retrieved.")
    }
  ...
  const updateFieldHandler: RouteHandler<typeof updateFieldRoute> = async (c) => {
    ...
    const { b_id } = c.req.valid("param") as { b_id: string }
    // @ts-expect-error: @hono/zod-openapi type inference is broken with TypeScript 6 + Zod v4
    const body = c.req.valid("json") as z.infer<typeof UpdateFieldBodySchema>
    if (Object.values(body).every((value) => value === undefined)) {
      throw new ApiError(400, "validation-failed", "At least one field must be provided.")
    }
    const existing = await services.getField(fdm, principal.effectivePrincipalId, b_id)
    if (!existing?.b_id) {
      throw new ApiError(404, "not-found", `Field '${b_id}' not found.`)
    }
    ...
    const updated = await services.updateField(
      ...
      body.b_id_source !== undefined ? body.b_id_source : existing.b_id_source,
      body.b_geometry !== undefined
        ? (body.b_geometry as unknown as Parameters<typeof updateField>[5])
        : existing.b_geometry,
      body.b_start !== undefined ? new Date(body.b_start) : (existing.b_start ?? undefined),
      body.b_acquiring_method !== undefined ? body.b_acquiring_method : existing.b_acquiring_method,
      body.b_end !== undefined
        ? body.b_end !== null
          ? new Date(body.b_end)
          : null
        : (existing.b_end ?? undefined),
      body.b_bufferstrip !== undefined ? body.b_bufferstrip : existing.b_bufferstrip,
    )
    if (!updated?.b_id) {
      throw new ApiError(404, "not-found", `Field '${b_id}' not found.`)
    }

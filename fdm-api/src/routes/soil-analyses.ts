...
const CreateSoilAnalysisBodySchema = z
    .object({
        ...
        a_depth_upper: z
            .number()
            .optional()
            .describe("Upper sampling depth in cm (default 0)."),
    })
...
const listFarmSoilAnalysesRoute = createRoute({
    ...
    description:
        "Returns all soil analyses across all fields on the specified farm.",
    security: [{ ApiKeyHeader: [] }, { BearerAuth: [] }],
    request: {
...
const updateSoilAnalysisRoute = createRoute({
    ...
    description:
        "Partially updates measurement data on an existing soil analysis.",
    security: [{ ApiKeyHeader: [] }, { BearerAuth: [] }],
    request: {
...
export function registerSoilAnalysisRoutes(
    ...
    const listSoilAnalysesHandler: RouteHandler<
        typeof listSoilAnalysesRoute
    > = async (c) => {
        ...
        >
        const analyses = await services.getSoilAnalyses(
            fdm,
    ...
    const listFarmSoilAnalysesHandler: RouteHandler<
        typeof listFarmSoilAnalysesRoute
    > = async (c) => {
        const principal = c.get("principal") as unknown as ApiPrincipalContext
        // @ts-expect-error: @hono/zod-openapi type inference is broken with TypeScript 6 + Zod v4
    ...
    const getCurrentSoilDataHandler: RouteHandler<
        typeof currentSoilDataRoute
    > = async (c) => {
        const principal = c.get("principal") as unknown as ApiPrincipalContext
        // @ts-expect-error: @hono/zod-openapi type inference is broken with TypeScript 6 + Zod v4
        const { b_id } = c.req.valid("param") as { b_id: string }
        ...
        if (currentSoilData.length === 0) {
            throw new ApiError(
                404,
                "not-found",
                `Current soil data for field '${b_id}' not found.`,
            )
        }
        return c.json(serialiseCurrentSoilData(currentSoilData), 200)
    }

    const createSoilAnalysisHandler: RouteHandler<
        typeof createSoilAnalysisRoute
    > = async (c) => {
        const principal = c.get("principal") as unknown as ApiPrincipalContext
        // @ts-expect-error: @hono/zod-openapi type inference is broken with TypeScript 6 + Zod v4
        const { b_id } = c.req.valid("param") as { b_id: string }
        ...
        const body = c.req.valid("json") as z.infer<
            typeof CreateSoilAnalysisBodySchema
        >
        const {
            a_date,
            a_source,
        ...
        const analysis = await services.getSoilAnalysis(
            fdm,
            principal.effectivePrincipalId,
            a_id,
        )
        if (!analysis?.a_id) {
            throw new ApiError(
                500,
                "internal-error",
                "Soil analysis created but could not be retrieved.",
            )
        }
        c.header(
            "Location",
            `${new URL(c.req.url).origin}/soil-analyses/${a_id}`,
        )
        return c.json(serialiseSoilAnalysis(analysis), 201)
    }

    const getSoilAnalysisHandler: RouteHandler<
        typeof getSoilAnalysisRoute
    > = async (c) => {
        const principal = c.get("principal") as unknown as ApiPrincipalContext
        // @ts-expect-error: @hono/zod-openapi type inference is broken with TypeScript 6 + Zod v4
        ...
        if (!analysis?.a_id) {
            throw new ApiError(
                404,
                "not-found",
                `Soil analysis '${a_id}' not found.`,
            )
        }
        return c.json(serialiseSoilAnalysis(analysis), 200)
    }

    const updateSoilAnalysisHandler: RouteHandler<
        typeof updateSoilAnalysisRoute
    > = async (c) => {
        const principal = c.get("principal") as unknown as ApiPrincipalContext
        // @ts-expect-error: @hono/zod-openapi type inference is broken with TypeScript 6 + Zod v4
        const { a_id } = c.req.valid("param") as { a_id: string }
        // @ts-expect-error: @hono/zod-openapi type inference is broken with TypeScript 6 + Zod v4
        const body = c.req.valid("json") as z.infer<
            typeof UpdateSoilAnalysisBodySchema
        >
        ...
        if (a_date !== undefined)
            soilAnalysisData.a_date = new Date(a_date as string)
    ...
    const deleteSoilAnalysisHandler: RouteHandler<
        typeof deleteSoilAnalysisRoute
    > = async (c) => {
        const principal = c.get("principal") as unknown as ApiPrincipalContext
        // @ts-expect-error: @hono/zod-openapi type inference is broken with TypeScript 6 + Zod v4
        const { a_id } = c.req.valid("param") as { a_id: string }
        await services.removeSoilAnalysis(
            fdm,
            principal.effectivePrincipalId,
            a_id,
        )

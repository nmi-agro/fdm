 ...
 * ```
 */
export function createApiKeyAuth(auth: FdmAuth, skipPaths: string[]): MiddlewareHandler<ApiEnv> {
  ...
  return createMiddleware<ApiEnv>(async (c, next) => {
    ...
    return withAuditContext({ channel: "api", credential_id: result.key.id }, () => next())
  })
}

...
describe("GET /fields/:b_id/measures", () => {
  ...
  it("returns 403 when principal lacks access", async () => {
    const app = makeApp({
      getMeasures: vi.fn().mockRejectedValue(new Error("Permission denied")),
    })
    const res = await app.request("/fields/field-1/measures", {
      headers: { "x-api-key": "valid" },
    })
...
describe("POST /fields/:b_id/measures", () => {
  ...
  it("returns 403 when principal lacks permission", async () => {
    const app = makeApp({
      addMeasure: vi.fn().mockRejectedValue(new Error("Permission denied")),
    })
    const res = await app.request("/fields/field-1/measures", {
      method: "POST",
      headers: {
...
describe("GET /measures/:b_id_measure", () => {
  ...
  it("returns 404 when the measure does not exist", async () => {
    const app = makeApp({
      getMeasure: vi.fn().mockResolvedValue({ ...baseMeasure, b_id_measure: undefined }),
    })
  ...
  it("returns 403 when principal lacks access", async () => {
    const app = makeApp({
      getMeasure: vi.fn().mockRejectedValue(new Error("Permission denied")),
    })
    const res = await app.request("/measures/measure-1", {
      headers: { "x-api-key": "valid" },
    })
...
describe("PATCH /measures/:b_id_measure", () => {
  ...
  it("returns 403 when principal lacks permission", async () => {
    const app = makeApp({
      updateMeasure: vi.fn().mockRejectedValue(new Error("Permission denied")),
    })
    const res = await app.request("/measures/measure-1", {
      method: "PATCH",
      headers: {
...
describe("DELETE /measures/:b_id_measure", () => {
  ...
  it("returns 403 when principal lacks permission", async () => {
    const app = makeApp({
      removeMeasure: vi.fn().mockRejectedValue(new Error("Permission denied")),
    })
    const res = await app.request("/measures/measure-1", {
      method: "DELETE",
      headers: { "x-api-key": "valid" },

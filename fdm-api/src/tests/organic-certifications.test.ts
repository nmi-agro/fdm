...
describe("GET /farms/:b_id_farm/organic-certifications", () => {
  ...
  it("returns 200 with paginated certifications", async () => {
    const app = makeApp({
      listOrganicCertifications: vi.fn().mockResolvedValue([baseCertification]),
    })
  ...
  it("returns 403 when principal lacks access", async () => {
    const app = makeApp({
      listOrganicCertifications: vi.fn().mockRejectedValue(new Error("Permission denied")),
    })
    const res = await app.request("/farms/farm-1/organic-certifications", {
      headers: { "x-api-key": "valid" },
    })
...
describe("POST /farms/:b_id_farm/organic-certifications", () => {
  ...
  it("returns 201 with the created certification", async () => {
    const app = makeApp({
      ...
      getOrganicCertification: vi.fn().mockResolvedValue({
        ...baseCertification,
        b_id_organic: "organic-new",
      }),
    })
    const res = await app.request("/farms/farm-1/organic-certifications", {
      method: "POST",
  ...
  it("returns 403 when principal lacks permission", async () => {
    const app = makeApp({
      addOrganicCertification: vi.fn().mockRejectedValue(new Error("Permission denied")),
    })
    const res = await app.request("/farms/farm-1/organic-certifications", {
      method: "POST",
      headers: {
...
describe("GET /organic-certifications/:b_id_organic", () => {
  ...
  it("returns 200 with the certification", async () => {
    const app = makeApp({
      getOrganicCertification: vi.fn().mockResolvedValue(baseCertification),
    })
  ...
  it("returns 404 when the certification does not exist", async () => {
    const app = makeApp({
      getOrganicCertification: vi.fn().mockResolvedValue(undefined),
    })
    const res = await app.request("/organic-certifications/missing", {
      headers: { "x-api-key": "valid" },
    })
  ...
  it("returns 403 when principal lacks access", async () => {
    const app = makeApp({
      getOrganicCertification: vi.fn().mockRejectedValue(new Error("Permission denied")),
    })
    const res = await app.request("/organic-certifications/organic-1", {
      headers: { "x-api-key": "valid" },
    })
...
describe("DELETE /organic-certifications/:b_id_organic", () => {
  ...
  it("returns 204 on success", async () => {
    const app = makeApp({
      removeOrganicCertification: vi.fn().mockResolvedValue(undefined),
    })
    const res = await app.request("/organic-certifications/organic-1", {
      method: "DELETE",
      headers: { "x-api-key": "valid" },
  ...
  it("returns 403 when principal lacks permission", async () => {
    const app = makeApp({
      removeOrganicCertification: vi.fn().mockRejectedValue(new Error("Permission denied")),
    })
    const res = await app.request("/organic-certifications/organic-1", {
      method: "DELETE",
      headers: { "x-api-key": "valid" },

...
describe("requestNutrientAdvice", () => {
  ...
  it("should throw an error if nmiApiKey is not provided", async () => {
    ...
    await expect(requestNutrientAdvice(inputs)).rejects.toThrow("NMI API key not provided")
  ...
  it("should throw an error if NMI API request fails", async () => {
    ...
    await expect(requestNutrientAdvice(inputs)).rejects.toThrow("Request to NMI API failed")
    expect(fetch).toHaveBeenCalledTimes(1)
  })

...
export function calculateWaterSupplyBySom({
   ...
   */
  const calculateWhc = (a_som_loi: number, a_clay_mi: number, a_silt_mi: number): Decimal => {
    ...
    const whc = new Decimal(0.01).add(thetaS.sub(0.01).div(new Decimal(1).add(alpha.pow(n)).pow(m)))

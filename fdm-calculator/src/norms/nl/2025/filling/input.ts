...
export async function collectNL2025InputForFertilizerApplicationFilling(
  ...
  const has_grazing_intention = await getGrazingIntention(fdm, principal_id, b_id_farm, year)
  ...
  const cultivations = await getCultivations(fdm, principal_id, b_id, timeframe2025)

  // 5. Retrieve all fertilizer applications for the farm within the current year's timeframe.
  const applications = await getFertilizerApplications(fdm, principal_id, b_id, timeframe2025)
...
/**
 * Collects all necessary input data for the NL 2025 norms filling for every field on a farm.
 *
 ...
 * @returns A promise that resolves to a `Map<b_id, NL2025NormsFillingInput>` keyed by field ID.
 */
export async function collectNL2025InputForFertilizerApplicationFillingForFarm(
...
): Promise<Map<string, NL2025NormsFillingInput>> {
  const year = 2025
  ...
  const [
    ...
    isOrganicCertificationValid(fdm, principal_id, b_id_farm, new Date(year, 4, 15)),

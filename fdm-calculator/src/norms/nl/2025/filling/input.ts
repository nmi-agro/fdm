 ...
 * receive a unified input object (NL2025NormsFillingInput).
 *
 * Caller needs to supply fosfaatsgebruiksnorm from the `calculateNormForPhosphate` for each field themselves.
 *
 * @param {FdmType} fdm - The FdmType instance for interacting with the Farm Data Model.
 * @param {string} principal_id - The ID of the principal (user or organization) performing the calculation.
 * @param {string} b_id_farm - The ID of the farm, fields of which the norms are being calculated for.
 * @returns {Promise<NL2025NormsFillingInput[]>} A promise that resolves to an array of standardized input
 *   object with each input containing cultivations, fertilizer applications, fertilizers,
 *   organic certification status, grazing intention status, the phosphate usage norm, and the field's
 *   centroid.
 * @throws {Error} Throws an error if the specified field cannot be found.
 */
export async function collectNL2025InputForFertilizerApplicationFillingForFarm(
    fdm: FdmType,
    principal_id: PrincipalId,
    b_id_farm: string,
): Promise<Omit<NL2025NormsFillingInput, "fosfaatgebruiksnorm">[]> {
    // Define the calendar year for the norms calculation.
    const year = 2025
    // Define the timeframe for data collection for the current year.
    const startOfYear = new Date(year, 0, 1) // January 1st of the specified year
    ...
    // 5. Retrieve all fertilizer applications for the farm within the current year's timeframe.
    const applications = await getFertilizerApplicationsForFarm(
        fdm,
        principal_id,
        b_id_farm,
        timeframe2025,
    ...
    return fields.map((field) => ({
        ...
        applications: applications[field.b_id] ?? [],
        fertilizers: fertilizers,
        has_organic_certification: has_organic_certification,
        has_grazing_intention: has_grazing_intention,

...
describe("collectNL2025InputForFertilizerApplicationFilling", () => {
  ...
  it("should successfully collect all input data for a valid scenario", async () => {
    ...
    expect(getField).toHaveBeenCalledWith(mockFdm, mockPrincipalId, mockFieldId)
    expect(getGrazingIntention).toHaveBeenCalledWith(mockFdm, mockPrincipalId, "farm789", 2025)
    ...
    expect(getCultivations).toHaveBeenCalledWith(mockFdm, mockPrincipalId, mockFieldId, {
      start: new Date(2025, 0, 1),
      end: new Date(2025, 11, 31, 23, 59, 59, 999),
    })
    expect(getFertilizerApplications).toHaveBeenCalledWith(mockFdm, mockPrincipalId, "field456", {
      start: new Date(2025, 0, 1),
      end: new Date(2025, 11, 31, 23, 59, 59, 999),
    })
    expect(getFertilizers).toHaveBeenCalledWith(mockFdm, mockPrincipalId, "farm789")
  ...
  it("should throw an error if the field is not found", async () => {
    ...
    ).rejects.toThrow(`Field with id ${mockFieldId} not found for principal ${mockPrincipalId}`)
  })
...
describe("collectNL2025InputForFertilizerApplicationFillingForFarm", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(getFertilizerApplicationsForFarm).mockResolvedValue(
      new Map([
        [mockFieldId, [{ p_app_id: "app1", p_app_amount: 1000 }] as FertilizerApplication[]],
      ]),
    )
  })

...
import type { NL2025NormsFillingInput, WorkingCoefficientDetails } from "./types"
...
export async function calculateNL2025FertilizerApplicationFillingForStikstofGebruiksNorm(
  ...
  const { applications, fertilizers, b_centroid, has_grazing_intention, cultivations } = input
 ...
 */
export function isBouwland(cultivations: Cultivation[], p_app_date: Date): boolean {
  const activeCultivation = cultivations.find((c) => {
    ...
    return p_app_date >= startDate && (endDate === undefined || p_app_date <= endDate)
  })

  if (!activeCultivation || nonBouwlandCodes.includes(activeCultivation.b_lu_catalogue)) {
    return false
  }
...
export function getWorkingCoefficient(
  ...
  for (const entry of table9) {
    if (entry.p_type_rvo.includes(p_type_rvo)) {
      ...
      if (entry.onFarmProduced !== undefined && entry.onFarmProduced !== fertilizerOnFarmProduced) {
      ...
      if (entry.subTypes) {
        const matchingSubType = entry.subTypes.find((subType) => {
          ...
          if (subType.grondsoortCode && !subType.grondsoortCode.includes(soilType as RegionKey)) {
            return false
          }

          if (subType.isBouwland !== undefined && subType.isBouwland !== isBouwland) {
            return false
          }
          ...
          if (subType.applicationPeriod) {
            ...
            if (subType.applicationPeriod === "1 september t/m 31 januari") {
              // September (month 8) to January (month 0)
              if (!((appMonth >= 8 && appMonth <= 11) || (appMonth === 0 && appDay <= 31))) {
                return false
              }
            }
          }
 ...
 * @returns {Promise<NormFilling>} An object containing the total norm filling and details for each application.
 */
export const getNL2025FertilizerApplicationFillingForStikstofGebruiksNorm = withCalculationCache(

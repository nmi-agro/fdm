 ...
 */
export const methodRequirements: Record<NSupplyMethod, { required: string[]; optional: string[] }> =
  {
    /**
...
export function assessDataCompleteness(
  ...
  const availableRequired = required.filter(
    (p) =>
      soilData[p] !== null && soilData[p] !== undefined && soilMeta?.[p]?.source !== NMI_SOURCE,
  ).length
  ...
  const score =
    required.length > 0
      ? (availableRequired / required.length) * 80 +
        (optional.length > 0 ? (availableOptional / optional.length) * 20 : 20)
      : 100

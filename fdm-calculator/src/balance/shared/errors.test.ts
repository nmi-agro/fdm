...
describe("handleInputCollectionError", () => {
  const handleNutrientBalanceInputCollectionError = handleInputCollectionError(
  ...
  it("should wrap errors in the context of a farm", () => {
    ...
    try {
      ...
      const wrapped = handleNutrientBalanceInputCollectionError(error, "test-farm")
  ...
  it("should handle errors that are not instance of Error", () => {
    try {
      ...
      const wrapped = handleNutrientBalanceInputCollectionError(error)
      expect(wrapped.message).toEqual("Failed to collect nutrient balance input: null")

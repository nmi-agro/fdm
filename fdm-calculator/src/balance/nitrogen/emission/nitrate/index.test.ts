...
import { calculateNitrogenEmissionViaNitrate, determineNitrateLeachingFactor } from "."
...
describe("determineNitrateLeachingFactor", () => {
  ...
  it("should return the correct factor for grassland on clay soil", () => {
    const factor = determineNitrateLeachingFactor("grassland", "zeeklei", "III")
    expect(factor.toString()).toBe("0.11")
  })

  it("should return the correct factor for cropland on clay soil", () => {
    const factor = determineNitrateLeachingFactor("cropland", "zeeklei", "III")
  ...
  it("should return the correct factor for all sandy soil GWL classes", () => {
    ...
    for (const [gwlClass, factors] of Object.entries(gwlClasses)) {
      ...
      const croplandFactor = determineNitrateLeachingFactor("cropland", "dekzand", gwlClass as any)
      expect(croplandFactor.toString()).toBe(factors.cropland)
    }
  })

  it("should throw an error for unknown soil type", () => {
    expect(() => determineNitrateLeachingFactor("grassland", "unknown" as any, "V")).toThrow(
      "Unknown soil type: unknown",
    )
  })

  it("should throw an error for unknown GWL class on sandy soil", () => {
    expect(() => determineNitrateLeachingFactor("grassland", "dekzand", "unknown" as any)).toThrow(
      "Unknown GWL class 'unknown' for sandy soil 'dekzand'",
    )
  })

  it("should throw an error for unknown land type", () => {
    expect(() => determineNitrateLeachingFactor("unknown" as any, "dekzand", "V")).toThrow(

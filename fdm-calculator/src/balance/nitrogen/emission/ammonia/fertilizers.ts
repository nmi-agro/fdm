...
export function calculateNitrogenEmissionViaAmmoniaByFertilizers(
  ...
  const aggregatedEmissions = fertilizerApplications.reduce((acc, application) => {
    ...
    switch (fertilizerDetail.p_type) {
      case "mineral": {
        const p_n_rt_mineral = new Decimal(fertilizerDetail.p_n_rt ?? 0)
        const p_ef_nh3_mineral = fertilizerDetail.p_ef_nh3

        if (p_ef_nh3_mineral != null) {
          emissionFactor = new Decimal(p_ef_nh3_mineral)
        } else {
          emissionFactor = determineMineralAmmoniaEmissionFactor(fertilizerDetail)
        }
      ...
      default: {
        ...
        const p_nh4_rt_organic = new Decimal(fertilizerDetail.p_nh4_rt ?? 0)
 ...
 */
function determineMineralAmmoniaEmissionFactor(fertilizerDetail: FertilizerDetail): Decimal {
...
function determineManureAmmoniaEmissionFactor(
  fertilizerApplication: BaseFertilizerApplication,
  cultivations: FieldInput["cultivations"],
  ...
  switch (p_app_method) {
    ...
    default:
      throw new Error(`Unsupported application method ${p_app_method} for ${p_app_name} (${p_id})`)
  }
}

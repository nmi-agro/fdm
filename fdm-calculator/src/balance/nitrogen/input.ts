...
async function collectInputForNitrogenBalanceForFarm(
  ...
  try {
    ...
    const dbResult: {
      ...
      if (b_id) {
        ...
        const harvestPromises = cultivations.map(async (cultivation) => {
          return await getHarvests(tx, principal_id, cultivation.b_lu, timeframe)
        })
        const harvestArrays = await Promise.all(harvestPromises)
        const harvests = harvestArrays.flat()
        const harvestsFiltered = harvests.filter((harvest) => harvest.b_lu !== undefined)
      ...
      return {
        ...
        fieldData: farmFields.map((field) => {
          ...
          const harvests = cultivations.flatMap((c) => harvestsByField.get(c.b_lu) ?? [])
          return {
            field,
            cultivations,
            harvests,
    ...
    const depositionByField = await calculateAllFieldsNitrogenSupplyByDeposition(
      dbResult.farmFields,
      timeframe,
      fdmPublicDataUrl,
    )
    ...
    return await fdm.transaction(async (tx: FdmType) => {
      ...
      const farmSettled = await Promise.allSettled(
        uniqueFarmIds.map(async (b_id_farm) => {
          const onlyFieldInput = await collectInputForNitrogenBalanceForFarm(
            tx,
            principal_id,
            b_id_farm,
            timeframe,
      ...
      return farmSettled
        .filter((result) => {
          if (result.status === "rejected") {
            console.error(handleNitrogenBalanceInputCollectionError(result.reason).message)
            return false
          }
          return true
        })
        .map(
          (result) =>
            (result as PromiseFulfilledResult<NitrogenBalanceInput & { b_id_farm: string }>).value,
        )
    })
  } catch (error) {
  ...
  try {
    return await fdm.transaction(async (tx) => {
      if (!timeframe.start || !timeframe.end) {
        throw new Error("Timeframe start and end must be provided for nitrogen balance calculation")
      }
...
export const handleNitrogenBalanceInputCollectionError = handleInputCollectionError(

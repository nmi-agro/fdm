...
describe("aggregateFarmNitrogenBalanceResults", () => {
    const result1: NitrogenBalanceNumeric = {
        balance: 100,
        supply: {
            total: 50,
            deposition: 30,
    ...
    const result2: NitrogenBalanceNumeric = {
        balance: 145,
        supply: {
            total: 95,
            deposition: 20,
        ...
        fields: [
            {
                b_id: "test-field-id-2-1",
                b_area: 0.3,
                b_bufferstrip: false,
            },
            {
                b_id: "test-field-id-2-2",
                b_area: 0.7,
                b_bufferstrip: true,
            },
        ],
    ...
    it("aggregates one farm result correctly", () => {
        expect(aggregateFarmNitrogenBalanceResults([result1])).toEqual(result1)
    })

    it("aggregates two farm results correctly", () => {
        expect(aggregateFarmNitrogenBalanceResults([result1, result2])).toEqual(
            {
                balance: 130,
                supply: {
                    total: 80,
                ...
                hasErrors: false,
                fieldErrorMessages: [],
            } satisfies ReturnType<typeof aggregateFarmNitrogenBalanceResults>,
        )
    })
        ...
        expect(
            aggregateFarmNitrogenBalanceResults([
                result1,
                {
                    fields: [] as unknown[],
                    hasErrors: true,
                    fieldErrorMessages: ["cultivation glowberries not found"],
    ...
    it("should handle no valid results", () => {
        const combined = aggregateFarmNitrogenBalanceResults([
            { hasErrors: true } as NitrogenBalanceNumeric,
        ])

        expect(combined.hasErrors).toBeTruthy()
        expect(combined.fieldErrorMessages).toContain(

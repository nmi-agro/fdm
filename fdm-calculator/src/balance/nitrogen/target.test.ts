...
describe("calculateTargetForNitrogenBalance", () => {
  ...
  const createSoilAnalysis = (b_soiltype_agr: string, b_gwl_class: string): SoilAnalysisPicked => ({
    b_soiltype_agr,
    b_gwl_class,
    a_c_of: 20,
    a_cn_fr: 10,
  ...
  it("should calculate target for arable land on dry sandy soil", () => {
    const cultivations: FieldInput["cultivations"] = [createCultivation("crop1", "cereal")]
  ...
  it("should adjust target value based on time frame", () => {
    const cultivations: FieldInput["cultivations"] = [createCultivation("grass1", "grass")]
    const soilAnalysis = createSoilAnalysis("duinzand", "VIIo")
    const cultivationDetailsMap = createCultivationDetailsMap("grass1", "grass")

...
export async function calculateAllFieldsNitrogenSupplyByDeposition(
  ...
  const depositionPromises = fields.map(async (field) => {
    ...
    if (timeFrame.start && timeFrame.end) {
      ...
      const effectiveStart = new Date(Math.max(fStart.getTime(), timeFrame.start.getTime()))
      const effectiveEnd = new Date(Math.min(fEnd.getTime(), timeFrame.end.getTime()))
      const days = differenceInCalendarDays(effectiveEnd, effectiveStart)
      fraction = days >= 0 ? new Decimal(days).add(1).dividedBy(365) : new Decimal(0)
    }

...
describe("collectInputForOrganicMatterBalance", () => {
  ...
  it("should collect input for all fields in a farm", async () => {
    ...
    vi.spyOn(fdmCore, "getCultivationsForFarm").mockResolvedValue(
      new Map(
        (mockFields as any[]).map((f: { b_id: string }) => [f.b_id, mockCultivationsData]),
      ) as any,
    )
    vi.spyOn(fdmCore, "getSoilAnalysesForFarm").mockResolvedValue(new Map() as any)
    ...
    expect(fdmCore.getFields).toHaveBeenCalledWith(mockFdm, principal_id, b_id_farm, timeframe)
  ...
  it("should collect input for a single specified field", async () => {
    ...
    expect(fdmCore.getField).toHaveBeenCalledWith(mockFdm, principal_id, "field1")
  ...
  it("should correctly structure the output", async () => {
    ...
    vi.spyOn(fdmCore, "getFertilizersFromCatalogue").mockResolvedValue([mockFertilizer] as any)
    vi.spyOn(fdmCore, "getCultivationsFromCatalogue").mockResolvedValue([mockCultivation] as any)
...
describe("collectInputForOrganicMatterBalanceForFarms", () => {
  ...
  it("should collect cultivation details only once", async () => {
    ...
    // Setup mocks
    vi.spyOn(fdmCore, "getFields").mockImplementation(async (_1, _2, b_id_farm) =>
      b_id_farm === "test-farm-id-2" ? mockFieldsData2 : mockFieldsData,
    )
    vi.spyOn(fdmCore, "getCultivationsForFarm").mockImplementation(async (_1, _2, b_id_farm) =>
      b_id_farm === "test-farm-id-2"
        ? new Map(mockFieldsData2.map((f) => [f.b_id, mockCultivationsData2]))
        : new Map(mockFieldsData.map((f) => [f.b_id, mockCultivationsData])),
    )
    vi.spyOn(fdmCore, "getSoilAnalysesForFarm").mockImplementation(async (_1, _2, b_id_farm) =>
      b_id_farm === "test-farm-id-2"
        ? (new Map(
            mockFieldsData2.map(
    ...
    vi.spyOn(fdmCore, "getEnabledCultivationCataloguesForFarms").mockResolvedValue({
      "test-farm-id": ["brp"],
      "test-farm-id-2": ["brp"],
    })
    vi.spyOn(fdmCore, "getEnabledFertilizerCataloguesForFarms").mockResolvedValue({
    ...
    vi.spyOn(fdmCore, "getFertilizersFromCatalogues").mockResolvedValue(allFertilizerDetails as any)
    ...
    expect(fdmCore.getEnabledCultivationCataloguesForFarms).toHaveBeenCalledWith(
      mockFdm,
      principal_id,
      ["test-farm-id", "test-farm-id-2"],
    )
    expect(fdmCore.getEnabledCultivationCataloguesForFarms).toHaveBeenCalledTimes(1)
    expect(fdmCore.getEnabledFertilizerCataloguesForFarms).toHaveBeenCalledWith(
      mockFdm,
      principal_id,
      ["test-farm-id", "test-farm-id-2"],
    )
    expect(fdmCore.getEnabledFertilizerCataloguesForFarms).toHaveBeenCalledTimes(1)

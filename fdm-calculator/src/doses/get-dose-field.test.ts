...
describe("getDoseForField", () => {
  ...
  it("should calculate the correct dose for a field with a single application", async () => {
    ...
    p_id = await addFertilizer(fdm, principal_id, p_id_catalogue, b_id_farm, 1000, new Date())
    await addFertilizerApplication(fdm, principal_id, b_id, p_id, 100, undefined, new Date())
    ...
    expect(await getDoseForField({ fdm, principal_id, b_id })).toEqual(expectedResult)
  })

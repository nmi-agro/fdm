...
describe("geotiff module", () => {
  ...
  describe("getTiff", () => {
    ...
    it("should respect abort signal", async () => {
      ...
      await expect(getTiff("http://example.com/abort.tif", controller.signal)).rejects.toThrow(
        /Aborted by caller/,
      )
    })
  })
  ...
  describe("getGeoTiffValue", () => {
    ...
    it("should respect abort signal when acquiring semaphore", async () => {
      ...
      await expect(getGeoTiffValue(url, 50, 50, controller.signal)).rejects.toThrow(
        "Manual abort reason",
      )
    })
    ...
    it("should handle simultaneous calls via semaphore queue", async () => {
      ...
      const promises = Array.from({ length: 15 }).map(() => getGeoTiffValue(url, 50, 50))

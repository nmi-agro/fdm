  ...
  const promise = (async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(`HEAD request failed for ${url}, falling back to fromUrl`, headErr)
      }
      ...
      throw new Error(`Failed to fetch or parse GeoTIFF from ${url}: ${reason}`, { cause: error })
    }
  })()
...
export async function getGeoTiffValue(
  ...
  try {
    ...
    if (
      value !== undefined &&
      (noDataValue === null ||
        (Number.isNaN(noDataValue as number) ? !Number.isNaN(value) : value !== noDataValue))
    ) {
      return value
    }

...
export function compareFields(
  ...
  const processMatch = (local: FieldWithCultivations, rvo: RvoField) => {
    ...
    const localCultivation = local.cultivations
      ...
      : undefined
    const rvoCode = rvo.properties.CropTypeCode ? `nl_${rvo.properties.CropTypeCode}` : undefined

    let rvoCultivationInfo: { b_lu_catalogue: string; b_lu_name: string } | undefined
    ...
    if (rvoCode) {
      const rvoCatalogueEntry = cultivationsCatalogue.find((c) => c.b_lu_catalogue === rvoCode)
      rvoCultivationInfo = {
        b_lu_catalogue: rvoCode,
        b_lu_name: rvoCatalogueEntry ? rvoCatalogueEntry.b_lu_name : rvoCode,
      }
    }
    ...
    return {
      status: diffs.length > 0 ? RvoImportReviewStatus.CONFLICT : RvoImportReviewStatus.MATCH,
      localField: local,
      rvoField: rvo,
  ...
  for (const local of localFields) {
    if (local.b_id_source) {
      const rvoMatch = rvoFields.find((r) => r.properties.CropFieldID === local.b_id_source)
      if (rvoMatch) {
  ...
  for (const rvo of rvoFields) {
    ...
    const candidates = remainingLocals.filter(
      (l) => !matchedLocalIds.has(l.field.b_id) && bboxOverlap(l.bbox, rvoBbox),
    )
    ...
    if (bestMatch && bestIoU > IOU_THRESHOLD) {
      ...
      const rvoCatalogueEntry = rvoCode
        ? cultivationsCatalogue.find((c) => c.b_lu_catalogue === rvoCode)
        : undefined
      results.push({
...
function detectDiffs(local: Field, rvo: RvoField): FieldDiff[] {
  ...
  if (local.b_name !== rvo.properties.CropFieldDesignator && rvo.properties.CropFieldDesignator) {
    diffs.push("b_name")
  }
  ...
  // 3. Dates (Start)
  const localStart =
    local.b_start instanceof Date ? local.b_start.toISOString().split("T")[0] : local.b_start

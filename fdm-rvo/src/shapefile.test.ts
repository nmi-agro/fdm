...
describe("getRvoFieldsFromShapefile", () => {
  ...
  it("should project field geometry", async () => {
    ...
    const expectedCoords = [
      createMockGeometry().coordinates[0].map((coord) => projector.forward(coord)),
    ]

    expect(parsed).toHaveLength(1)

    expect((parsed[0].geometry as Geometry).type).toBe("Polygon")
    expect((parsed[0].geometry as Polygon).coordinates).toStrictEqual(expectedCoords)
  })

  it("should throw an error with invalid geometry", async () => {
    vi.mocked(shpjs.parseShp).mockRejectedValueOnce(new Error("Failed to parse shp"))
  ...
  it("should throw an error with invalid attributes", async () => {
    ...
    vi.mocked(shpjs.parseDbf).mockRejectedValueOnce(new Error("Failed to parse shp"))
  ...
  it("should handle multi-polygon geometry", async () => {
    ...
    expect((parsed[0].geometry as Polygon).coordinates).toStrictEqual(MOCK_MULTIPOLYGON.coordinates)
  })

  it("should handle not-supported geometry", async () => {
    const MOCK_UNSUPPORTED_GEOMETRY = geometry("Point", [0, 0])
    vi.mocked(shpjs.parseShp).mockResolvedValueOnce([MOCK_UNSUPPORTED_GEOMETRY])
    vi.mocked(shpjs.parseDbf).mockResolvedValueOnce([MOCK_PROPERTIES])
...
describe("convertShapefileFeatureIntoRvoField", () => {
  ...
  it("should map the data fields correctly", () => {
    ...
    expect(new Date(parsed.properties.BeginDate).getTime()).toBe(1704067200000)
    expect(new Date(parsed.properties.EndDate ?? "").getTime()).toBe(1706659200000)
    expect(parsed.properties.CropTypeCode).toBe("02")
    expect(parsed.properties.UseTitleCode).toBe("Geliberaliseerde pacht, 6 jaar of korter")
  })

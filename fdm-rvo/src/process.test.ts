...
describe("processRvoImport", () => {
  ...
  it("should process ADD_REMOTE action", async () => {
    ...
    await processRvoImport(mockFdm, principalId, farmId, [item], choices, year, onFieldAdded)
    ...
    expect(onFieldAdded).toHaveBeenCalledWith(mockFdm, "new-field-id", expect.anything())
  })
  ...
  it("should process UPDATE_FROM_REMOTE action with cultivation change", async () => {
    ...
    expect(updateField).toHaveBeenCalled()
    expect(removeCultivation).toHaveBeenCalledWith(mockFdm, principalId, "cult-1")
    expect(addCultivation).toHaveBeenCalledWith(
      mockFdm,
      principalId,
      "nl_101",
  ...
  it("should process REMOVE_LOCAL action", async () => {
    ...
    expect(removeField).toHaveBeenCalledWith(mockFdm, principalId, "local-1")
  })

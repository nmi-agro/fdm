...
export const FertilizerApplicationSchema = z.object({
  ...
  p_app_amount_display: z.number().describe("Application amount in the native display unit"),
  p_app_amount_unit: z
...
export const FieldPlanEntrySchema = z.object({
  ...
  fieldSummary: z.string().describe("Brief Dutch explanation (≤ 75 words) specific to this field"),
  applications: z
...
const FarmTotalsSchema = z.object({
  normsFilling: NormValuesSchema.describe("Current farm-level norm fillings in kg"),
...
export const FertilizerPlanSchema = z.object({
  ...
  plan: z.array(FieldPlanEntrySchema).describe("One entry per field with fertilizer applications"),
})

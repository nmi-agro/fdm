...
4. ORGANIC MATTER: Aim for a positive organic matter balance (organische stofbalans) on every field. Prioritize compost ("p_type": "compost") or high-EOM organic fertilizers where the balance is at risk. The simulation tool will return the net balance in kg EOM/ha (positive is better). However, when legal norms constrain the available nutrient space, fulfilling NPK advice (especially nitrogen) ALWAYS takes priority over building organic matter content or achieving a positive OM balance. Do not sacrifice NPK fulfillment to improve the OM balance.

...
export const ClarifyingQuestionOptionSchema = z.object({
    id: z.string().describe("Kort uniek ID voor deze optie (bijv. 'a', 'b', 'c', 'd')"),
    label: z.string().describe("De tekst van de optie zoals getoond aan de gebruiker (Nederlands, max 80 tekens)"),
})
...
export const ClarifyingQuestionSchema = z.object({
    ...
    question: z.string().describe("De vraag in het Nederlands, gericht aan de agronomisch adviseur of teler (max 120 tekens)"),
    rationale: z.string().optional().describe("Interne redenering waarom deze vraag relevant is voor het plan (niet getoond aan gebruiker)"),
    selection: z.enum(["single", "multi"]).describe("'single' als slechts één optie van toepassing is; 'multi' als meerdere opties tegelijk mogelijk zijn"),
    options: z
        .array(ClarifyingQuestionOptionSchema)
        .min(2)
        .max(4)
        .describe("2 tot 4 antwoordopties (exclusief het vrije 'Anders'-veld)"),
    allowOther: z.boolean().default(true).describe("Altijd true — er is altijd een vrije 'Anders, namelijk…'-optie"),
})
...
export const ClarifyingQuestionsSchema = z.object({
    questions: z
        ...
        .max(5)
        .describe("0 tot 5 relevante verduidelijkingsvragen. Geef een lege lijst als er niets te verduidelijken is."),
})
...
export const ClarificationAnswerSchema = z.object({
    ...
    other: z.string().optional().describe("Ingevulde tekst als de gebruiker 'Anders' heeft gekozen"),
})

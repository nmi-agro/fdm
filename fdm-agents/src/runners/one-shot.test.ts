...
describe("runOneShotAgent", () => {
  ...
  it("should throw when the stream throws an error", async () => {
    const agent = createThrowingAgent(new Error("Gemini API error [400]: Bad Request"))
  ...
  it("should extract tool call names from tools node updates", async () => {
    const agent = createMockAgent([
      ["updates", { tools: { messages: [makeToolMessage("getFarmFields")] } }],
      ["updates", { model_request: { messages: [makeAIMessage("Done.")] } }],
    ])
    const result = await runOneShotAgent(agent, "Generate plan")
  ...
  it("should deduplicate repeated tool call names", async () => {
    const agent = createMockAgent([
      ["updates", { tools: { messages: [makeToolMessage("searchFertilizers")] } }],
      ["updates", { tools: { messages: [makeToolMessage("searchFertilizers")] } }],
      ["updates", { model_request: { messages: [makeAIMessage("Done.")] } }],
    ])
    const result = await runOneShotAgent(agent, "Generate plan")
  ...
  it("should throw AgentTimeoutError when the agent exceeds timeoutMs", async () => {
    ...
    const runPromise = runOneShotAgent(neverYieldingAgent, "Generate plan", {}, undefined, 5000)
  ...
  it("should wrap LangGraph recursion limit errors in AgentRecursionLimitError", async () => {
    const agent = createThrowingAgent(
      new Error("Recursion limit of 100 reached without hitting a stop condition."),
    )
    await expect(runOneShotAgent(agent, "Generate plan")).rejects.toThrow(AgentRecursionLimitError)
  })
  ...
  it("should not wrap non-recursion errors in AgentRecursionLimitError", async () => {
    ...
    await expect(runOneShotAgent(agent, "Generate plan")).rejects.toThrow("Some other error")
  })
})

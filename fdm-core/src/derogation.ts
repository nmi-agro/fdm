...
export async function addDerogation(
  ...
  try {
    return await fdm.transaction(async (tx) => {
      await checkPermission(tx, "farm", "write", b_id_farm, principal_id, "addDerogation")
      ...
      const existingDerogation = await tx
        ...
        .leftJoin(
          ...
          eq(schema.derogations.b_id_derogation, schema.derogationApplying.b_id_derogation),
        )
        .where(
          and(
            eq(schema.derogationApplying.b_id_farm, b_id_farm),
            eq(schema.derogations.b_derogation_year, b_derogation_year),
          ),
        )
        .limit(1)

      if (existingDerogation.length > 0) {
        throw new Error("Derogation already granted for this farm and year.")
      }
  ...
  try {
    await fdm.transaction(async (tx) => {
      const application = await tx
        ...
        .where(eq(schema.derogationApplying.b_id_derogation, b_id_derogation))
...
export async function listDerogations(
  ...
  try {
    return await fdm.transaction(async (tx) => {
      await checkPermission(tx, "farm", "read", b_id_farm, principal_id, "listDerogations")
      ...
      return await tx
        .select()
        .from(schema.derogations)
        .where(inArray(schema.derogations.b_id_derogation, derogationIds))
    })
  } catch (err) {

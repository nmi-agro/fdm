...
describe("Measure Data Model", () => {
  ...
  beforeEach(async () => {
    ...
    await syncMeasuresCatalogueArray(fdm, [TEST_CATALOGUE_ITEM, TEST_CATALOGUE_ITEM_2])
  ...
  describe("addMeasure", () => {
    ...
    it("should throw for unknown m_id (FK violation)", async () => {
      await expect(
        addMeasure(fdm, principal_id, b_id, "bln_UNKNOWN", new Date("2023-03-01")),
      ).rejects.toThrow()
    })
    ...
    it("should throw when an existing doorlopend measure of the same m_id is present", async () => {
      ...
      await addMeasure(fdm, principal_id, b_id, "bln_BM1", new Date("2023-01-01"))
      await expect(
        addMeasure(
          fdm,
          principal_id,
    ...
    it("should throw when the catalogue is not enabled for the farm", async () => {
      ...
      await expect(
        addMeasure(fdm, principal_id, other_field, "bln_BM1", new Date("2023-03-01")),
      ).rejects.toMatchObject({
  ...
  describe("getMeasure", () => {
    ...
    it("should throw when b_id_measure does not exist", async () => {
      await expect(getMeasure(fdm, principal_id, "nonexistent")).rejects.toThrow()
    })
  })

  describe("getMeasures", () => {
    it("should return all measures for a field", async () => {
      await addMeasure(fdm, principal_id, b_id, "bln_BM1", new Date("2023-03-01"))
      await addMeasure(fdm, principal_id, b_id, "bln_BM2", new Date("2023-05-01"))
    ...
    it("should filter by timeframe — exclude measures outside range", async () => {
      ...
      await addMeasure(fdm, principal_id, b_id, "bln_BM2", new Date("2022-01-01"))
  ...
  describe("updateMeasure", () => {
    it("should update start date", async () => {
      ...
      await updateMeasure(fdm, principal_id, b_id_measure, new Date("2023-04-01"))
    ...
    it("should clear m_end when passed null (doorlopend)", async () => {
      ...
      await updateMeasure(fdm, principal_id, b_id_measure, undefined, null)
    ...
    it("should throw for unknown b_id_measure", async () => {
      await expect(updateMeasure(fdm, principal_id, "nonexistent")).rejects.toThrow()
    })
    ...
    it("should throw when m_end is earlier than m_start", async () => {
      ...
      await expect(
        updateMeasure(fdm, principal_id, b_id_measure, undefined, new Date("2023-01-01")),
      ).rejects.toMatchObject({
    ...
    it("should throw when update would create an overlap with a sibling measure", async () => {
      ...
      await expect(
        updateMeasure(fdm, principal_id, b_id_measure2, new Date("2023-04-01")),
      ).rejects.toMatchObject({
        cause: {
          message: expect.stringContaining("overlapping time window"),
        },
  ...
  describe("removeMeasure", () => {
    ...
    it("should throw for unknown b_id_measure", async () => {
      await expect(removeMeasure(fdm, principal_id, "nonexistent")).rejects.toThrow()
    })
  })
})

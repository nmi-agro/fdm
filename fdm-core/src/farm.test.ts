...
describe("Farm Functions", () => {
  ...
  describe("getFarm", () => {
    ...
    it("should throw an error if the principal does not have read access", async () => {
      ...
      await expect(getFarm(fdm, other_principal_id, b_id_farm)).rejects.toThrowError(
    ...
    it("should handle errors during farm retrieval", async () => {
      ...
      await expect(getFarm(fdmMock, principal_id, b_id_farm)).rejects.toThrowError(
        "Exception for getFarm",
      )
    })
  })
  ...
  describe("getFarms", () => {
    it("should retrieve a list of farms accessible by the principal", async () => {
      ...
      expect(farms.some((farm) => farm.b_id_farm === b_id_farm)).toBeTruthy() // Assert that the test farm is in the list
    })
    ...
    it("should handle errors during farm list retrieval", async () => {
      ...
      await expect(getFarms(asFdm(authorizationMock), principal_id)).rejects.toThrowError(
        "Exception for getFarms",
      )
    })
  })
  ...
  describe("grantRoleToFarm", () => {
    it("should create an invitation for a principal for a given farm", async () => {
      ...
      const invitation = invitations.find((i) => i.target_principal_id === target_id)
      expect(invitation).toBeDefined()
      expect(invitation?.role).toBe("advisor")
      expect(invitation?.status).toBe("pending")

      if (!invitation) {
        throw new Error("Test did not create an invitation as expected, cannot continue with test")
      }
      ...
      const principals = await listPrincipalsForResource(fdm, "farm", b_id_farm)
    ...
    it("should throw an error if the principal does not have share permission", async () => {
      const other_principal_id = createId()
      await expect(
        grantRoleToFarm(fdm, other_principal_id, target_username, b_id_farm, "advisor"),
      ).rejects.toThrowError("Principal does not have permission to perform this action")
    })
    ...
    it("should handle errors during the grant role process", async () => {
      ...
      await expect(
        grantRoleToFarm(asFdm(fdmMock), principal_id, target_username, b_id_farm, "advisor"),
    ...
    it("should throw an error if the target principal does not exist", async () => {
      ...
      await expect(
        grantRoleToFarm(fdm, principal_id, nonExistentUsername, b_id_farm, "advisor"),
      ).rejects.toThrowError("Exception for grantRoleToFarm")
    })
    ...
    it("should throw an error if target is already a member of the farm", async () => {
      ...
      await expect(
        grantRoleToFarm(fdm, principal_id, target_username, b_id_farm, "advisor"),
      ).rejects.toThrowError("Exception for grantRoleToFarm")
    })
    ...
    it("should create an email-based invitation for an unregistered email", async () => {
      ...
      await grantRoleToFarm(fdm, principal_id, unregisteredEmail, b_id_farm, "researcher")

      const invitations = await fdm
        .select()
        .from(authZSchema.invitation)
        .where(eq(authZSchema.invitation.target_email, unregisteredEmail))
  ...
  describe("updateRoleOfPrincipalAtFarm", () => {
    it("should update a role to a principal for a given farm", async () => {
      await updateRoleOfPrincipalAtFarm(fdm, principal_id, target_username, b_id_farm, "researcher")
    ...
    it("should throw an error if target principal does not exist", async () => {
      const nonExistentUsername = "nonexistentuser"
      await expect(
        updateRoleOfPrincipalAtFarm(fdm, principal_id, nonExistentUsername, b_id_farm, "advisor"),
      ).rejects.toThrowError("Exception for updateRoleOfPrincipalAtFarm")
    })
  })
  ...
  describe("revokePrincipalFromFarm", () => {
    ...
    it("should throw an error if the principal does not have share permission", async () => {
      const other_principal_id = createId()
      await expect(
        revokePrincipalFromFarm(fdm, other_principal_id, target_username, b_id_farm),
      ).rejects.toThrowError("Principal does not have permission to perform this action")
    })
    ...
    it("should handle errors during the revoke principal process", async () => {
      ...
      await expect(
        revokePrincipalFromFarm(asFdm(fdmMock), principal_id, target_username, b_id_farm),
    ...
    it("should throw an error if the target principal does not exist", async () => {
      ...
      await expect(
        revokePrincipalFromFarm(fdm, principal_id, nonExistentUsername, b_id_farm),
      ).rejects.toThrowError("Exception for revokePrincipalFromFarm")
    })
    ...
    it("should throw an error if trying to revoke the last owner", async () => {
      ...
      await expect(
        revokePrincipalFromFarm(fdm, principal_id, principal_id, b_id_farm),
      ).rejects.toThrowError("Exception for revokePrincipalFromFarm")
    })
  })
  ...
  describe("listPrincipalsForFarm", () => {
    it("should list principals associated with a farm", async () => {
      const principals = await listPrincipalsForFarm(fdm, principal_id, b_id_farm)
      ...
      const owner = principals.find((p) => p?.username === ownerPrincipal?.username)
      ...
      const advisor = principals.find((p) => p?.username === targetPrincipal?.username)
    ...
    it("should throw an error if the principal does not have read access", async () => {
      const other_principal_id = createId()
      await expect(listPrincipalsForFarm(fdm, other_principal_id, b_id_farm)).rejects.toThrowError(
        "Principal does not have permission to perform this action",
      )
    })
    ...
    it("should include invitation_id and invitation_expires_at for pending principals", async () => {
      ...
      await grantRoleToFarm(fdm, principal_id, target_username, pendingFarmId, "advisor")

      const principals = await listPrincipalsForFarm(fdm, principal_id, pendingFarmId)
  ...
  describe("isAllowedToShareFarm", () => {
    it("should return true if principal is allowed to share the farm", async () => {
      const isAllowed = await isAllowedToShareFarm(fdm, principal_id, b_id_farm)
    ...
    it("should return false if principal is not allowed to share the farm", async () => {
      ...
      const isAllowed = await isAllowedToShareFarm(fdm, other_principal_id, b_id_farm)
  ...
  describe("isAllowedToDeleteFarm", () => {
    it("should return true if principal is allowed to delete the farm", async () => {
      const isAllowed = await isAllowedToDeleteFarm(fdm, principal_id, b_id_farm)
      expect(isAllowed).toBe(true)
    })
    ...
    it("should return false if principal is not allowed to delete the farm", async () => {
      ...
      const isAllowed = await isAllowedToDeleteFarm(fdm, other_principal_id, b_id_farm)
  ...
  describe("removeFarm", () => {
    ...
    it("should successfully remove a farm and all its associated data", async () => {
      ...
      await expect(getFarm(fdm, testPrincipalId, testFarmId)).rejects.toThrowError(
      ...
      await expect(getFields(fdm, testPrincipalId, testFarmId)).rejects.toThrowError(
        "Principal does not have permission to perform this action",
      )
      ...
      const customFertilizerCatalogue = await fdm
        ...
        .where(eq(schema.fertilizersCatalogue.p_id_catalogue, testFertilizerCatalogueId))
      expect(customFertilizerCatalogue).toEqual([])

      // Verify principals are revoked
      const principals = await listPrincipalsForResource(fdm, "farm", testFarmId)
    ...
    it("should throw an error if the principal does not have write access", async () => {
      ...
      await expect(removeFarm(fdm, other_principal_id, newFarmId)).rejects.toThrowError(
        "Principal does not have permission to perform this action",
      )
    })
    ...
    it("should handle errors during farm removal", async () => {
      ...
      await expect(removeFarm(fdmMock, principal_id, newFarmId)).rejects.toThrowError(
        "Exception for removeFarm",
      )
    })

    it("should throw an error if the farm does not exist", async () => {
      const nonExistentFarmId = createId()
      await expect(removeFarm(fdm, principal_id, nonExistentFarmId)).rejects.toThrowError(
        "Principal does not have permission to perform this action",
      )
    })
  })
  ...
  describe("acceptInvitation", () => {
    ...
    it("should accept a pending invitation and grant the role", async () => {
      await grantRoleToFarm(fdm, principal_id, target_username, invitationFarmId, "researcher")
      ...
      const principals = await listPrincipalsForResource(fdm, "farm", invitationFarmId)
    ...
    it("should throw if invitation is already accepted", async () => {
      await expect(acceptInvitation(fdm, invitationId, target_id)).rejects.toThrowError(
        "Exception for acceptInvitation",
      )
    })

    it("should throw if invitation does not exist", async () => {
      await expect(acceptInvitation(fdm, createId(), target_id)).rejects.toThrowError(
        "Exception for acceptInvitation",
      )
    })
    ...
    it("should throw if the user is not the invitation target", async () => {
      ...
      await grantRoleToFarm(fdm, principal_id, target_username, otherFarmId, "advisor")
      const rows = await fdm
        .select()
        .from(authZSchema.invitation)
  ...
  describe("declineInvitation", () => {
    ...
    beforeAll(async () => {
      ...
      await grantRoleToFarm(fdm, principal_id, target_username, declineFarmId, "advisor")
      const rows = await fdm
        .select()
        .from(authZSchema.invitation)
    ...
    it("should decline a pending invitation", async () => {
      ...
      const rows = await fdm
        .select()
        .from(authZSchema.invitation)
        .where(eq(authZSchema.invitation.invitation_id, declineInvitationId))
      expect(rows[0].status).toBe("declined")
    })

    it("should throw if invitation is already declined", async () => {
      await expect(declineInvitation(fdm, declineInvitationId, target_id)).rejects.toThrowError(
        "Exception for declineInvitation",
      )
    })
    ...
    it("should throw if the user is not the invitation target", async () => {
      ...
      await grantRoleToFarm(fdm, principal_id, target_username, anotherFarmId, "advisor")
      const rows = await fdm
        .select()
        .from(authZSchema.invitation)
  ...
  describe("listPendingInvitationsForFarm", () => {
    ...
    beforeAll(async () => {
      ...
      await grantRoleToFarm(fdm, principal_id, target_username, listFarmId, "advisor")
    })

    it("should return pending invitations for a farm", async () => {
      const invitations = await listPendingInvitationsForFarm(fdm, principal_id, listFarmId)
      expect(invitations.length).toBeGreaterThanOrEqual(1)
      expect(invitations[0].status).toBe("pending")
    ...
    it("should throw if principal does not have share permission", async () => {
      ...
      await expect(listPendingInvitationsForFarm(fdm, otherId, listFarmId)).rejects.toThrowError(
        "Principal does not have permission to perform this action",
      )
    })
  })
  ...
  describe("cancelInvitationForFarm", () => {
    ...
    beforeAll(async () => {
      ...
      await grantRoleToFarm(fdm, principal_id, target_username, cancelFarmId, "advisor")
      const invitations = await listPendingInvitationsForFarm(fdm, principal_id, cancelFarmId)
    ...
    it("should remove the invitation from pending list after cancellation", async () => {
      ...
      const invitations = await listPendingInvitationsForFarm(fdm, principal_id, cancelFarmId)
      const found = invitations.find((i) => i.invitation_id === cancelInvitationId)
    ...
    it("should throw if the invitation does not exist", async () => {
      await expect(cancelInvitationForFarm(fdm, principal_id, createId())).rejects.toThrowError(
        "Exception for cancelInvitationForFarm",
      )
    })
    ...
    it("should throw if the principal does not have share permission", async () => {
      ...
      await grantRoleToFarm(fdm, principal_id, target_username, anotherCancelFarmId, "advisor")
      ...
      await expect(cancelInvitationForFarm(fdm, otherPrincipalId, invId)).rejects.toThrowError(
        "Principal does not have permission to perform this action",
      )
    })
  })
  ...
  describe("updateRoleOfInvitationForFarm", () => {
    ...
    beforeEach(async () => {
      ...
      await grantRoleToFarm(fdm, principal_id, target_username, updateInvFarmId, "advisor")
      const invitations = await listPendingInvitationsForFarm(fdm, principal_id, updateInvFarmId)
      updateInvitationId = invitations[0].invitation_id
    })

    it("should update the role on a pending invitation", async () => {
      await updateRoleOfInvitationForFarm(fdm, principal_id, updateInvitationId, "researcher")

      // Verify via listPrincipalsForFarm: the pending entry should show the new role
      const principals = await listPrincipalsForFarm(fdm, principal_id, updateInvFarmId)
      const pending = principals.find(
        (p) => p.status === "pending" && p.invitation_id === updateInvitationId,
      )
      expect(pending).toBeDefined()
    ...
    it("should throw if the invitation does not exist", async () => {
      await expect(
        updateRoleOfInvitationForFarm(fdm, principal_id, createId(), "owner"),
      ).rejects.toThrowError("Exception for updateRoleOfInvitationForFarm")
    })
    ...
    it("should throw if the invitation is not pending", async () => {
      ...
      await expect(
        updateRoleOfInvitationForFarm(fdm, principal_id, updateInvitationId, "owner"),
      ).rejects.toThrowError("Exception for updateRoleOfInvitationForFarm")
    })
    ...
    it("should throw if the principal does not have share permission", async () => {
      ...
      await grantRoleToFarm(fdm, principal_id, target_username, anotherUpdateFarmId, "advisor")
      const invitations = await listPendingInvitationsForFarm(
        fdm,
        principal_id,
        anotherUpdateFarmId,
      ...
      const otherPrincipalId = createId()
      await expect(
        updateRoleOfInvitationForFarm(fdm, otherPrincipalId, invId, "researcher"),
      ).rejects.toThrowError("Principal does not have permission to perform this action")
    })
  })
  ...
  describe("listPendingInvitationsForUser", () => {
    ...
    beforeAll(async () => {
      ...
      await grantRoleToFarm(fdm, principal_id, "listuser@example.com", listUserFarmId, "researcher")
    })

    it("should return pending invitations for a user by email", async () => {
      const invitations = await listPendingInvitationsForUser(fdm, listUserTargetId)
      expect(invitations.length).toBeGreaterThanOrEqual(1)
    ...
    it("should return pending invitations for a user by principal_id", async () => {
      ...
      const invitations = await listPendingInvitationsForUser(fdm, target_id)
    ...
    it("should return empty array if user does not exist or has no invitations", async () => {
      ...
      const invitations = await listPendingInvitationsForUser(fdm, nonExistentUserId)

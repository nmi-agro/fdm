...
describe("autoAcceptInvitationsForNewUser", () => {
  ...
  it("should auto-accept a pending email-based invitation when user verifies email", async () => {
    ...
    await grantRoleToFarm(fdm, ownerPrincipalId, targetEmail, farmId, "advisor")
  ...
  it("should skip expired invitations", async () => {
    ...
    // Create email-based invitation
    await grantRoleToFarm(fdm, ownerPrincipalId, expiredEmail, anotherFarmId, "researcher")
    ...
    const principals = await listPrincipalsForResource(fdm, "farm", anotherFarmId)
    const grantee = principals.find((p) => p.principal_id === expiredUser.user.id)
  ...
  it("should handle email case-insensitively", async () => {
    ...
    await grantRoleToFarm(fdm, ownerPrincipalId, mixedCaseEmail, yetAnotherFarmId, "advisor")
    ...
    const principals = await listPrincipalsForResource(fdm, "farm", yetAnotherFarmId)
...
describe("acceptInvitation", () => {
  ...
  it("should throw if invitation does not exist", async () => {
    ...
    await expect(acceptInvitation(fdm, createId(), user.user.id)).rejects.toThrowError(
  ...
  it("should throw if invitation is expired", async () => {
    ...
    await expect(acceptInvitation(fdm, invitation_id, target.user.id)).rejects.toThrowError(
      "Exception for acceptInvitation",
    )
  })
})
...
describe("declineInvitation", () => {
  ...
  it("should throw if invitation does not exist", async () => {
    ...
    await expect(declineInvitation(fdm, createId(), user.user.id)).rejects.toThrowError(
  ...
  it("should throw if invitation is expired", async () => {
    ...
    await expect(declineInvitation(fdm, invitation_id, target.user.id)).rejects.toThrowError(
      "Exception for declineInvitation",
    )
  })
...
describe("createInvitation spam prevention", () => {
  ...
  it("should reject when target already has too many pending invitations", async () => {
    ...
    for (let i = 0; i < MAX_INVITATIONS_PENDING_PER_TARGET; i++) {
      ...
      await grantRoleToFarm(fdm, pendingCapOwnerPrincipalId, targetEmail, farmId, "advisor")
    }
    ...
    await expect(
      grantRoleToFarm(fdm, pendingCapOwnerPrincipalId, targetEmail, overflowFarmId, "advisor"),
    ).rejects.toThrowError("Exception for grantRoleToFarm")
  })
})

...
describe("Principals", () => {
  ...
  describe("identifyPrincipal", () => {
    ...
    it("should identify a principal by organization slug", async () => {
      const principalDetails = await identifyPrincipal(fdm, organizationSlug)
      expect(principalDetails).toBeDefined()
    ...
    it("should return undefined if no principal is found", async () => {
      ...
      const principalDetails = await identifyPrincipal(fdm, nonExistentIdentifier)
    ...
    it("should handle database errors and throw an error", async () => {
      ...
      // Act & Assert
      await expect(identifyPrincipal(mockFdm, userName)).rejects.toThrowError(
        "Exception for identifyPrincipal",
      )
    })
    ...
    it("should prioritize username over organization slug if both exist", async () => {
      ...
      const principalDetails = await identifyPrincipal(fdm, conflictingSlug)
      ...
      await fdm
        ...
        .where(eq(authNSchema.organization.id, conflictingOrganization_id))
        .execute()
    })
  })
  ...
  describe("lookupPrincipal", () => {
    ...
    it("should find organizations by partial name", async () => {
      ...
      expect(results).toBeDefined()
      expect(
        results.filter((x) => x.displayUserName?.match(new RegExp(identifier, "i"))).length,
    ...
    it("should find organizations by partial slug", async () => {
      ...
      expect(
        results.filter((x) => x.username?.match(new RegExp(identifier, "i"))).length,
      ).toBeGreaterThanOrEqual(1)
      expect(results.some((r) => r.type === "organization")).toBe(true)
    })
    ...
    it("should prioritize email match of user over slug match of organization", async () => {
      ...
      await fdm
        .delete(authNSchema.organization)
        .where(eq(authNSchema.organization.id, conflicting_organization_id))
        .execute()
    })

...
describe("Harvest Data Model", () => {
  ...
  it("should add a new harvest to a cultivation", async () => {
    ...
    const newHarvestId = await addHarvest(fdm, principal_id, b_lu, harvestDate, {
    ...
    const newHarvest = harvests.find((h) => h.b_id_harvesting === newHarvestId)

    expect(newHarvest).toBeDefined()
    expect(newHarvest?.b_lu_harvest_date).toEqual(harvestDate)
    expect(newHarvest?.harvestable.harvestable_analyses[0].b_lu_yield_fresh).toEqual(yieldValue)
  })

  it("should retrieve a harvest by its ID", async () => {
    b_id_harvesting = await addHarvest(fdm, principal_id, b_lu, new Date("2024-07-01"), {
  ...
  it("should update an existing harvest", async () => {
    ...
    await updateHarvest(fdm, principal_id, b_id_harvesting, newHarvestDate, {
      b_lu_yield_fresh: newYield,
      b_lu_moist: newMoist,
      b_lu_cp: newCP,
    })

    const updatedHarvest = await getHarvest(fdm, principal_id, b_id_harvesting)
  ...
  it("should throw an error when updating a harvest without permission", async () => {
    ...
    await expect(
      updateHarvest(fdm, other_principal_id, b_id_harvesting, new Date(), {
        b_lu_yield: 5000,
        b_lu_n_harvestable: 1.1,
      }),
    ).rejects.toThrowError("Principal does not have permission to perform this action")
  ...
  it("should throw an error when updating with an invalid harvest date", async () => {
    ...
    await expect(
      updateHarvest(fdm, principal_id, b_id_harvesting, invalidHarvestDate, {
        b_lu_yield: 5000,
        b_lu_n_harvestable: 1.1,
      }),
  ...
  it("should not update cultivation end date for 'multiple' harvestable cultivations", async () => {
    ...
    await updateHarvest(fdm, principal_id, b_id_harvesting_multiple, newHarvestDate, {
      b_lu_yield_bruto: 10000,
      b_lu_tarra: 5,
      b_lu_uww: 400,
      b_lu_n_harvestable: 20,
  ...
  it("should throw an error when updating harvest date after terminating date for 'multiple' harvestable cultivations", async () => {
    ...
    await expect(
      updateHarvest(fdm, principal_id, b_id_harvesting_multiple, newHarvestDate, {}),
    ).rejects.toThrowError("Exception for updateHarvest")
  })
})
...
describe("getParametersForHarvestCat", () => {
  ...
  it('should return correct parameters for "HC040"', () => {
    ...
    expect(params).toEqual(["b_lu_yield_bruto", "b_lu_tarra", "b_lu_dm", "b_lu_n_harvestable"])
  })
  ...
  it('should return correct parameters for "HC042"', () => {
    ...
    expect(params).toEqual(["b_lu_yield_bruto", "b_lu_tarra", "b_lu_uww", "b_lu_n_harvestable"])
  })
...
describe("getDefaultsForHarvestParameters", () => {
  ...
  it("should throw an error if cultivation catalogue item is not found", () => {
    expect(() =>
      getDefaultsForHarvestParameters("non-existent", mockCultivationsCatalogue),
  ...
  describe("HC010 - Standard", () => {
    it("should return correct default parameters for HC010", () => {
      const defaults = getDefaultsForHarvestParameters("HC010-catalogue", mockCultivationsCatalogue)
    ...
    it("should convert HC010 defaults back to original standardized values", () => {
      ...
      expect(converted.b_lu_n_harvestable).toBe(originalCatalogueItem.b_lu_n_harvestable)
    })
  })

  describe("HC020 - Grassland", () => {
    it("should return correct default parameters for HC020", () => {
      const defaults = getDefaultsForHarvestParameters("HC020-catalogue", mockCultivationsCatalogue)
      expect(defaults).toEqual({
        b_lu_yield: 2000,
        b_lu_cp: 125, // 20 * 6.25
      })
  ...
  describe("HC031 - Maize", () => {
    it("should return correct default parameters for HC031", () => {
      const defaults = getDefaultsForHarvestParameters("HC031-catalogue", mockCultivationsCatalogue)
      expect(defaults).toEqual({
        b_lu_yield: 4900,
  ...
  describe("HC040 - Root crops", () => {
    it("should return correct default parameters for HC040", () => {
      const defaults = getDefaultsForHarvestParameters("HC040-catalogue", mockCultivationsCatalogue)
      expect(defaults).toEqual({
  ...
  describe("HC041 - Sugar beet", () => {
    it("should return correct default parameters for HC041", () => {
      const defaults = getDefaultsForHarvestParameters("HC041-catalogue", mockCultivationsCatalogue)
      expect(defaults).toEqual({
  ...
  describe("HC042 - Potatoes", () => {
    it("should return correct default parameters for HC042", () => {
      const defaults = getDefaultsForHarvestParameters("HC042-catalogue", mockCultivationsCatalogue)
      expect(defaults).toEqual({
  ...
  describe("HC050 - Cereals", () => {
    it("should return correct default parameters for HC050", () => {
      const defaults = getDefaultsForHarvestParameters("HC050-catalogue", mockCultivationsCatalogue)
      expect(defaults).toEqual({
  ...
  it("should return empty object for an unrecognized harvest category (null)", () => {
    const defaults = getDefaultsForHarvestParameters("UNKNOWN-catalogue", mockCultivationsCatalogue)
    expect(defaults).toEqual({})
  })
})
...
describe("getHarvestsForFarm", () => {
  ...
  beforeEach(async () => {
    ...
    b_lu = await addCultivation(fdm, principal_id, b_lu_catalogue, b_id, new Date("2024-03-01"))

    b_lu_2 = await addCultivation(fdm, principal_id, b_lu_catalogue, b_id_2, new Date("2024-04-01"))
  })
  ...
  it("should include nested harvestable and harvestable_analyses", async () => {
    ...
    expect(harvests[0].harvestable.harvestable_analyses[0].b_lu_yield_fresh).toBe(5000)
  })
  ...
  it("should only return harvests within the given timeframe", async () => {
    ...
    const result = await getHarvestsForFarm(fdm, principal_id, b_id_farm, timeframe)
  ...
  it("should throw when principal does not have permission", async () => {
    ...
    await expect(getHarvestsForFarm(fdm, unauthorized_principal, b_id_farm)).rejects.toThrowError(

...
describe("Cultivation Data Model", () => {
  ...
  beforeEach(async () => {
    ...
    await enableCultivationCatalogue(fdm, principal_id, b_id_farm_2, b_lu_source_2)
  })
  ...
  describe("Cultivation CRUD", () => {
    beforeEach(async () => {
      ...
      b_lu = await addCultivation(fdm, principal_id, b_lu_catalogue, b_id, b_lu_start)
      await addCultivation(fdm, principal_id, b_lu_catalogue_2, b_id_2, b_lu_start)
    })

    it("should get cultivations from catalogue", async () => {
      const cultivations = await getCultivationsFromCatalogue(fdm, principal_id, b_id_farm)
      expect(
        cultivations.find((cultivation) => cultivation.b_lu_catalogue === b_lu_catalogue),
      ).toBeDefined()
    })

    it("should get all cultivations for farms from catalogue using composable functions", async () => {
      const farmCatalogues = await getEnabledCultivationCataloguesForFarms(fdm, principal_id, [
        b_id_farm,
        b_id_farm_2,
      ])
      ...
      const allSources = [
        ...new Set([...farmCatalogues[b_id_farm], ...farmCatalogues[b_id_farm_2]]),
      ]
      const cultivations = await getCultivationsFromCatalogues(fdm, allSources)
      const farmSources1 = new Set(farmCatalogues[b_id_farm])
      expect(
        cultivations.find(
          (c) => c.b_lu_catalogue === b_lu_catalogue && farmSources1.has(c.b_lu_source),
        ),
      ).toBeDefined()
      const farmSources2 = new Set(farmCatalogues[b_id_farm_2])
      expect(
        cultivations.find(
          (c) => c.b_lu_catalogue === b_lu_catalogue_2 && farmSources2.has(c.b_lu_source),
        ),
      ).toBeDefined()
    })
    ...
    it("should handle empty catalogues", async () => {
      ...
      await enableCultivationCatalogue(fdm, principal_id, b_id_farm, "invalid-catalogue")
      expect(await getCultivationsFromCatalogue(fdm, principal_id, b_id_farm)).toEqual([])
    })
    ...
    it("(getCultivationsFromCatalogue) should wrap errors with the correct message", async () => {
      ...
      try {
        ...
        expect((e as Error).message).toBe("Exception for getCultivationsFromCatalogue")
      }
    })
    ...
    it("(getCultivationsFromCatalogues) should wrap errors with the correct message", async () => {
      ...
      try {
        ...
        expect((err as Error).message).toBe("Exception for getCultivationsFromCatalogues")
      }
    })
    ...
    it("should throw an error when adding a cultivation with an invalid catalogue ID", async () => {
      ...
      await expect(
        addCultivation(fdm, principal_id, invalid_b_lu_catalogue, b_id, b_lu_start),
    ...
    it("should add a new cultivation", async () => {
      ...
      const new_b_lu = await addCultivation(fdm, principal_id, b_lu_catalogue, b_id, b_lu_start)
      expect(b_lu).toBeDefined()

      const cultivation = await getCultivation(fdm, principal_id, new_b_lu)
    ...
    it("should handle duplicate cultivation gracefully", async () => {
      ...
      await expect(
        addCultivation(fdm, principal_id, b_lu_catalogue, b_id, b_lu_start),
      ).rejects.toThrow("Exception for addCultivation")
    })
    ...
    it("should throw an error when adding a cultivation with an invalid field ID", async () => {
      ...
      await expect(
        addCultivation(fdm, principal_id, b_lu_catalogue, invalid_b_id, b_lu_start),
      ).rejects.toThrow("Principal does not have permission to perform this action")
    })

    it("should get cultivations by field ID", async () => {
      await addCultivation(fdm, principal_id, b_lu_catalogue, b_id, new Date("2024-03-01"))
    ...
    it("should get cultivations by field ID within a timeframe", async () => {
      ...
      await addCultivation(fdm, principal_id, b_lu_catalogue, b_id, new Date("2024-07-01"))
      await addCultivation(
        fdm,
        principal_id,
        b_lu_catalogue,
      ...
      const cultivations2 = await getCultivations(fdm, principal_id, b_id, {
      ...
      const cultivations3 = await getCultivations(fdm, principal_id, b_id, {
    ...
    it("should remove a cultivation", async () => {
      ...
      await expect(getCultivation(fdm, principal_id, b_lu)).rejects.toThrowError(
    ...
    it("should update an existing cultivation", async () => {
      ...
      await updateCultivation(fdm, principal_id, b_lu, newCatalogueId, newSowingDate)

      const updatedCultivation = await getCultivation(fdm, principal_id, b_lu)
    ...
    it("should throw an error when updating a non-existent cultivation", async () => {
      ...
      await expect(
        updateCultivation(fdm, principal_id, nonExistentBlu, b_lu_catalogue, new Date()),
    ...
    it("should throw an error when updating with invalid catalogue id", async () => {
      ...
      await expect(
        updateCultivation(fdm, principal_id, b_lu, nonExistentCatalogueId, new Date()),
    ...
    it("should update a cultivation with only the sowing date", async () => {
      ...
      await updateCultivation(fdm, principal_id, b_lu, undefined, newSowingDate)
    ...
    it("should delete harvests when the terminating date is null if the crop is harvestable only once", async () => {
      ...
      await updateCultivation(fdm, principal_id, b_lu, undefined, undefined, null, undefined)
    ...
    it("should clear the terminating date and keep the harvests when b_lu_end is null and the cultivation can be harvested multiple times", async () => {
      ...
      const b_lu = await addCultivation(fdm, principal_id, b_lu_catalogue, b_id, b_lu_start)
    ...
    it("should update a cultivation with only the crop residue", async () => {
      await updateCultivation(fdm, principal_id, b_lu, undefined, undefined, undefined, true)
    ...
    it("should throw an error when updating with invalid sowing date - before termination date", async () => {
      ...
      await expect(
        updateCultivation(fdm, principal_id, b_lu, undefined, newSowingDate, newTerminationDate),
      ).rejects.toThrowError("Exception for updateCultivation")
    })
    ...
    it("should clear an existing variety from a cultivation", async () => {
      ...
      const cultivationWithVariety = await getCultivation(fdm, principal_id, b_lu)
    ...
    it("should add a new cultivation to the catalogue with variety options", async () => {
      ...
      const cultivation = cultivations.find((c) => c.b_lu_catalogue === b_lu_catalogue)
      expect(cultivation).toBeDefined()
      expect(cultivation?.b_lu_variety_options).toEqual(b_lu_variety_options)
    })
    ...
    describe("getDefaultDatesOfCultivation", () => {
      ...
      it("should throw an error if cultivation is not found", async () => {
        ...
        await expect(
          getDefaultDatesOfCultivation(fdm, principal_id, b_id_farm, nonExistentCatalogueId, year),
  ...
  describe("Cultivation Plan", () => {
    ...
    beforeEach(async () => {
      ...
      const p_id_catalogue = await addFertilizerToCatalogue(fdm, principal_id, b_id_farm, {
    ...
    it("should get cultivation plan for a farm", async () => {
      ...
      const cultivationPlan = await getCultivationPlan(fdm, principal_id, b_id_farm)
    ...
    it("should return permission denied if farm does not exist", async () => {
      await expect(getCultivationPlan(fdm, principal_id, createId())).rejects.toThrowError(
        "Principal does not have permission to perform this action",
      )
    })
    ...
    it("should get cultivation plan for a farm with multiple cultivations and fields", async () => {
      ...
      const cornCultivation = cultivationPlan.find((c) => c.b_lu_catalogue === b_lu_catalogue2)
      ...
      const field1InWheatPlan = wheatCultivation?.fields.find((f) => f.b_id === b_id)
      expect(field1InWheatPlan?.fertilizer_applications.length).toEqual(1) // Fertilizer for wheat in field 1

      const field1InCornPlan = cornCultivation?.fields.find((f) => f.b_id === b_id)
      ...
      const field2InWheatPlan = wheatCultivation?.fields.find((f) => f.b_id === b_id2)
    ...
    it("should get cultivation plan for a farm within a timeframe", async () => {
      ...
      const cultivationPlan = await getCultivationPlan(fdm, principal_id, b_id_farm, timeframe)
...
describe("getCultivationsFromCatalogue error handling", () => {
  ...
  it("should handle database errors", async () => {
    ...
    try {
      ...
      expect(e.message).toContain("Exception for getCultivationsFromCatalogue")
...
describe("buildCultivationTimeframeCondition", () => {
  ...
  beforeEach(async () => {
    ...
    principal_id = createId()
    b_id_farm = await addFarm(fdm, principal_id, "Test Farm", "123456", "123 Farm Lane", "12345")
    b_id = await addField(
      fdm,
      principal_id,
      b_id_farm,
...
describe("getCultivationsForFarm", () => {
  ...
  it("should return a Map with cultivations grouped by field ID", async () => {
    await addCultivation(fdm, principal_id, b_lu_catalogue, b_id, new Date("2023-03-01"))
    await addCultivation(fdm, principal_id, b_lu_catalogue, b_id_2, new Date("2023-04-01"))

    const result = await getCultivationsForFarm(fdm, principal_id, b_id_farm)
  ...
  it("should only return cultivations within the given timeframe", async () => {
    ...
    const result = await getCultivationsForFarm(fdm, principal_id, b_id_farm, timeframe)
  ...
  it("should not include cultivations from other farms", async () => {
    ...
    await enableCultivationCatalogue(fdm, principal_id, other_farm, b_lu_source)
    await addCultivation(fdm, principal_id, b_lu_catalogue, other_b_id, new Date("2023-03-01"))
    await addCultivation(fdm, principal_id, b_lu_catalogue, b_id, new Date("2023-04-01"))

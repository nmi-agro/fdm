...
export async function getFertilizersFromCatalogue(
  ...
  try {
    const catalogueIds = await getEnabledFertilizerCatalogues(fdm, principal_id, b_id_farm)
...
export async function getFertilizersFromCatalogues(
  ...
  try {
    ...
    const authorizedRows = await fdm
      ...
      .innerJoin(
        ...
        and(
          ...
          eq(authZSchema.role.resource_id, schema.fertilizerCatalogueEnabling.b_id_farm),
      ...
      .where(
        and(
          inArray(schema.fertilizerCatalogueEnabling.p_source, catalogueIds),
          or(
    ...
    const authorizedSources = new Set(authorizedRows.map((r: { p_source: string }) => r.p_source))
    const filteredCatalogueIds = catalogueIds.filter((id) => authorizedSources.has(id))
    if (filteredCatalogueIds.length === 0) {
      return []
    }

    const fertilizersCatalogue: schema.fertilizersCatalogueTypeSelect[] = await fdm
      .select()
      .from(schema.fertilizersCatalogue)
      .where(inArray(schema.fertilizersCatalogue.p_source, filteredCatalogueIds))
      .orderBy(
    ...
    return fertilizersCatalogue.map((result) => ({
      ...result,
      p_app_method_options: result.p_app_method_options as ApplicationMethods[] | null,
...
export async function addFertilizerToCatalogue(
  ...
  try {
    await checkPermission(fdm, "farm", "write", b_id_farm, principal_id, "addFertilizerToCatalogue")
    ...
    input.hash = await hashFertilizer(input as unknown as CatalogueFertilizerItem)
...
export async function addFertilizer(
  ...
  try {
    await checkPermission(fdm, "farm", "write", b_id_farm, principal_id, "addFertilizer")
    ...
    return await fdm.transaction(async (tx) => {
      ...
      await tx.insert(schema.fertilizerAcquiring).values(fertilizerAcquiringData)

      await tx.insert(schema.fertilizerPicking).values(fertilizerPickingData)
  ...
  try {
    ...
    const fertilizer = await fdm
      .select({
        ...
        p_app_amount_unit: schema.fertilizersCatalogue.p_app_amount_unit,
      ...
      .leftJoin(
        ...
        eq(schema.fertilizerPicking.p_id_catalogue, schema.fertilizersCatalogue.p_id_catalogue),
      )
    ...
    if (result.b_id_farm) {
      await checkPermission(fdm, "farm", "read", result.b_id_farm, principal_id, "getFertilizer")
    }

    const { b_id_farm: _b_id_farm, ...rest } = result
    return {
      ...rest,
      p_type: deriveFertilizerType(rest as Partial<schema.fertilizersCatalogueTypeSelect>),
...
export async function updateFertilizerFromCatalogue(
  ...
  try {
    ...
    const updatedProperties: schema.fertilizersCatalogueTypeInsert = {
      ...
      p_type_manure:
        p_type !== undefined ? p_type === "manure" : existingFertilizer[0].p_type_manure,
      p_type_mineral:
        p_type !== undefined ? p_type === "mineral" : existingFertilizer[0].p_type_mineral,
      p_type_compost:
        p_type !== undefined ? p_type === "compost" : existingFertilizer[0].p_type_compost,
    }
...
export async function getFertilizers(
  ...
  try {
    await checkPermission(fdm, "farm", "read", b_id_farm, principal_id, "getFertilizers")
...
export async function removeFertilizer(
  ...
  try {
    return await fdm.transaction(async (tx) => {
      ...
      await checkPermission(tx, "farm", "write", b_id_farm, principal_id, "removeFertilizer")

      await tx.delete(schema.fertilizerAcquiring).where(eq(schema.fertilizerAcquiring.p_id, p_id))

      await tx.delete(schema.fertilizerPicking).where(eq(schema.fertilizerPicking.p_id, p_id))

      await tx.delete(schema.fertilizers).where(eq(schema.fertilizers.p_id, p_id))
    })
  } catch (err) {
...
export async function addFertilizerApplication(
  ...
  try {
    await checkPermission(fdm, "field", "write", b_id, principal_id, "addFertilizerApplication")
  ...
  try {
    ...
    const p_app_amount =
      p_app_amount_display !== null && p_app_amount_display !== undefined
        ? toKgPerHa(p_app_amount_display, fertilizer.p_app_amount_unit, fertilizer.p_density)
        : undefined
    await fdm
...
export async function getFertilizerApplication(
  ...
  try {
    ...
    const result = (await fdm
      ...
      .leftJoin(
        schema.fertilizerPicking,
        eq(schema.fertilizerPicking.p_id, schema.fertilizerApplication.p_id),
      )
      .leftJoin(
        schema.fertilizersCatalogue,
        eq(schema.fertilizersCatalogue.p_id_catalogue, schema.fertilizerPicking.p_id_catalogue),
      )
      .where(eq(schema.fertilizerApplication.p_app_id, p_app_id))) as (BaseFertilizerApplication & {
      p_app_amount_unit: AppAmountUnit
      p_density: number | null
    })[]

    return result.length > 0
      ? extendFertilizerApplication(result[0], result[0].p_app_amount_unit, result[0].p_density)
      : null
  } catch (err) {
...
export async function getFertilizerApplications(
  ...
  try {
    await checkPermission(fdm, "field", "read", b_id, principal_id, "getFertilizerApplications")
      ...
      .where(
        timeframe
          ? and(
              ...
              timeframe.start
                ? gte(schema.fertilizerApplication.p_app_date, timeframe.start)
                : undefined,
              timeframe.end
                ? lte(schema.fertilizerApplication.p_app_date, timeframe.end)
                : undefined,
            )
          : eq(schema.fertilizerApplication.b_id, b_id),
      )
      .orderBy(desc(schema.fertilizerApplication.p_app_date))) as (BaseFertilizerApplication & {
      p_app_amount_unit: AppAmountUnit
      p_density: number | null
    })[]

    return results.map((result) =>
      extendFertilizerApplication(result, result.p_app_amount_unit, result.p_density),
    )
  } catch (err) {
  ...
  try {
    ...
    const rows = (await fdm
      ...
      .innerJoin(
        ...
        eq(schema.fieldAcquiring.b_id, schema.fertilizerApplication.b_id),
      )
      .where(
        timeframe
          ? and(
    ...
    for (const row of rows) {
      ...
      if (existing) {
        ...
        result.set(row.b_id, [fertilizerApplication as FertilizerApplication])
      }
    }
    return result
  } catch (err) {
...
export function getFertilizerParametersDescription(
  ...
  const fertilizerParameterDescription: FertilizerParameterDescription = [
    ...
    },
    {
      ...
      type: "enum",
      category: "general",
      description: "Eenheid voor het weergeven van de hoeveelheid van deze meststof",
 ...
 * @internal
 */
function deriveFertilizerType(fertilizer: Partial<schema.fertilizersCatalogueTypeSelect>) {

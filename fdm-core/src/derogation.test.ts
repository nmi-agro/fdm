...
describe("Derogation Functions", () => {
  ...
  describe("addDerogation", () => {
    ...
    it("should throw an error if the derogation year is before 2006", async () => {
      const year = 2005
      await expect(addDerogation(fdm, principal_id, b_id_farm, year)).rejects.toThrowError(
    ...
    it("should throw an error if the principal does not have write access", async () => {
      ...
      const year = 2024
      await expect(addDerogation(fdm, other_principal_id, b_id_farm, year)).rejects.toThrowError(
  ...
  describe("listDerogations", () => {
    ...
    it("should throw an error if the principal does not have read access", async () => {
      const other_principal_id = createId()
      await expect(listDerogations(fdm, other_principal_id, b_id_farm)).rejects.toThrowError(
        "Principal does not have permission to perform this action",
      )
    })
  })
  ...
  describe("removeDerogation", () => {
    ...
    it("should throw an error if the principal does not have write access", async () => {
      ...
      const b_id_derogation = await addDerogation(fdm, principal_id, b_id_farm, year)
      const other_principal_id = createId()
      await expect(removeDerogation(fdm, other_principal_id, b_id_derogation)).rejects.toThrowError(
        "Principal does not have permission to perform this action",
      )
    })

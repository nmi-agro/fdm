...
export async function getCultivationsFromCatalogue(
  ...
  try {
    const catalogueIds = await getEnabledCultivationCatalogues(fdm, principal_id, b_id_farm)
...
export async function getCultivationsFromCatalogues(
  ...
  try {
    ...
    return fdm
      ...
      .where(inArray(schema.cultivationsCatalogue.b_lu_source, catalogueIds))
      .orderBy(
...
export async function addCultivationToCatalogue(
  ...
  try {
    return await fdm.transaction(async (tx) => {
      ...
      if (properties.b_lu_start_default && !dateRegex.test(properties.b_lu_start_default)) {
        throw new Error("Invalid b_lu_start_default format. Expected MM-dd.")
      }
      if (properties.b_date_harvest_default && !dateRegex.test(properties.b_date_harvest_default)) {
        throw new Error("Invalid b_date_harvest_default format. Expected MM-dd.")
      }
  ...
  try {
    ...
    const cultivationsCatalogue = await fdm
      .select({
        ...
        b_lu_start_default: schema.cultivationsCatalogue.b_lu_start_default,
        b_date_harvest_default: schema.cultivationsCatalogue.b_date_harvest_default,
      })
      .from(schema.cultivationsCatalogue)
      .where(
        and(
          inArray(
            ...
            enabledCatalogues.map((c: { b_lu_source: string }) => c.b_lu_source),
          ),
    ...
    const defaultStart = cultivationsCatalogue[0].b_lu_start_default ?? "03-15"
    const defaultEnd = cultivationsCatalogue[0].b_date_harvest_default ?? "09-15"
    ...
    if (
      ...
      cultivationDefaultDates.b_lu_end.getTime() <= cultivationDefaultDates.b_lu_start.getTime()
    ) {
      cultivationDefaultDates.b_lu_start = new Date(`${year - 1}-${defaultStart}`)
    }
    ...
    return await fdm.transaction(async (tx) => {
        ...
        .where(
          and(
            ...
            or(
              ...
              and(
                eq(schema.cultivationStarting.b_lu_start, b_lu_start),
      ...
      if (b_lu_variety) {
        const catalogueEntry = await tx
          .select({
            b_lu_variety_options: schema.cultivationsCatalogue.b_lu_variety_options,
          })
          .from(schema.cultivationsCatalogue)
          .where(eq(schema.cultivationsCatalogue.b_lu_catalogue, b_lu_catalogue))
          .limit(1)
        ...
        if (
          ...
          !catalogueEntry[0].b_lu_variety_options.includes(b_lu_variety)
        ) {
          throw new Error("Variety not available for this cultivation")
        }
      }
      ...
      if (b_lu_end) {
        const harvestableType = await getHarvestableTypeOfCultivation(tx, b_lu)
        ...
        if (harvestableType === "once") {
          ...
          await addHarvest(tx, principal_id, b_lu, b_lu_end, defaultHarvestParameters)
        }
      }
      return b_lu
    })
  ...
  try {
    ...
    const cultivation = await fdm
      .select({
        ...
        b_lu_rest_oravib: schema.cultivationsCatalogue.b_lu_rest_oravib,
...
export async function getCultivations(
  ...
  try {
    ...
    const cultivations = await fdm
      ...
      .where(
        timeframeCondition
          ? and(eq(schema.cultivationStarting.b_id, b_id), timeframeCondition)
          : eq(schema.cultivationStarting.b_id, b_id),
      )
      .orderBy(
...
export async function getCultivationsForFarm(
  ...
  try {
    await checkPermission(fdm, "farm", "read", b_id_farm, principal_id, "getCultivationsForFarm")
...
export async function getCultivationPlan(
  ...
  try {
    ...
    const cultivations = await fdm
      ...
      .from(schema.farms)
      .leftJoin(schema.fieldAcquiring, eq(schema.farms.b_id_farm, schema.fieldAcquiring.b_id_farm))
      .leftJoin(schema.fields, eq(schema.fieldAcquiring.b_id, schema.fields.b_id))
      .leftJoin(schema.cultivationStarting, eq(schema.fields.b_id, schema.cultivationStarting.b_id))
      .leftJoin(
        schema.cultivationEnding,
        eq(schema.cultivationEnding.b_lu, schema.cultivationStarting.b_lu),
      )
      .leftJoin(schema.cultivations, eq(schema.cultivationStarting.b_lu, schema.cultivations.b_lu))
      .leftJoin(
        schema.cultivationsCatalogue,
        eq(schema.cultivations.b_lu_catalogue, schema.cultivationsCatalogue.b_lu_catalogue),
      )
      ...
      .leftJoin(
        schema.harvestables,
        eq(schema.cultivationHarvesting.b_id_harvestable, schema.harvestables.b_id_harvestable),
      )
      .leftJoin(
      ...
      (acc: CultivationPlan[], curr: (typeof cultivations)[0]) => {
        let existingCultivation = acc.find(
          (item) =>
            ...
            (item.b_lu_start?.getTime() ?? 0) === (curr.b_lu_start?.getTime() ?? 0) &&
            (item.b_lu_end?.getTime() ?? 0) === (curr.b_lu_end?.getTime() ?? 0),
        )
...
export async function removeCultivation(
  ...
  try {
    ...
    return await fdm.transaction(async (tx) => {
      ...
      await tx.delete(schema.cultivationEnding).where(eq(schema.cultivationEnding.b_lu, b_lu))

      await tx.delete(schema.cultivationStarting).where(eq(schema.cultivationStarting.b_lu, b_lu))

      await tx.delete(schema.cultivations).where(eq(schema.cultivations.b_lu, b_lu))
    })
  } catch (err) {
...
export async function updateCultivation(
  ...
  try {
    ...
    if (b_lu_start && b_lu_end && b_lu_end.getTime() <= b_lu_start.getTime()) {
      throw new Error("Terminate date must be after sowing date")
    }
    return await fdm.transaction(async (tx) => {
      ...
      if (b_lu_variety !== undefined) {
        if (b_lu_variety) {
          ...
          const catalogueIdToValidate = b_lu_catalogue ?? existingCultivation[0].b_lu_catalogue
      ...
      if (b_lu_start) {
        ...
        if (b_lu_end === undefined) {
          ...
          if (result.length > 0 && result[0].b_lu_end) {
            if (b_lu_start.getTime() >= result[0].b_lu_end.getTime()) {
              throw new Error("Sowing date must be before termination date")
            }
          }
        }
      ...
      if (b_lu_end) {
        ...
        if (harvestableType === "once") {
          ...
          const harvests = await getHarvests(tx, principal_id, b_lu)
          await Promise.all(
            harvests.map((harvest) => removeHarvest(tx, principal_id, harvest.b_id_harvesting)),
          )
        } else {
          await tx
            .update(schema.cultivationEnding)

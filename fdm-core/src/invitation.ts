  ...
  try {
    return await fdm.transaction(async (tx) => {
      ...
      if (targetDetails) {
        ...
        const existingMembers = await listPrincipalsForResource(tx, resource, resource_id)
        const isAlreadyMember = existingMembers.some((m) => m.principal_id === targetPrincipalId)
        if (isAlreadyMember) {
          throw new Error("Target is already a member of this resource")
        }
      } else {
        if (!isEmail(normalizedTarget)) {
          throw new Error("Target not found and not a valid email address")
        }
      ...
      if (rateRow.value >= MAX_INVITATIONS_PER_INVITER_PER_HOUR) {
        throw new Error("Rate limit exceeded: too many invitations sent in the last hour")
      }
      ...
      if (targetEmail) {
        ...
        if (existing.length > 0) {
          await tx
            ...
            .where(eq(authZSchema.invitation.invitation_id, existing[0].invitation_id))
        } else {
          ...
          if (pendingRow.value >= MAX_INVITATIONS_PENDING_PER_TARGET) {
            throw new Error("Target has too many pending invitations. Please try again later.")
          }
        ...
        const existing = await tx
          ...
          .where(
            and(
              eq(authZSchema.invitation.resource, resource),
              eq(authZSchema.invitation.resource_id, resource_id),
              eq(authZSchema.invitation.target_principal_id, targetPrincipalId!),
              eq(authZSchema.invitation.status, "pending"),
            ),
          )
          .limit(1)
    ...
    return await fdm.transaction(async (tx) => {
      ...
      if (invitation.target_principal_id) {
        const orgMembership = await tx
          ...
          .where(
            and(
              eq(authNSchema.member.organizationId, invitation.target_principal_id),
        ...
        if (orgMembership.length > 0) {
          ...
          if (userRecord.length === 0 || !userRecord[0].emailVerified) {
            throw new Error("Email must be verified before accepting an invitation")
          }
        }
        ...
        if (userRecord[0].email.toLowerCase().trim() !== invitation.target_email) {
  ...
  try {
    return await fdm.transaction(async (tx) => {
      ...
      if (invitation.target_principal_id) {
        ...
        if (
          userRecord.length === 0 ||
          userRecord[0].email.toLowerCase().trim() !== invitation.target_email
        ) {
          throw new Error("This invitation is not for your email address")
        }
        if (!userRecord[0].emailVerified) {
          throw new Error("Email must be verified before declining an invitation")
        }
      }
  ...
  try {
    return await fdm.transaction(async (tx) => {
      ...
      if (orgIds.length > 0) {
        conditions.push(
          and(
            inArray(authZSchema.invitation.target_principal_id, orgIds),
            eq(authZSchema.invitation.status, "pending"),
            gt(authZSchema.invitation.expires, now),
          ),
        )
      ...
      for (const inv of pendingInvitations) {
        if (inv.expires < now) {
          await tx
            .update(authZSchema.invitation)
            .set({ status: "expired" })
            .where(eq(authZSchema.invitation.invitation_id, inv.invitation_id))
          continue
        }

...
describe("Organic Certifications", () => {
  ...
  beforeEach(async () => {
    ...
    farmId = await addFarm(fdm, principalId, farmName, farmBusinessId, farmAddress, farmPostalCode)
  })
  ...
  it("should throw an error for invalid TRACES format", async () => {
    ...
    await expect(
      addOrganicCertification(fdm, principalId, farmId, traces, skal, issued, expires),
  ...
  it("should list organic certifications for a farm", async () => {
    ...
    await addOrganicCertification(fdm, principalId, farmId, traces1, skal1, issued1, expires1)
    ...
    await addOrganicCertification(fdm, principalId, farmId, traces2, skal2, issued2, expires2)

    const certifications = await listOrganicCertifications(fdm, principalId, farmId)
  ...
  it("should check if an organic certification is valid for a given date", async () => {
    ...
    await addOrganicCertification(fdm, principalId, farmId, traces, skal, issued, expires)

    const validDate = new Date("2025-06-15T12:00:00Z")
    const isValid = await isOrganicCertificationValid(fdm, principalId, farmId, validDate)
    expect(isValid).toBe(true)

    const expiredDate = new Date("2026-01-01T00:00:00Z")
    const isExpired = await isOrganicCertificationValid(fdm, principalId, farmId, expiredDate)
    expect(isExpired).toBe(false)

    const futureDate = new Date("2023-12-31T23:59:59Z")
    const isFuture = await isOrganicCertificationValid(fdm, principalId, farmId, futureDate)

    ...
    fromDriver(value) {
      ...
      try {
        ...
        if (Array.isArray(type) && !type.includes(data.type as TType)) {
          throw new Error(`Expected geometry type ${type.join(", ")}, got ${data.type}`)
        }
...
export const parseHexToGeometry = (hex: string): GeoJSON.Geometry => {
  try {
    const byteStr = hex.match(/.{1,2}/g)?.map((byte) => Number.parseInt(byte, 16))
    if (!byteStr) {
    ...
    const geometry = parseGeometry(dataView, littleEndian, geometryType, byteOffset)
...
function readPoint(dataView: DataView, littleEndian: boolean, offset: number): GeoJSON.Position {
...
function readMultiPolygon(
  ...
  for (let i = 0; i < numPolygons; i++) {
    ...
    if (polygonType !== GeometryType.Polygon) {
      throw new Error(`Expected nested Polygon, got geometry type ${polygonType}`)
    }
...
export const parseGeometry = (
  ...
  switch (type) {
    ...
    case GeometryType.Polygon:
      return {
        type: "Polygon",
        coordinates: readPolygon(dataView, littleEndian, offset) as GeoJSON.Polygon["coordinates"],
      }

...
export async function addField(
  ...
  try {
    ...
    return await fdm.transaction(async (tx) => {
      ...
      if (
        b_acquiring_method &&
        !schema.acquiringMethodOptions.map((option) => option.value).includes(b_acquiring_method)
      ) {
        console.warn(`Invalid b_acquiring_method: ${b_acquiring_method}. Inserting as 'unknown'.`)
...
export async function getField(
  ...
  try {
    ...
    const field = await fdm
      ...
      .from(schema.fields)
      .innerJoin(schema.fieldAcquiring, eq(schema.fields.b_id, schema.fieldAcquiring.b_id))
      .leftJoin(schema.fieldDiscarding, eq(schema.fields.b_id, schema.fieldDiscarding.b_id))
      .where(eq(schema.fields.b_id, b_id))
      .limit(1)
...
export async function updateField(
  ...
  return await fdm.transaction(async (tx) => {
    try {
      ...
      await tx.update(schema.fields).set(setFields).where(eq(schema.fields.b_id, b_id))
      ...
      if (field.b_end && field.b_start && field.b_start.getTime() >= field.b_end.getTime()) {
        throw new Error("Acquiring date must be before discarding date")
      }
...
export async function removeField(
  ...
  try {
    await checkPermission(fdm, "field", "write", b_id, principal_id, "removeField")
    ...
    await fdm.transaction(async (tx) => {
      ...
      if (cultivations.length > 0) {
        const cultivationIds = cultivations.map(
          (c: { b_lu: schema.cultivationsTypeSelect["b_lu"] }) => c.b_lu,
        )
        ...
        if (harvestings.length > 0) {
          const harvestableIds = harvestings.map(
            (h: { b_id_harvestable: schema.cultivationHarvestingTypeSelect["b_id_harvestable"] }) =>
              h.b_id_harvestable,
          )
          ...
          const harvestableSamplings = await tx
            .select({
              b_id_harvestable_analysis: schema.harvestableSampling.b_id_harvestable_analysis,
            })
          ...
          await tx
            ...
            .where(inArray(schema.harvestables.b_id_harvestable, harvestableIds))
        }

        // Step 8: Delete from cultivation_starting, cultivation_ending and cultivations
        await tx
          .delete(schema.cultivationStarting)
          .where(inArray(schema.cultivationStarting.b_lu, cultivationIds))
        await tx
          .delete(schema.cultivationEnding)
          .where(inArray(schema.cultivationEnding.b_lu, cultivationIds))
        await tx
      ...
      if (soilSamplings.length > 0) {
        const soilAnalysisIds = soilSamplings.map(
          (ss: { a_id: schema.soilAnalysisTypeSelect["a_id"] }) => ss.a_id,
        )
      ...
      await tx.delete(schema.measureAdopting).where(eq(schema.measureAdopting.b_id, b_id))
      if (measuresToDelete.length > 0) {
        const measureIds = measuresToDelete.map(
          (m: { b_id_measure: schema.measuresTypeSelect["b_id_measure"] }) => m.b_id_measure,
        )
        await tx.delete(schema.measures).where(inArray(schema.measures.b_id_measure, measureIds))
      }
...
export function determineIfFieldIsBuffer(
  ...
  const bufferAssumedByShape =
    b_perimeter / Math.sqrt(b_area * 10000) >= BUFFERSTROKEN_CONSTANT && b_area < 2.5

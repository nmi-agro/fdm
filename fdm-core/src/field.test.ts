...
describe("Farm Data Model", () => {
  ...
  describe("Field CRUD", () => {
    ...
    describe("getFields", () => {
      ...
      it("should throw an error when permission check fails", async () => {
        ...
        await expect(getFields(fdm, invalidPrincipalId, b_id_farm)).rejects.toThrowError(
      ...
      it("should get fields within a timeframe", async () => {
        ...
        const fields1 = await getFields(fdm, principal_id, b_id_farm, timeframe1)
        ...
        const fields2 = await getFields(fdm, principal_id, b_id_farm, timeframe2)
        ...
        const fields3 = await getFields(fdm, principal_id, b_id_farm, timeframe3)
        ...
        expect(fields4.map((f) => f.b_name)).toEqual(
          expect.arrayContaining([field2Name, field3Name, field4Name]),
        )
  ...
  describe("removeField", () => {
    ...
    beforeEach(async () => {
      ...
      farmId = await addFarm(fdm, principal_id, "Test Farm", "123", "Address", "12345")

      await enableCultivationCatalogue(fdm, principal_id, farmId, b_lu_source)
    ...
    it("should not remove a field if the principal does not have write permission", async () => {
      ...
      await expect(removeField(fdm, unauthorized_principal_id, fieldId)).rejects.toThrow(
        "Principal does not have permission to perform this action",
      )
    ...
    it("should throw an error when trying to remove a non-existent field", async () => {
      ...
      await expect(removeField(fdm, principal_id, nonExistentFieldId)).rejects.toThrow()
    })
    ...
    it("should test deeper cascading relationships", async () => {
      ...
      const harvestId = await addHarvest(fdm, principal_id, cultivationId, new Date("2025-08-10"), {
  ...
  describe("determineIfFieldIsBuffer", () => {
    ...
    it("should determine if a field is buffer by checking shape and name", async () => {
      const isBuffer = determineIfFieldIsBuffer(1.0, 1000.0, "Bufferstrip")
      expect(isBuffer).toBe(true)
    })

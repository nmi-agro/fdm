...
import { checkHelpdeskPermission, getHelpdeskPermission, getHelpdeskRole } from "./authorization"
...
describe("getHelpdeskRole", () => {
  ...
  test("should return 'user' for an array containing an unknown ID", async ({ fdm }) => {
    const role = await getHelpdeskRole(fdm, [agent_id, createId()])
    expect(role).toBe("user")
  })

  test("should return 'agent' for an array of all agents", async ({ fdm }) => {
  ...
  test("should return 'admin' for an array of all admins", async ({ fdm }) => {
  ...
  test("should return 'agent' for an array mixing agent and admin", async ({ fdm }) => {
    // Not all users are considered admins, but all users are agents
    const role = await getHelpdeskRole(fdm, [agent_id, admin_id])
    expect(role).toBe("agent")
  })

  test("should return 'user' for an array mixing agent and unknown users", async ({ fdm }) => {
    // Not all users are considered admins, but all users are agents
    const role = await getHelpdeskRole(fdm, [agent_id, createId()])
    expect(role).toBe("user")
  })

  test("should return 'user' for an array mixing admin and unknown users", async ({ fdm }) => {
    // Not all users are considered admins, but all users are agents
...
describe("checkHelpdeskPermission", () => {
  ...
  test("should return true when the principal has permission", async ({ fdm }) => {
    const result = await checkHelpdeskPermission(fdm, "helpdesk", "read", "", admin_id, "test")
    expect(result).toBe(true)
  })

  test("should throw when strict=true and permission is denied", async ({ fdm }) => {
    await expect(
      checkHelpdeskPermission(fdm, "helpdesk", "write", "", user_id, "test", true),
...
describe("getHelpdeskPermission — helpdesk resource", () => {
  ...
  test("should let admins manage the helpdesk", async ({ fdm }) => {
    const result = await getHelpdeskPermission(fdm, "helpdesk", "write", "", admin_id)
    expect(result).not.toBeNull()
    expect(result?.granting_resource).toBe("helpdesk")
  })

  test("should let regular agents access info about the helpdesk", async ({ fdm }) => {
    const result = await getHelpdeskPermission(fdm, "helpdesk", "read", "", agent_id)
    expect(result).not.toBeNull()
    expect(result?.granting_resource).toBe("helpdesk")
  })

  test("should not let regular agents manage the helpdesk", async ({ fdm }) => {
    const result = await getHelpdeskPermission(fdm, "helpdesk", "write", "", agent_id)
    expect(result).toBeNull()
  })

  test("should not let a regular user view the helpdesk", async ({ fdm }) => {
    const result = await getHelpdeskPermission(fdm, "helpdesk", "read", "", user_id)
    expect(result).toBeNull()
  })

  test("should not let a regular user modify the helpdesk", async ({ fdm }) => {
    const result = await getHelpdeskPermission(fdm, "helpdesk", "write", "", user_id)
    expect(result).toBeNull()
  })
})
...
describe("getHelpdeskPermission — agent resource", () => {
  ...
  test("should let admin read any agent", async ({ fdm }) => {
    const result = await getHelpdeskPermission(fdm, "agent", "read", agent_id, admin_id)
    expect(result).not.toBeNull()
    expect(result?.granting_resource).toBe("agent")
  })

  test("should let admin write any agent", async ({ fdm }) => {
    const result = await getHelpdeskPermission(fdm, "agent", "write", agent_id, admin_id)
    expect(result).not.toBeNull()
    expect(result?.granting_resource).toBe("agent")
  })

  test("should let regular agents view themselves", async ({ fdm }) => {
    const result = await getHelpdeskPermission(fdm, "agent", "read", agent_id, agent_id)
    expect(result).not.toBeNull()
    expect(result?.granting_resource).toBe("agent")
  })

  test("should let regular agents modify themselves", async ({ fdm }) => {
    const result = await getHelpdeskPermission(fdm, "agent", "write", agent_id, agent_id)
    expect(result).not.toBeNull()
    expect(result?.granting_resource).toBe("agent")
  })

  test("should not let regular agents modify each other", async ({ fdm }) => {
    const result = await getHelpdeskPermission(fdm, "agent", "write", other_agent_id, agent_id)
    expect(result).toBeNull()
  })

  test("should not grant access for a nonexistent agent resource_id", async ({ fdm }) => {
    const result = await getHelpdeskPermission(fdm, "agent", "read", createId(), admin_id)
    expect(result).toBeNull()
  })

  test("should let an inactive agent read their own data", async ({ fdm }) => {
...
describe("getHelpdeskPermission — saved reply resource", () => {
  ...
  test("should not let regular users view a saved reply", async ({ fdm }) => {
    ...
    expect(
      await getHelpdeskPermission(fdm, "saved_reply", "read", saved_reply_id, principal_id),
    ).toBeNull()
  })

  test("should not let agents read a nonexistent saved reply", async ({ fdm }) => {
    const saved_reply_id = createId()
    expect(
      await getHelpdeskPermission(fdm, "saved_reply", "read", saved_reply_id, admin_id),
    ).toBeNull()
  })
})
...
describe("getHelpdeskPermission — ticket-user-side resource", () => {
  ...
  test("should not let unrelated user read someone else's ticket", async ({ fdm }) => {
    const result = await getHelpdeskPermission(
      fdm,
      "ticket-user-side",
      "read",
  ...
  test("should let active agent read any ticket", async ({ fdm }) => {
    const result = await getHelpdeskPermission(fdm, "ticket-user-side", "read", ticket_id, admin_id)
    expect(result).not.toBeNull()
  })
})
...
describe("getHelpdeskPermission — ticket-agent-side resource", () => {
  ...
  test("should not let inactive agent write ticket-agent-side", async ({ fdm }) => {
    const result = await getHelpdeskPermission(
      fdm,
      "ticket-agent-side",
      "write",
  ...
  test("should not let requester write ticket-agent-side", async ({ fdm }) => {
    const result = await getHelpdeskPermission(
      fdm,
      "ticket-agent-side",
      "write",
  ...
  test("should let requester read their own ticket-agent-side", async ({ fdm }) => {
    const result = await getHelpdeskPermission(
      fdm,
      "ticket-agent-side",
      "read",
...
describe("getHelpdeskPermission — message resource", () => {
  ...
  test.beforeEach(async ({ fdm }) => {
    ...
    internal_message_id = await addMessage(fdm, ticket_id, admin_id, "agent", "Internal note", true)
  })

  test("should let requester read a non-internal message on their ticket", async ({ fdm }) => {
    const result = await getHelpdeskPermission(
      fdm,
      "message",
      "read",
  ...
  test("should not let the requester read an internal message on their ticket", async ({ fdm }) => {
    const result = await getHelpdeskPermission(
      fdm,
      "message",
      "read",
  ...
  test("should let an admin read any message including internal", async ({ fdm }) => {
    const result = await getHelpdeskPermission(
      fdm,
      "message",
      "read",
  ...
  test("should let the requester write their own message", async ({ fdm }) => {
    const result = await getHelpdeskPermission(
      fdm,
      "message",
      "write",
  ...
  test("should let admin write any message", async ({ fdm }) => {
    const result = await getHelpdeskPermission(fdm, "message", "write", public_message_id, admin_id)
    expect(result).not.toBeNull()
  })

  test("should not let an unrelated user write a message they did not send", async ({ fdm }) => {
    const result = await getHelpdeskPermission(
      fdm,
      "message",
      "write",

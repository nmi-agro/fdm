...
describe("Tag CRUD", () => {
  ...
  test("should throw when getting a tag that does not exist", async ({ fdm }) => {
  ...
  test("should not let tags with empty name", async ({ fdm }) => {
    ...
    try {
      await createTag(fdm, admin_id, "", "#ff2222", "This is a very mysterious tag.")
      throw failError
    } catch (err) {
      if (err === failError) throw err
      const error = err as Error
      expect(error.cause).toBeDefined()
      expect((error.cause as Error).message).toBe("Tag name cannot be empty")
    }
  })

  test("should prevent name collision when creating a tag", async ({ fdm }) => {
    const nameSuffix = createId(8)
    await createTag(fdm, admin_id, `Red${nameSuffix}`, "#ff0000", "This is a red tag.")
    ...
    try {
      ...
      const error = err as Error
      expect(error.cause).toBeDefined()
      expect((error.cause as Error).message).toBe("Another tag with name already exists")
    }
  })
  ...
  test("should update tags", async ({ fdm }) => {
    ...
    await updateTag(fdm, admin_id, tag_id, undefined, "#ff0022", "This is a reddish tag.")
...
describe("Ticket Tags", () => {
  ...
  test.beforeEach(async ({ fdm }) => {
    ...
    ticket_id = await createTicket(fdm, requester_id, "This is a ticket with tags.")
  ...
  test("should throw an error if the target ticket doesn't exist", async ({ fdm }) => {
    const failError = new Error("Should have thrown")
    try {
  ...
  test("should do nothing while adding a tag to a ticket that it already has", async ({ fdm }) => {
    await addTagToTicket(fdm, admin_id, ticket_id, blue_tag_id)
    await addTagToTicket(fdm, admin_id, ticket_id, blue_tag_id)
  ...
  test("should remove a tag to a ticket", async ({ fdm }) => {
    ...
    expect(ticket.tags.some((tag) => tag.tag_id === blue_tag_id)).toBe(false)
  })
  ...
  test("should remove tag from all tickets then delete it", async ({ fdm }) => {
    const tag_to_delete_id = await createTag(fdm, admin_id, `Purple${createId(8)}`, "#aa00ff")
    await addTagToTicket(fdm, admin_id, ticket_id, tag_to_delete_id)
    await deleteTag(fdm, admin_id, tag_to_delete_id)

    const ticketTags = (await getTagsForTickets(fdm, admin_id, [ticket_id])).get(ticket_id) ?? []
    expect(ticketTags.some((tag) => tag.tag_id === tag_to_delete_id)).toBe(false)
  })
})
...
describe("getTags", () => {
  ...
  test("should return all tags", async ({ fdm }) => {
    const nameSuffix = createId(8)
    const tag_id_1 = await createTag(fdm, admin_id, `Alpha${nameSuffix}`, "#aaaaaa")
    const tag_id_2 = await createTag(fdm, admin_id, `Beta${nameSuffix}`, "#bbbbbb")
...
describe("Tag authorization and edge cases", () => {
  ...
  test("should let regular agents read tags for their tickets", async ({ fdm }) => {
    ...
    const tag_id = await createTag(fdm, admin_id, `AgentTag${createId(8)}`, "#aabbcc")
    const tag_id_2 = await createTag(fdm, admin_id, `AgentTag${createId(8)}`, "#ddeeff")
    ...
    const tagsMap = await getTagsForTickets(fdm, regular_agent_id, [ticket_id])
  ...
  test("regular user cannot create a tag", async ({ fdm }) => {
    await expect(createTag(fdm, user_id, `UserTag${createId(8)}`, "#123456")).rejects.toThrow(
      "Principal does not have permission to perform this action",
    )
  })

  test("updateTag throws when tag does not exist", async ({ fdm }) => {
    await expect(updateTag(fdm, admin_id, "nonexistent-tag-id", "New Name")).rejects.toThrow()
  })
})

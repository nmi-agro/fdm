...
export async function getAssigneesForTicketsUnchecked(
  ...
  try {
    const allAssignees = await fdm
      ...
      .innerJoin(schema.agents, eq(schema.ticketAssignments.agent_id, schema.agents.agent_id))
      .where(
        and(
      ...
      .orderBy(schema.ticketAssignments.ticket_id, asc(schema.agents.display_name))

    const result = new Map<schema.TicketTypeSelect["ticket_id"], TicketAssignmentSummary[]>()
...
export async function assignTicket(
  ...
  try {
    ...
    await assignTicketInner(fdm, ticket_id, agent_id, assigned_by, is_primary)
  } catch (err) {
...
export async function getTicketCountsForAssignees(
  ...
  try {
    ...
    const aliasedTicketAssignments = alias(schema.ticketAssignments, "GROUP_COLUMN")
    const entries = await fdm
      ...
      .innerJoin(schema.tickets, eq(aliasedTicketAssignments.ticket_id, schema.tickets.ticket_id))
      .leftJoin(schema.ticketTagsMap, eq(schema.tickets.ticket_id, schema.ticketTagsMap.ticket_id))
      .leftJoin(schema.ticketViews, and(eq(schema.ticketViews.ticket_id, schema.tickets.ticket_id)))
      .where(
        and(

...
describe("Agent CRUD", () => {
  ...
  test("should create an admin agent without permission checks", async ({ fdm }) => {
  ...
  test("should not overwrite existing agent as admin", async ({ fdm }) => {
    ...
    try {
      ...
      expect((error.cause as Error).message).toBe("Agent with same ID already exists")
    }
  })
  ...
  test("should update existing agent if adding with the same id", async ({ fdm }) => {
    await addAgent(fdm, admin_id, agent_id, "Support Agent")
  ...
  test("should not let regular helpdesk agents create agents", async ({ fdm }) => {
    ...
    try {
      ...
      expect(error.message).toContain("Principal does not have permission to perform this action")

      await expect(getAgent(fdm, admin_id, third_agent_id)).rejects.toThrow(
  ...
  test("should not let regular helpdesk agents update each other's information", async ({
    ...
    const failError = new Error("Should have thrown")
    try {
      await updateAgent(fdm, agent_id, third_agent_id, "Support Agent with Low Performance")
      throw failError
    } catch (err) {
      if (err === failError) throw err
      const error = err as Error
  ...
  test("should not allow deactivating the last active admin", async ({ fdm }) => {
    const failError = new Error("Should have thrown")
    try {
      ...
      const error = err as Error
      expect(error.cause).toBeDefined()
      expect((error.cause as Error).message).toBe("There must be at least one admin left")
    }
  })
  ...
  test("should let an agent update their own display name", async ({ fdm }) => {
...
describe("getAgentCount", () => {
  ...
  test("should throw when a regular user tries to get agent count", async ({ fdm }) => {
...
describe("updateAgentRole", () => {
  ...
  test("should allow admin to promote an agent to admin role", async ({ fdm }) => {
  ...
  test("should not allow demoting the last active admin", async ({ fdm }) => {
    ...
    try {
      ...
      const error = err as Error
      expect(error.cause).toBeDefined()
      expect((error.cause as Error).message).toBe("There must be at least one active admin left")
    }
  })

  test("should allow demoting an admin when another admin exists", async ({ fdm }) => {
  ...
  test("should not let regular agents change agent roles", async ({ fdm }) => {
    await expect(updateAgentRole(fdm, agent_id, admin_id, "agent")).rejects.toThrow(
      "Principal does not have permission to perform this action",
    )
  })
})

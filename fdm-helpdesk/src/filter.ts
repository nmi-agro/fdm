...
export function getMessageWhereClause(filters: MessageFilters) {
  return and(
    ...
    typeof filters.isInternal === "boolean"
      ...
      : undefined,
    !filters?.includeDeleted ? isNull(schema.messages.deleted_at) : undefined,
    filters?.fromDate ? gte(schema.messages.created, filters.fromDate) : undefined,
    filters?.toDate ? lte(schema.messages.created, filters.toDate) : undefined,
...
export function getTicketWhereClause(
  ...
  if (filters.minPriority || filters.maxPriority) {
    ...
    const minPriorityIndex = filters?.minPriority ? priorities.indexOf(filters.minPriority) : -1
    const maxPriorityIndex = filters?.maxPriority ? priorities.indexOf(filters.maxPriority) : -1
    if (minPriorityIndex !== -1 || maxPriorityIndex !== -1) {
      priorityFilter = inArray(
        ...
        priorities.slice(
          ...
          maxPriorityIndex !== -1 ? maxPriorityIndex + 1 : priorities.length,
        ),
      )
    }
  }
  ...
  return and(
    ...
    filters.text && filters.text.trim().length > 0
      ...
      : undefined,
    // Timeframe filter
    filters?.fromDate ? gte(schema.tickets.created, filters.fromDate) : undefined,
    filters?.toDate ? lte(schema.tickets.created, filters.toDate) : undefined,
    // Viewed filter
          ...
          fdm
            ...
            .where(
              and(
                ...
                inArray(schema.ticketViews.actor_id, filters.notViewedBy),
              ),
            ),
        )
      : undefined,

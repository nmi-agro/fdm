...
describe("handleError", () => {
  ...
  test("should pass through the permission denied message unchanged", () => {
    const cause = new Error("Principal does not have permission to perform this action")
    const result = handleError(cause, "Exception for someOperation")
    expect(result.message).toBe("Principal does not have permission to perform this action")
  })

  test("should attach context to the error", () => {
    const result = handleError(new Error("cause"), "Exception for operation", {
      resource: "ticket",
      id: "abc",
    })

...
test.describe("Inbox", () => {
  ...
  test.beforeEach(async ({ fdm }) => {
    ...
    blue_tag_id = await createTag(fdm, agent_id, blue_tag_name, "#0000ff", "This is a blue tag.")
...
test.describe("getTickets", () => {
  ...
  test.beforeEach(async ({ fdm }) => {
    ...
    await addMessage(fdm, ticket_id_1, requester_id, "customer", "Public Anchovy")
    ticket_id_2 = await createTicket(fdm, requester_id, "Ticket 2. Banana", {
      priority: "normal",
    })
    await addMessage(fdm, ticket_id_2, agent_id, "agent", "Internal Bread", true)
  ...
  test("should filter by from-date", async ({ fdm }) => {
    const new_ticket_id = await createTicket(fdm, requester_id, "Ticket In The Future", {
      context: { b_id_farm: b_id_farm },
      priority: "low",
    })
  ...
  test("should filter by to-date", async ({ fdm }) => {
    const new_ticket_id = await createTicket(fdm, requester_id, "Ticket 3", {
      context: { b_id_farm: b_id_farm },
      priority: "low",
    })
  ...
  test("should search for ticket ref when subject is null", async ({ fdm }) => {
    const ticket_id = await createTicket(fdm, requester_id, "Very mysterious ticket")
  ...
  test("should only list regular user's own tickets", async ({ fdm }) => {
    const new_ticket_id = await createTicket(fdm, agent_id, "Ticket by Agent", {
      context: { b_id_farm: b_id_farm },
      priority: "low",
    })
  ...
  test("should sort by priority", async ({ fdm }) => {
    ...
    const tickets = await getTickets(fdm, agent_id, { assignees: [agent_id] }, "priority")

    expect(tickets[0].ticket_id, "Ticket 1 has a higher priority than Ticket 2").toBe(ticket_id_1)
    expect(tickets[1].ticket_id).toBe(ticket_id_2)
  })

  test("should sort by text relevance", async ({ fdm }) => {
    const specific_ticket_id_1 = await createTicket(fdm, requester_id, "New helpdesk feature")
    const specific_ticket_id_2 = await createTicket(fdm, requester_id, "New feature")
  ...
  test("should sort by creation date without a text filter", async ({ fdm }) => {
    const specific_agent_id = createId()
    await addAgent(fdm, agent_id, specific_agent_id, "Specific Agent")
    const specific_ticket_id_1 = await createTicket(fdm, requester_id, "New helpdesk feature")
    const specific_ticket_id_2 = await createTicket(fdm, requester_id, "New feature")
    await assignTicket(fdm, specific_ticket_id_1, specific_agent_id, specific_agent_id, true)
    await assignTicket(fdm, specific_ticket_id_2, specific_agent_id, specific_agent_id, true)
...
describe("updateTicketSubjectAndPriorityUnchecked", () => {
  ...
  test("should update without any permission checks", async ({ fdm }) => {
    ...
    await updateTicketSubjectAndPriorityUnchecked(fdm, ticket_id, "Ticket 1 Subject", "high")
  ...
  test("should be able to update subject only", async ({ fdm }) => {
    ...
    await updateTicketSubjectAndPriorityUnchecked(fdm, ticket_id, "Ticket 1 Subject")
    const ticket = await getTicket(fdm, agent_id, ticket_id)
    expect(ticket.subject).toBe("Ticket 1 Subject")
    expect(ticket.priority).toBe("high")
    expect(ticket.updated).not.toBeNull()
  ...
  test("should be able to update priority only", async ({ fdm }) => {
    ...
    await updateTicketSubjectAndPriorityUnchecked(fdm, ticket_id, undefined, "low")
    const ticket = await getTicket(fdm, agent_id, ticket_id)
...
describe("validateTicketStatusTransition", () => {
  ...
  test("should disallow unknown transitions", () => {
    ...
    expect(validateTicketStatusTransition("closed", "in_progress")).toBe(false)
...
describe("updateTicketStatus", () => {
  ...
  test("should let agent update ticket status with a valid transition", async ({ fdm }) => {
  ...
  test("should set resolved_at when transitioning to resolved", async ({ fdm }) => {
  ...
  test("should not let regular user update ticket status", async ({ fdm }) => {
    await expect(updateTicketStatus(fdm, requester_id, ticket_id, "in_progress")).rejects.toThrow(
  ...
  test("should throw on an invalid status transition", async ({ fdm }) => {
    ...
    await expect(updateTicketStatus(fdm, agent_id, ticket_id, "in_progress")).rejects.toThrow()
  ...
  test("should not update close time if closing twice", async ({ fdm }) => {
    ...
    expect(
      Math.abs((ticketAfter.closed_at as Date).getTime() - closeDate.getTime()),
      "Ticket close date must not change",
    ).toBeLessThan(1000)
  })

  test("should clear the closed_at date if opening again", async ({ fdm }) => {
    await updateTicketStatus(fdm, agent_id, ticket_id, "closed")
...
describe("assignTicket", () => {
  ...
  test("should mark other assignments as non-primary if is_primary=true", async ({ fdm }) => {
    ...
    const secondAgent = assignments.find((a) => a.agent_id === second_agent_id)
  ...
  test("should not let regular user assign a ticket", async ({ fdm }) => {
    await expect(assignTicket(fdm, ticket_id, agent_id, requester_id)).rejects.toThrow(
      "Principal does not have permission to perform this action",
    )
  })
})
...
describe("createTicket", () => {
  ...
  test("should create a ticket and return its id", async ({ fdm }) => {
    const ticket_id = await createTicket(fdm, requester_id, "My ticket body")
  ...
  test("should set the priority from options", async ({ fdm }) => {
    const ticket_id = await createTicket(fdm, requester_id, "Urgent ticket", { priority: "urgent" })
  ...
  test("should escape HTML in the body", async ({ fdm }) => {
    const ticket_id = await createTicket(fdm, requester_id, `"test"<script>alert('xss')</script>`)

    const ticket = await getTicket(fdm, requester_id, ticket_id)

    const messages = await getMessagesForTicket(fdm, requester_id, ticket_id)
...
describe("getTicket", () => {
  ...
  test("should return a ticket with its tags and assignees", async ({ fdm }) => {
  ...
  test("should throw when an unrelated user tries to view the ticket", async ({ fdm }) => {
...
describe("getTicketCount", () => {
  ...
  test("should not count tickets multiple times", async ({ fdm }) => {
    ...
    await addMessage(fdm, ticket_id, requester_id, "agent", "Please don't hesitate to ask.")
    const tag_id_1 = await createTag(fdm, admin_id, `Blue${createId(8)}`, "#0000ff")
    const tag_id_2 = await createTag(fdm, admin_id, `Red${createId(8)}`, "#ff0000")
...
describe("getTicketCount wrappers", async () => {
  ...
  test.beforeEach(async ({ fdm }) => {
    ...
    const assigned_ticket = await createTicket(fdm, requester_id, "Ticket 3")
  ...
  test("should get the number of unread and requested tickets", async ({ fdm }) => {
    const ticketCount = await getUnreadRequestedTicketCount(fdm, requester_id)
    expect(ticketCount).toBe(2)
  })

  test("should get the number of unread and assigned tickets", async ({ fdm }) => {
...
describe("getDefaultTicketSubject", () => {
  ...
  test("should break first word if subject is becoming too short", () => {
    expect(getDefaultSubjectLine(veryLongFirstWord)).toBe(veryLongFirstWord.slice(0, 20))
  })
  test("should break second word if subject is becoming too short", () => {
    expect(getDefaultSubjectLine(veryLongSecondWord)).toBe(veryLongSecondWord.slice(0, 20))
  })
...
describe("markTicketAsViewed", () => {
  ...
  test("should throw an error if the actor cannot read the ticket", async ({ fdm }) => {
    const third_user_id = createId()
    await expect(markTicketAsViewed(fdm, third_user_id, ticket_id)).rejects.toThrow(
      "Principal does not have permission to perform this action",
    )
  })
})

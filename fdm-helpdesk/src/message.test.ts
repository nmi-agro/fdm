...
describe("Message CRUD", () => {
  ...
  test.beforeEach(async ({ fdm }) => {
    ...
    ticket_id = await createTicket(fdm, requester_id, `Seed message ${createId(8)}`)
  })

  test("should persist a customer message", async ({ fdm }) => {
    const body = `Customer message ${createId(8)}`

    const message_id = await addMessage(fdm, ticket_id, requester_id, "customer", body)
  ...
  test("should reject internal messages by regular users", async ({ fdm }) => {
    await expect(
      addMessage(
        fdm,
        ticket_id,
  ...
  test("should hide internal messages from regular users", async ({ fdm }) => {
    ...
    const requesterMessages = await getMessagesForTicket(fdm, requester_id, ticket_id, {
      includeDeleted: false,
    })

    expect(
      requesterMessages.some((msg) => msg.message_id === customer_message_id),
      "Expected regular user to see non-internal messages",
    ).toBe(true)
    expect(
      requesterMessages.some((msg) => msg.message_id === internal_message_id),
      "Expected regular user to not see internal messages",
    ).toBe(false)
  })

  test("should support includeDeleted and sender filter for agents", async ({ fdm }) => {
    ...
    const withoutDeleted = await getMessagesForTicket(fdm, admin_id, ticket_id, {
      includeDeleted: false,
    })

    const withDeleted = await getMessagesForTicket(fdm, admin_id, ticket_id, {
      includeDeleted: true,
    })

    expect(withoutDeleted.some((msg) => msg.message_id === deleted_message_id)).toBe(false)
    expect(withDeleted.some((msg) => msg.message_id === deleted_message_id)).toBe(true)
    expect(withDeleted.some((msg) => msg.message_id === active_message_id)).toBe(true)
  })
  ...
  test("should update message body", async ({ fdm }) => {
    ...
    await updateMessage(fdm, requester_id, message_id, "Updated Message Body")
  ...
  test("should escape message body when updating", async ({ fdm }) => {
    ...
    await updateMessage(fdm, requester_id, message_id, `"test"<script>alert('xss')</script>`)
  ...
  test("should not allow a regular user to make a message internal", async ({ fdm }) => {
    const message_id = await addMessage(
      fdm,
      ticket_id,
      requester_id,
    ...
    await expect(updateMessage(fdm, requester_id, message_id, undefined, true)).rejects.toThrow(
      "Principal does not have permission to perform this action",
    )
  })

  test("should reject updates by unrelated users", async ({ fdm }) => {
    const message_id = await addMessage(fdm, ticket_id, requester_id, "customer", "Message Body")

    await expect(
      updateMessage(fdm, other_user_id, message_id, "Hacked!!11!1!", false),
    ).rejects.toThrow("Principal does not have permission to perform this action")
  })
...
describe("Message Pagination", () => {
  ...
  test.beforeAll(async ({ fdm }) => {
    ...
    const messages: Promise<string>[] = [Promise.resolve(currentMessages[0].message_id)]
    ...
    for (let i = 1; i <= 99; i += 2) {
      messages.push(addOrderedMessage(fdm, ticket_id, admin_id, "agent", i))
      messages.push(addOrderedMessage(fdm, ticket_id, requester_id, "customer", i + 1))
    }
    message_ids = await Promise.all(messages)
  })

  test("should not paginate when no custom pagination specified", async ({ fdm }) => {
    const messages = await getMessagesForTicket(fdm, requester_id, ticket_id)

    expect(messages).toHaveLength(message_ids.length)
    expect(messages.map((message) => message.message_id)).toEqual(message_ids)
  })
  ...
  test("should paginate with custom pagination", async ({ fdm }) => {
    ...
    expect(messages.map((message) => message.message_id)).toEqual(message_ids.slice(11, 11 + 19))
  })
  ...
  test("should normalize bad pagination input", async ({ fdm }) => {
    ...
    expect(messages.map((message) => message.message_id)).toEqual(message_ids.slice(0, 1))
  })
})
...
describe("Message Filters", () => {
  ...
  test("should filter messages by fromDate", async ({ fdm }) => {
    const early_id = await addMessage(fdm, ticket_id, requester_id, "customer", "Early message")
    const late_id = await addMessage(fdm, ticket_id, requester_id, "customer", "Late message")
  ...
  test("should filter messages by sentBy", async ({ fdm }) => {
    ...
    const agent_msg_id = await addMessage(fdm, ticket_id, admin_id, "agent", "Agent message")
    ...
    expect(messages.some((m) => m.message_id === requester_msg_id)).toBe(false)
  })
})

...
import { TICKET_VIEWER_PAGE_SIZE, TicketViewer } from "~/components/blocks/helpdesk/ticket-viewer"
...
export const meta: Route.MetaFunction = ({ loaderData }) => {
  ...
  if (loaderData.defaultFilterType === "unassigned") {
    return [
      ...
      {
        name: "description",
        content: "Bekijk en bewerk de tickets die nog niet toegewezen zijn.",
      },
    ]
  }
...
function mergeFiltersInner(end: number, filters: TicketFilters[]): TicketFilters {
  if (end === 0) return {}
  ...
  if (base.assignees || current.assignees) {
    result.assignees = [...(base.assignees ?? []), ...(current.assignees ?? [])]
  }
  if (base.requesterIds || current.requesterIds) {
    result.requesterIds = [...(base.requesterIds ?? []), ...(current.requesterIds ?? [])]
  }
...
export async function loader({ request, url }: Route.LoaderArgs) {
  try {
    ...
    const pageOffset = pageOffsetStr ? Number.parseInt(pageOffsetStr, 10) : undefined
    const pageLimitStr = url.searchParams.get("pageLimit")
    let pageLimit = pageLimitStr ? Number.parseInt(pageLimitStr, 10) : undefined
    ...
    const {
      ...
      ...userFilters
    } = TicketFilterSchema.parse(JSON.parse(url.searchParams.get("filters") ?? "{}"))
    ...
    const searchParamSorting = (url.searchParams.get("sorting") ?? defaultSorting) as TicketSorting
    const sorting = (["created", "priority", "text_relevance"] satisfies TicketSorting[]).includes(
      searchParamSorting,
    )
      ? searchParamSorting
      : defaultSorting

    const totalTicketCount = await getTicketCount(fdm, session.principal_id, filters)
    ...
    if (
      ...
      newSearchParams.set(
        ...
        Math.max(
          0,
          (Math.ceil(totalTicketCount / TICKET_VIEWER_PAGE_SIZE) - 1) * TICKET_VIEWER_PAGE_SIZE,
        ).toString(),
      )
      newSearchParams.set("pageLimit", TICKET_VIEWER_PAGE_SIZE.toString())
      return redirect(`${url.pathname}?${newSearchParams.toString()}`)
...
export default function PersonalTicketViewer() {
  const { tickets, totalTicketCount, principals, helpdeskReadPermission, availableTags } =

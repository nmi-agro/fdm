  ...
  try {
    ...
    const features = await Promise.all(
      fields.map(async (field) => {
        ...
        let cultivation: string | undefined | null = (field as { b_lu_name?: string }).b_lu_name
        if (!cultivation) {
          try {
        ...
        <div className="space-y-6 p-10 pb-0">
          <div className="flex items-center">
            <div className="space-y-0.5">
              ...
              <p className="text-muted-foreground">Zoom in en selecteer je percelen</p>
            </div>
          </div>
              ...
              <MapGL
                ...
                onClick={(evt) => {
                  ...
                  const polygonFeature = evt.features.find(
                    (f) => f.source === fieldsSavedId && f.geometry?.type === "Polygon",
                  )
                  if (polygonFeature) {
                    void handleClickSavedField(polygonFeature as Feature<Polygon>)
                  }
                }}
              >
                <Controls
                ...
                <FieldsSourceNotClickable id={fieldsSavedId} fieldsData={fieldsSaved}>
...
export async function action({ request, params }: ActionFunctionArgs) {
  try {
    ...
    try {
      firstFieldIndex =
        (await getFields(fdm, session.principal_id, b_id_farm, timeframe)).length + 1
    } catch (e) {
      console.warn(e)
      firstFieldIndex = 1
    }

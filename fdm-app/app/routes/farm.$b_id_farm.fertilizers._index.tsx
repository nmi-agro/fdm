...
export async function loader({ request, params }: LoaderFunctionArgs) {
  try {
    ...
    const rvoLabelByValue = new Map(
      p_type_rvo_options.map((opt: { value: string | null; label: string }) => [
        String(opt.value ?? ""),
        opt.label,
      ]),
    )
    ...
    const fertilizers: Fertilizer[] = await getFertilizers(
      ...
      fertilizers.map((fertilizer) => ({
        ...
        p_type_rvo_label: rvoLabelByValue.get(String(fertilizer.p_type_rvo ?? "")) ?? null,
...
export default function FarmFertilizersIndexPage({ params: _params }: Route.ComponentProps) {

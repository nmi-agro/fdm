...
export async function loader({ request, params }: Route.LoaderArgs) {
  try {
    ...
    if (permissions.canInvite) {
      ...
      invitations = (Array.isArray(invitationsListResponse) ? invitationsListResponse : []).filter(
...
export default function OrganizationIndex() {
  const { organization, invitations, members } = useLoaderData<typeof loader>()
        ...
        <div className="grid gap-4 lg:grid-cols-1">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Zie wie er onderdeel uitmaakt van deze organisatie.</CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-6">
                  {members.map((member) => (
                    <MemberRow key={member.id} member={member} permissions={permissions} />
                  ))}
                </div>
              </div>
            </CardContent>
          ...
          {permissions.canInvite ? (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Nodig nieuwe leden uit en zie welke uitnodigingen nog open staan.
                </CardDescription>
              </CardHeader>
              <CardContent>
...
const MemberAction = ({
  ...
  return (
    <fetcher.Form method="post" className="flex items-center space-x-4">
      <Spinner className={cn(fetcher.state !== "submitting" && "invisible")} />
      <input type="hidden" name="memberId" value={member.id} />
      <Select name="role" value={role} onValueChange={submitRoleChange} disabled={disabled}>
...
const FormSchema = z.object({
  ...
  role: z.enum(["owner", "admin", "member"], { error: "Ongeldige rol" }).optional(),
  memberId: z.string().optional(),
  invitation_id: z.string().optional(),
  organization_id: z.string().optional(),
  intent: z.enum(["invite_user", "update_role", "remove_user", "cancel_invite"]),
})
...
export async function action({ request, params }: Route.ActionArgs) {
  try {
    ...
    if (formValues.intent === "invite_user") {
      if (!formValues.email) {
        return dataWithError(null, "Vul een e-mailadres in om iemand uit te nodigen")
      }
      if (!formValues.role) {
        return handleActionError("missing: role")
      }

      let invitation: Awaited<ReturnType<typeof auth.api.createInvitation>> | undefined
      try {
        ...
        if (invitation?.id) {
          try {
            ...
            void handleActionError(cancellationError)
          }
        }
        ...
        void handleActionError(sendingError)
        return dataWithError(null, {
    ...
    if (formValues.intent === "cancel_invite") {
      if (!formValues.invitation_id) throw new Error("invalid invitation_id")
      await auth.api.cancelInvitation({
        headers: request.headers,
        body: {

...
export async function loader({ request, params }: LoaderFunctionArgs) {
  try {
    ...
    const b_date_harvest_default =
      cultivationsCatalogue.find((item) => item.b_lu_catalogue === cultivation.b_lu_catalogue)
...
export default function HarvestNewBlock() {
  ...
  return (
    <HarvestFormDialog
      ...
      b_lu_yield_fresh={loaderData.defaultHarvestParameters.b_lu_yield_fresh ?? undefined}
      b_lu_yield_bruto={loaderData.defaultHarvestParameters.b_lu_yield_bruto ?? undefined}
      ...
      b_lu_n_harvestable={loaderData.defaultHarvestParameters.b_lu_n_harvestable ?? undefined}
...
export async function action({ request, params }: ActionFunctionArgs) {
  try {
    ...
    if (formValues.intent === "batch_harvest" && effectiveHarvestable !== "multiple") {
    ...
    if (formValues.intent === "batch_harvest") {
      const errors = formValues.harvests.map((row) => validateRow(cultivation, row))
      ...
      await fdm.transaction((tx) =>
        Promise.all(
          formValues.harvests.map((row) =>
            addHarvestFromRow(tx, session.principal_id, cultivation, row),
          ),
        ),
      )

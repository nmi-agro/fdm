...
import { getCultivations, getCurrentSoilData, getFarm, getFields } from "@nmi-agro/fdm-core"
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  try {
        ...
        try {
          ...
          if (fieldOmInput) {
            omBalanceResult = await getOrganicMatterBalanceField(fdm, {
              ...
              timeFrame: {
                start: omInput.timeFrame.start ?? timeframe.start ?? new Date(),
                end: omInput.timeFrame.end ?? timeframe.end ?? new Date(),
              },
            })
          }
    ...
    const getBase64Image = (filePath: string | undefined): string | undefined => {
    ...
    const safeName = (farm.b_name_farm || b_id_farm).replace(/[^\w\s-]/g, "_")

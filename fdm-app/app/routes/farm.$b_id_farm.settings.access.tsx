  ...
  try {
    ...
    if (formValues.intent === "invite_user") {
      if (!formValues.username) {
        return dataWithError(null, "Vul een gebruikers- of organisatienaam in om uit te nodigen")
      }
      if (!formValues.role) {
      ...
      try {
        ...
        if (isInactiveRecipientError(emailError)) {
          ...
          if (targetPrincipal && targetPrincipal.type === "user") {
            await revokePrincipalFromFarm(fdm, session.principal_id, formValues.username, b_id_farm)
          }
    ...
    if (formValues.intent === "remove_user") {
      ...
      if (formValues.invitation_id) {
        ...
        await cancelInvitationForFarm(fdm, session.user.id, formValues.invitation_id)
        ...
        await revokePrincipalFromFarm(fdm, session.user.id, formValues.username, b_id_farm)
      }
      return dataWithSuccess(null, {

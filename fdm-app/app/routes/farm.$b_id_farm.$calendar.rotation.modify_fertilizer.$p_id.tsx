  ...
  try {
    ...
    const allApplicationsPerField = await Promise.all(
      fieldIds.map(async (b_id) => {
        ...
        return getFertilizerApplications(fdm, session.principal_id, b_id).then((apps) =>
          apps.map((app) => ({
            ...app,
            b_id: b_id,
            b_name: field.b_name,
    ...
    const applicationsPerField = allApplicationsPerField.map((allApplications) =>
      allApplications
        .filter((app) => app.p_id === params.p_id)
        .sort((app1, app2) => app1.p_app_date.getTime() - app2.p_app_date.getTime()),
    )
...
export default function FertilizerApplicationListDialogRoute() {
  const { isForRotation, numFields, fertilizer, fertilizerApplications, returnUrl } =
      ...
      await fdm.transaction((tx) =>
        Promise.all(
          formData.appIds.map(({ p_app_id }) =>
            removeFertilizerApplication(tx, session.principal_id, p_app_id),
          ),
        ),
      )
      return dataWithSuccess(
        null,
        formData.appIds.length === 1 ? "Bemesting is verwijderd" : "Bemestingen zijn verwijderd",
      )
    }

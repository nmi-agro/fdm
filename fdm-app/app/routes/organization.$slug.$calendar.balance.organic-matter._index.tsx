...
export async function loader({ request, params }: LoaderFunctionArgs): Promise<LoaderData> {
  try {
    ...
    async function getAsyncData(principal_id: string) {
      ...
      const combinedResult = await calculateOrganicMatterBalanceForFarms(fdm, inputs)
      const rawFarmResultsMap: Record<string, OrganicMatterBalanceFieldResultNumeric[]> = {}
      ...
      const farmResults = await Promise.all(
        farmsExtended
          ...
          .map(async (farm) => {
            ...
            try {
              const organicMatterBalanceResult = calculateOrganicMatterBalancesFieldToFarm(
                fieldResults,
...
function OrganizationFarmBalanceOrganicMatterOverview(loaderData: LoaderData) {
  ...
  const { combinedResult: resolvedOrganicMatterBalanceResult, farmResults } = asyncData
  const hasErrors = farmResults.some(
    ({ organicMatterBalanceResult }) => organicMatterBalanceResult.hasErrors,
  )

  const orgAverage = Number.isFinite(resolvedOrganicMatterBalanceResult.balance)
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              <div className="flex items-center gap-4">
                <p>{resolvedOrganicMatterBalanceResult.balance}</p>
                {hasErrors ? (
                  <Tooltip>
                    <TooltipTrigger>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        <Card className="col-span-4">
          <CardHeader>
            ...
            <CardDescription>
              De gemiddelde organische stofbalans voor de geselecteerde bedrijven. De balans is het
          ...
          <CardContent className="pl-2">
            <OrganicMatterBalanceChart
              ...
              degradation={resolvedOrganicMatterBalanceResult.degradation}
            />
          </CardContent>
        </Card>

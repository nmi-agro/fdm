...
export async function loader({ request, params }: LoaderFunctionArgs) {
  try {
    ...
    const isEstimated =
      currentSoilData.length > 0 && currentSoilData.every((i) => i.a_source === "nl-other-nmi")
...
export async function action({ request, params }: ActionFunctionArgs) {
  try {
    ...
    if (request.method === "POST") {
      ...
      url.searchParams.set("fieldIds", fieldIds.filter((id) => id !== b_id).join(","))
      return redirectWithSuccess(`/farm/${b_id_farm}/${calendar}/field/new/fields${url.search}`, {

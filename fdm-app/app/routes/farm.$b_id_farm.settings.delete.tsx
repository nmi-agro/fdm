...
export async function loader({ request, params }: LoaderFunctionArgs) {
  try {
    ...
    let canDeleteFarm = await isAllowedToDeleteFarm(fdm, session.principal_id, b_id_farm)
    // Add temporary workaround (until implemented in fdm-core) so that advisors are not able to delete a farm
    if (canDeleteFarm && farm.roles.find((role) => role.role === "advisor")) {
      canDeleteFarm = false
    }
...
export default function FarmSettingsDeleteBlock() {
  ...
  const isSubmitting =
    navigation.state !== "idle" && navigation.formMethod?.toLowerCase() === "delete"
  return (
      ...
      {canDeleteFarm ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Deze actie kan niet ongedaan worden gemaakt. Dit verwijdert het bedrijf "
              {farm.b_name_farm}" en alle bijbehorende gegevens, inclusief percelen, gewassen,
              bemestingen, bodemanalyses en oogsten.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <FarmDeleteDialog farmName={farm.b_name_farm || ""} isSubmitting={isSubmitting} />
          </CardContent>
        </Card>
      ) : null}
    </div>

...
export async function loader({ request, params }: LoaderFunctionArgs) {
  try {
    ...
    const isGrazing = await getGrazingIntention(fdm, session.principal_id, b_id_farm, year)
    ...
    for (const app of applications) {
      if (app.p_app_date) {
        const name = fertilizerNameMap.get(app.p_id) ?? app.p_name_nl ?? "Mest"
    ...
    return (
      <Empty>
        <EmptyHeader>
          ...
          <EmptyDescription>
            Mineralisatieberekeningen zijn niet beschikbaar voor bufferstroken.
          </EmptyDescription>
        </EmptyHeader>
      </Empty>
    )
    ...
    return (
      <Empty>
        <EmptyHeader>
          ...
          <EmptyDescription>
            DYNA heeft een oogstdatum nodig om de berekening uit te voeren. Voeg een oogstdatum toe
            aan het gewas op dit perceel.
          </EmptyDescription>
        </EmptyHeader>
        <NavLink
...
function DynaContent({
  ...
  const { calculationDyna, nitrogenBalance, fertilizingRecommendations, harvestingRecommendation } =
    result
    ...
    <>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>
              {isCurrentYear ? "N aanbod nu" : `N aanbod op ${lastPointDateLabel}`}
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>
              {isCurrentYear ? "N opname nu" : `N opname op ${lastPointDateLabel}`}
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            N-beschikbaarheid (bandbreedte en verwachting) versus N-opname door het gewas
          </CardDescription>
        </CardHeader>
        <CardContent>
          <DynaChart
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <DynaBalanceCard nitrogenBalance={nitrogenBalance} leaching={totalLeaching} />
        <DynaAdviceCard
          fertilizingRecommendations={fertilizingRecommendations}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Cumulatieve nitraatuitspoeling (kg N-NO₃/ha)</CardDescription>
        </CardHeader>
        <CardContent>

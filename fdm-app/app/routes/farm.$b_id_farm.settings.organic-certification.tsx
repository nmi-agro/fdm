...
export async function action({ request, params }: ActionFunctionArgs) {
  try {
    ...
    if (request.method === "DELETE") {
      ...
      await removeOrganicCertification(fdm, session.principal_id, b_id_organic)
...
export default function OrganicCertificationSettings() {
  const { organicCertification, farmWritePermission } = useLoaderData<typeof loader>()
  const navigation = useNavigation()

  const isDeleting = navigation.state !== "idle" && navigation.formMethod === "DELETE"

  const form = useRemixForm<z.input<typeof formSchema>, any, z.output<typeof formSchema>>({
    mode: "onTouched",
      ...
      {organicCertification ? (
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
            ...
            <div className="flex flex-col">
              <span className="text-muted-foreground text-sm">SKAL-nummer</span>
              <span className="font-medium">
                {organicCertification.b_organic_skal || "Onbekend"}
              </span>
            </div>
            <div className="flex flex-col">
              <span className="text-muted-foreground text-sm">Geldig van:</span>
              <span className="font-medium">
                {format(new Date(String(organicCertification.b_organic_issued)), "PP", {
                  locale: nl,
                })}
              </span>
            </div>
            <div className="flex flex-col">
              <span className="text-muted-foreground text-sm">Verloopt op:</span>
              <span className="font-medium">
                {organicCertification.b_organic_expires
                  ? format(new Date(String(organicCertification.b_organic_expires)), "PP", {
                      locale: nl,
                    })
                  : "Onbekend"}
              </span>
            ...
            <Dialog>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Deze actie kan niet ongedaan worden gemaakt. Dit zal het bio-certificaat
                ...
                <DialogFooter>
                  ...
                  <Form method="delete">
                    <input
                      type="hidden"
                      name="b_id_organic"
                      value={organicCertification.b_id_organic}
                    />
        ...
        <Dialog>
          <Empty>
            <EmptyHeader>
              ...
              <EmptyTitle>Dit bedrijf heeft geen bio-certificaat</EmptyTitle>
              <EmptyDescription>
                {farmWritePermission
            ...
            <EmptyContent>
              <DialogTrigger asChild>
                <Button className={cn(!farmWritePermission ? "invisible" : "")}>Voeg toe</Button>
              </DialogTrigger>
            </EmptyContent>
          </Empty>
          <DialogContent>
            <RemixFormProvider {...form}>
              <Form onSubmit={form.handleSubmit} method="POST" id={formId}>
                <fieldset disabled={form.formState.isSubmitting} className="space-y-4">
                  <DialogHeader>
                    <DialogTitle>Voeg een bio-certificaat toe</DialogTitle>
                    <DialogDescription>
                      Vul de gegevens van het bio-certificaat in.
                    </DialogDescription>
                  </DialogHeader>
                  <DatePicker
                    form={form as any}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>TRACES-nummer</FormLabel>
                        <FormControl>
                          <Input {...field} />
                        </FormControl>
                        <FormDescription>
                          Het volledige documentnummer volgens de EU Traces-database.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>SKAL-nummer</FormLabel>
                        <FormControl>
                          <Input {...field} />
                        </FormControl>
                        <FormDescription>Het SKAL-nummer van de certificering.</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />

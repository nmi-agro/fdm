...
export default function FarmAtlasSoilBlock() {
  ...
  // State
  const [selectedSoilFeature, setSelectedSoilFeature] = useState<FeatureCollection | null>(null)
  ...
  const [cachedBodemData, setCachedBodemData] = useState<{ key: string; value: BodemData }[]>([])
  ...
  const fetchBodemData = useCallback(
    async (first_soilcode: string | undefined, signal?: AbortSignal) => {
      ...
      try {
        const found = cachedBodemData.find((item) => item.key === first_soilcode)
        if (found) {
          // If found in the cache, move the cached item to the end of the list
          setCachedBodemData((cachedBodemData) => {
            const cachedIndex = cachedBodemData.findIndex((item) => item.key === first_soilcode)
            if (cachedIndex > -1) {
        ...
        if (response.success && response.data) {
          ...
          if (data.omschrijving)
            data.omschrijving = DOMPurify(window).sanitize(data.omschrijving, {
              ALLOWED_TAGS: [],
            })
          setCachedBodemData((cachedBodemData) => {
            const update = [
  ...
  const onMapClick = useCallback(
    async (event: MapLayerMouseEvent) => {
      ...
      const sw = proj4("EPSG:3857").forward([bounds.getWest(), bounds.getSouth()])
      const ne = proj4("EPSG:3857").forward([bounds.getEast(), bounds.getNorth()])
        ...
        if (response.ok) {
          ...
          if (data.features && data.features.length > 0) {
            ...
            if (feature.geometry && (feature.geometry as any).coordinates) {
              ;(feature.geometry as any).coordinates = transformCoords(
            ...
            setPopupInfo((popupInfo) => {
              if (popupInfo && popupInfo.properties.first_soilcode === props.first_soilcode)
                return {
                  ...popupInfo,
                  properties: {
    ...
    <div className="relative h-full w-full">
      ...
      >
        ...
        {showSoil && (
          ...
          >
            <Layer id="soil-wms-layer" type="raster" paint={{ "raster-opacity": 0.7 }} />
          </Source>
        )}

        {/* Selected Soil Feature Highlight */}
        {showSoil && selectedSoilFeature && (
          <Source id="selected-soil-source" type="geojson" data={selectedSoilFeature}>

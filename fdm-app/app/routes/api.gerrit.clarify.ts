...
export async function loader({ request }: LoaderFunctionArgs) {
    ...
    if (!isGerritEnabled) {
        return new Response("Functie niet ingeschakeld voor deze gebruiker", {
            status: 403,
        })
    }
    ...
    const selectedFertilizerIdsRaw = url.searchParams.get(
        ...
        try {
            ...
            if (Array.isArray(parsed)) {
                ...
                if (filtered.length > 0)
                    allowedFertilizerCatalogueIds = filtered
            }
        } catch {}
    }
    ...
    const stream = new ReadableStream({
        async start(controller) {
            ...
            sendEvent("start", {
                message:
                    "Gerrit bekijkt het bedrijf om gerichte vragen te stellen…",
            })
            ...
            try {
                ...
                const fieldsData = await Promise.all(
                    rawFields.map(async (field) => {
                        ...
                        const getSoilParam = (param: string) =>
                            soilData.find((d) => d.parameter === param)
                                ?.value ?? null
                        return {
                            b_id: field.b_id,
                            ...
                            b_gwl_class: getSoilParam("b_gwl_class") as
                                | string
                                | null,
                            a_som_loi: getSoilParam("a_som_loi") as
                                | number
                                | null,
                        }
                    }),
                ...
                const fieldsSummary: FarmFieldSummary[] = fieldsData.map(
                    (f) => ({
                        b_id: f.b_id,
                        b_name: f.b_name,
                        b_area: f.b_area ?? 0,
                ...
                const agent = createClarifyAgent(
                    fdm,
                ...
                const fieldLines = productiveFields
                    .map(
                        (f) =>
                            `- ${f.b_lu_name} (${f.b_lu_catalogue}), ${f.b_area?.toFixed(1)} ha, grondsoort: ${f.b_soiltype_agr ?? "onbekend"}`,
                    )
                    .join("\n")
                const fertLines = fertilizers
                    ...
                    .map(
                        (f: any) =>
                            `- ${f.p_name_nl ?? f.p_id_catalogue} (${f.p_type})`,
                    )
                    .join("\n")
...
Gebruik de beschikbare tools voor een volledige analyse en stel daarna 0–5 verduidelijkingsvragen.`
                ...
                if (!structuredResponse) {
                    console.warn(
                        "[clarify] structuredResponse is undefined after agent run",
                    )
                }

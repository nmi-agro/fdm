...
export async function loader({ request, params }: LoaderFunctionArgs) {
  try {
    ...
    const applicationRefs = parseAppIds(url.searchParams.get("appIds") ?? "")
    ...
    let fieldIds = url.searchParams.get("fieldIds")?.split(",").filter(Boolean) ?? []
    ...
    const availableApplicationMethodOptions = (applicationMethods.options ?? [])
      .filter(
    ...
    const fertilizerOptions: FertilizerOption[] = fertilizers.map((fertilizer) => ({
      ...
      applicationMethodOptions: (fertilizer.p_app_method_options ?? [])
        .map((opt) => availableApplicationMethodOptions.find((x) => x.value === opt))
        .filter(
          (
            option,
          ): option is {
    ...
    let loaderExampleFertilizerApplication:
      ...
      | undefined
    let fertilizerApplication: Partial<z.infer<typeof FormSchemaPartialModify>> | undefined
    if (applicationRefs.length > 0) {
      const fertilizerApplications = await Promise.all(
        applicationRefs.map(async ({ p_app_id }) => {
          const app = await getFertilizerApplication(fdm, session.principal_id, p_app_id)
      ...
      for (const key of keys) {
        for (const app of fertilizerApplications) {
          ...
          if (keyTypes[key] === "number") {
            ...
            keyTypes[key] === "date" &&
            (exampleFertilizerApplication[key] as Date).getTime() !== (app[key] as Date).getTime()
          ) {
    ...
    const selectedFields = fields.filter((field) => field.b_id && fieldIds.includes(field.b_id))
...
export default function FarmFieldFertilizerAddIndex() {
  ...
  return (
    <SidebarInset>
      ...
      <main>
        <FarmTitle
          title={isModification ? "Bemesting wijzigen" : "Bemesting toevoegen"}
          description={
            isModification
                  ...
                  <Dialog open={open} onOpenChange={setOpen}>
                    ...
                    <DialogContent className="sm:max-w-[425px]">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Selecteer de percelen voor de bemesting.
                        </DialogDescription>
                      </DialogHeader>
                      ...
                      <DialogFooter>
                        <Button type="button" onClick={handleSelectionChange}>
                          Selectie bijwerken
                        </Button>
                      </DialogFooter>
                    </DialogContent>
                ...
                <CardContent>
                  {loaderData.fieldAmount > 0 ? (
                    <FertilizerApplicationForm
                      ...
                      b_id_or_b_lu_catalogue={searchParams.get("fieldIds") || ""}
                      fertilizerApplication={loaderData.fertilizerApplication}
...
export async function action({ request, params }: ActionFunctionArgs) {
  try {
    ...
    if (url.searchParams.has("appIds")) {
      const validatedData = await extractFormValuesFromRequest(request, FormSchemaPartialModify)

      const p_app_ids = validatedData.p_app_id.split(",").filter(Boolean)
      await fdm.transaction((tx) =>
        Promise.all(
          p_app_ids.map(async (p_app_id) => {
            const original = await getFertilizerApplication(tx, session.principal_id, p_app_id)
            ...
            return updateFertilizerApplication(
              ...
              validatedData.p_app_amount_display ?? original.p_app_amount_display,
    ...
    const fieldIds = [
      ...new Set(url.searchParams.get("fieldIds")?.split(",").filter(Boolean) ?? []),
    ]
    if (!fieldIds || fieldIds.length === 0) {
      return dataWithError(null, "Selecteer eerst een perceel.")
    }

    const validatedData = await extractFormValuesFromRequest(request, FormSchema)

...
import {
    countGerritRequestsToday,
    getGerritDailyLimit,
} from "~/lib/gerrit-limit.server"
...
export async function loader({ request }: LoaderFunctionArgs) {
    ...
    const strategies = {
        ...
        reduceAmmoniaEmissions:
            url.searchParams.get("reduceAmmoniaEmissions") === "true",
        keepNitrogenBalanceBelowTarget:
            url.searchParams.get("keepNitrogenBalanceBelowTarget") === "true",
        workOnRotationLevel:
            url.searchParams.get("workOnRotationLevel") === "true",
        ...
        try {
            ...
            if (Array.isArray(parsed)) {
                clarifications = parsed.map((c: any) => ({
                    question: sanitizeAdditionalContext(
                        String(c.question ?? ""),
                        200,
                    ),
                    selectedOptionLabels: (c.selectedOptionLabels ?? []).map(
                        (l: string) =>
                            sanitizeAdditionalContext(String(l), 120),
                    ),
                    other: c.other
                        ? sanitizeAdditionalContext(String(c.other), 200)
                        : undefined,
                }))
            }
        } catch {
            ...
            try {
                ...
                const enrichedPlan = fieldsData.map((fd) => {
                    ...
                    return {
                        ...
                        applications: (proposedField?.applications || []).map(
                            (app) => {
                                ...
                                const fert = fertilizers.find(
                                    (f: Fertilizer) =>
                                        f.p_id_catalogue ===
                                        sanitizedCatalogueId,
                                )
                                const methodMeta =
                                    applicationMethods?.options?.find(
                                        (x: any) =>
                                            x.value === app.p_app_method,
                                    )
                        ...
                        fieldMetrics:
                            (proposedField as any)?.fieldMetrics ?? null,

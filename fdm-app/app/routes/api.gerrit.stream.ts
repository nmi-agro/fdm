...
export async function loader({ request }: LoaderFunctionArgs) {
    ...
    const strategies = {
        ...
        keepNitrogenBalanceBelowTarget: url.searchParams.get("keepNitrogenBalanceBelowTarget") === "true",
        ...
        async start(controller) {
            ...
            try {
                ...
                if (productiveFields.length === 0) {
                    sendEvent("error", { message: "Er zijn geen percelen gevonden voor dit bedrijf. Voeg eerst percelen toe." })
                    closeStream()
                    return
                }

                if (fertilizers.length === 0) {
                    sendEvent("error", { message: "Er zijn geen meststoffen gevonden voor dit bedrijf. Voeg eerst meststoffen toe aan het bedrijf." })
                    closeStream()
                    return
                }
                ...
                if (useStructured?.success) {
                    ...
                    if (firstBrace === -1 || lastBrace <= firstBrace) {
                        sendEvent("error", { message: "Gerrit gaf een onleesbaar antwoord. Probeer het opnieuw." })
                        closeStream()
                        return
                    }
                    ...
                    try {
                        ...
                        if (repaired) {
                            try {
                                ...
                                sendEvent("error", { message: "Gerrit gaf een ongeldig plan terug. Probeer het opnieuw." })
                                closeStream()
                                return
                            }
                        } else {
                ...
                const enrichedPlan = fieldsData.map((fd) => {
                    ...
                    return {
                        ...
                        applications: (proposedField?.applications || []).map((app) => {
                            ...
                            const fert = fertilizers.find((f: Fertilizer) => f.p_id_catalogue === sanitizedCatalogueId)
                            const methodMeta = applicationMethods?.options?.find((x: any) => x.value === app.p_app_method)
                ...
                sendEvent("error", { message: "Er is een onverwachte fout opgetreden." })
            } finally {
                closeStream()
            }
        }

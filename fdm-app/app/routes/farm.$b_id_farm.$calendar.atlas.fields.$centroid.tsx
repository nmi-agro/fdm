...
function buildAdvancedCultivationHistory(
  ...
  const selectedYearEntry = rawAdvanced.find((y) => y.year === calendarYear) || sortedYears[0]
  ...
  const scaleFactor = officialAreaHa > 0 ? officialAreaHa / selectedYearOverlapAreaHa : 1
  ...
  const enrichedHistory = sortedYears.map((yearEntry) => {
    const fields = yearEntry.fields.map((field) => {
      ...
      return {
        ...
        overlap_pct_of_selected: field.b_area_overlap / selectedYearOverlapAreaHa,
        overlap_pct_of_historical: field.b_area > 0 ? field.b_area_overlap / field.b_area : 0,
      }
    })
    const total_overlap_pct = fields.reduce((sum, f) => sum + f.overlap_pct_of_selected, 0)
    return { year: yearEntry.year, fields, total_overlap_pct }
  })

  const history = enrichedHistory.map((yearEntry) => {
    const total_area_ha = yearEntry.fields.reduce((sum, f) => sum + f.b_area_overlap, 0)
...
async function loadAsyncData(_calendar: string, latitude: number, longitude: number) {
  try {
    ...
    const [estimates, cultivationCatalogue, fieldDetailsData, queriedField] = await Promise.all([
      estimatesPromise,
    ...
    const [regionTable2, isNvGebied, isGWBGGebied, isNatura2000Area] = fieldDetailsData
 ...
 */
function FieldDetailsAtlas({ b_id_farm, calendar, asyncData }: Awaited<ReturnType<typeof loader>>) {
  const {
    fieldDetails,
    cultivationHistory,
    advancedCultivationHistory,

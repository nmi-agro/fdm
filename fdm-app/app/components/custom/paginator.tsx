...
export function getPageSearch(search: string, pageSize: number, n: number) {
  return modifySearchParams(search, (searchParams) => {
    if (typeof n === "number") searchParams.set("pageOffset", (n * pageSize).toString())
    ...
    if (page === currentPage) {
      deltaPageButtons.push(
        <PaginationItem key={page}>
          <PaginationCurrent
            value={page}
            onValueChange={(v) => navigate(getPageSearch(location.search, pageSize, v))}
          />
        </PaginationItem>,
      )
    } else {
      deltaPageButtons.push(
        <PaginationItem key={page}>
          <PaginationLink size="default" href={getPageSearch(location.search, pageSize, page)}>
            {page + 1}
          </PaginationLink>
        </PaginationItem>,
      )
      ...
      <PaginationContent>
        <PaginationItem>
          <PaginationLink
            ...
            title="Eerste pagina"
            size="default"
            href={getPageSearch(location.search, pageSize, firstPage)}
          >
        ...
        <PaginationItem>
          <PaginationLink
            ...
            title="Vorige pagina"
            size="default"
            href={getPageSearch(location.search, pageSize, currentPage - 1)}
          >
        ...
        <PaginationItem>
          <PaginationLink
            ...
            size="default"
            href={getPageSearch(location.search, pageSize, lastPage)}
          >

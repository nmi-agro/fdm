...
export function Expandable({ children }: { children: ReactNode }) {
  ...
  return (
    <ctx.Provider value={{ expanded, setExpanded, isOverflowing, setIsOverflowing }}>
      {children}
    </ctx.Provider>
  )
}
...
export function ExpandableContent(props: ExpandableContentProps) {
  const { collapsedClassName, expandedClassName, className, children, ...restProps } = props
  ...
  return (
    <motion.div
      initial={false}
      animate={{
        height: expanded ? (fullHeight ?? "auto") : (collapsedHeight ?? "auto"),
      }}

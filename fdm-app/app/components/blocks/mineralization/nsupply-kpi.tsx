...
export function FarmNSupplyKpi({ results }: FarmNSupplyKpiProps) {
  ...
  const avgN =
    validResults.length > 0
      ? totalArea > 0
        ? validResults.reduce((sum, r) => sum + r.totalAnnualN * (r.area || 0), 0) / totalArea
        : validResults.reduce((sum, r) => sum + r.totalAnnualN, 0) / validResults.length
      : 0
    ...
    <>
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-medium">N Levering bedrijf</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-medium">Hoogste N levering</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-medium">Laagste N levering</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">
...
function getNAtDoy(
  ...
  return data.reduce((prev, curr) =>
    Math.abs(curr.doy - targetDoy) < Math.abs(prev.doy - targetDoy) ? curr : prev,
  ).d_n_supply_actual
}
...
export function FieldNSupplyDetailsCard({ results }: FieldNSupplyDetailsCardProps) {
  const bestResult =
          ...
          <div className="space-y-1.5">
            ...
            {todayN != null && (
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">Tot vandaag</span>
                <span className="font-mono font-medium">{Math.round(todayN)} kg N/ha</span>
              </div>
            )}
            {progress != null && (
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">Voortgang dit jaar</span>
...
export function FieldNSupplyKpi({ results, soilType, organicMatter }: FieldNSupplyKpiProps) {
  const bestResult =
    results.find((r) => !r.error && r.method === "minip") ?? results.find((r) => !r.error)
  ...
  return (
    <>
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-medium">N Levering</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">
            {bestResult && !bestResult.error
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-medium">Methoden</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-medium">Bodemtype</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-medium">Organische Stof</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">

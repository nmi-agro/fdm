...
export function groupEventsByDate(events: DynaChartEvent[]): Map<string, DynaChartEvent[]> {
...
export function EventDot({ viewBox, events }: EventDotProps) {
  ...
  return (
    <circle cx={x} cy={y} r={5} fill={color} fillOpacity={0.9} stroke="white" strokeWidth={1.5} />
  )
}
        ...
        {visibleEntries.map((entry) => (
          <div key={entry.dataKey} className="flex items-center gap-1.5">
            ...
            <span className="text-muted-foreground">
              {dynaChartConfig[entry.dataKey as keyof typeof dynaChartConfig]?.label ??
                entry.dataKey}
            </span>
            <span className="ml-auto font-medium tabular-nums">
              {Math.round(entry.value)}{" "}
              <span className="text-muted-foreground font-normal">kg N/ha</span>
            </span>
          </div>
        ))}
      </div>
...
export function DynaChart({
  ...
  const uniqueEventDates = Array.from(eventsByDate.entries()).filter(([date]) =>
    chartDates.has(date),
  )
            ...
            <ComposedChart data={chartData} margin={{ top: 20, right: 8, left: 0, bottom: 0 }}>
              <defs>
                <linearGradient id="bandGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="var(--color-b_nw)" stopOpacity={0.2} />
                  <stop offset="95%" stopColor="var(--color-b_nw)" stopOpacity={0.05} />
                </linearGradient>
              </defs>
              ...
              {uniqueEventDates.map(([date, evs]) => (
                <ReferenceLine
                  ...
                  strokeDasharray="4 3"
                  label={(props) => <EventDot viewBox={props.viewBox} events={evs} />}
                />
              ))}
            </ComposedChart>
          </ChartContainer>
            ...
            >
              <defs>
                <linearGradient id="differenceGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="var(--color-b_nw_difference)" stopOpacity={0.3} />
                  <stop offset="95%" stopColor="var(--color-b_nw_difference)" stopOpacity={0.05} />
                </linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
              <XAxis
              ...
              />
              <ChartTooltip content={<DynaTooltipContent isBalance />} />
              <ChartLegend content={<ChartLegendContent />} />

    ...
    <Table>
      <TableHeader>
        <TableRow>
          ...
          <TableHead className="text-right">N Levering (kg N/ha)</TableHead>
        ...
        {sorted.map((result) => (
          <TableRow key={result.b_id}>
            ...
            <TableCell className="text-right font-mono">
              {result.error ? "—" : Math.round(result.totalAnnualN)}
            </TableCell>
            <TableCell className="text-right">
              <NMineralizedToday result={result} currentDoy={currentDoy} />
            </TableCell>
          </TableRow>
        ))}
      </TableBody>
...
function NMineralizedToday({ result, currentDoy }: { result: NSupplyResult; currentDoy: number }) {
  if (result.error) {
    return (
      <>
  ...
  const todayPoint = result.data.reduce((prev, curr) =>
    Math.abs(curr.doy - currentDoy) < Math.abs(prev.doy - currentDoy) ? curr : prev,
  )
  const pct = Math.round((todayPoint.d_n_supply_actual / result.totalAnnualN) * 100)

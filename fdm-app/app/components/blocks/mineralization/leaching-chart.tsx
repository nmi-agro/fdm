...
export function LeachingChart({ data, events = [] }: LeachingChartProps) {
  ...
  return (
    <ChartContainer config={leachingChartConfig} className="h-[220px] w-full">
      <ComposedChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
        <defs>
          <linearGradient id="leachGradient" x1="0" y1="0" x2="0" y2="1">
            <stop offset="5%" stopColor="var(--color-b_no3_leach)" stopOpacity={0.4} />
            <stop offset="95%" stopColor="var(--color-b_no3_leach)" stopOpacity={0.05} />
          </linearGradient>
        </defs>
        ...
        <ChartTooltip
          content={
            <ChartTooltipContent
              ...
              formatter={(value) => [`${Math.round(Number(value))} kg N-NO₃/ha`, "Uitspoeling"]}
            />
          }
        />
        <Area

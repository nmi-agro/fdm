...
export function DynaFieldList({ fields, promises, b_id_farm, calendar }: DynaFieldListProps) {
  return (
    <Table>
      <TableHeader>
        <TableRow>
...
function DynaCells({ promise }: { promise: Promise<FarmDynaResult> }) {
  ...
  const hasValue = !error && !!result?.calculationDyna?.length && lastPoint !== undefined
  ...
  return (
    <>
      ...
      <TableCell className="text-right font-mono">{hasValue ? Math.round(nUptake) : "—"}</TableCell>

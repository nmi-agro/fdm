  ...
  return (
    <ChartContainer config={farmChartConfig} className="h-[300px] w-full">
      <AreaChart data={data} margin={{ top: 10, right: 16, left: 0, bottom: 0 }}>
        <defs>
          <linearGradient id="fillNSupply" x1="0" y1="0" x2="0" y2="1">
            <stop offset="5%" stopColor="var(--color-d_n_supply_actual)" stopOpacity={0.35} />
            <stop offset="95%" stopColor="var(--color-d_n_supply_actual)" stopOpacity={0.05} />
          </linearGradient>
        </defs>
        ...
        <ChartTooltip
          content={
            <ChartTooltipContent
              ...
              formatter={(value) => [`${Number(value).toFixed(1)} kg N/ha`, "Cumulatief"]}
            />
          }
        />
        {isCurrentYear && (
...
function mergeSeriesData(series: FieldDataSeries[]): Record<string, number | string>[] {
...
export function FieldMineralizationChart({
  ...
  return (
    <ChartContainer config={fieldChartConfig} className="h-[350px] w-full">
      <AreaChart data={mergedData} margin={{ top: 10, right: 16, left: 0, bottom: 0 }}>
        <defs>
          {/* Only MINIP gets a fill gradient */}
          <linearGradient id="fill-minip" x1="0" y1="0" x2="0" y2="1">
            <stop offset="5%" stopColor={METHOD_STYLE.minip.color} stopOpacity={0.25} />
            <stop offset="95%" stopColor={METHOD_STYLE.minip.color} stopOpacity={0} />
          </linearGradient>
        </defs>
        <CartesianGrid strokeDasharray="3 3" vertical={false} />
        <XAxis
          ...
          content={
            <ChartTooltipContent
              ...
              formatter={(value, name) => [
                ...
                fieldChartConfig[name as NSupplyMethod]?.label ?? name,
              ]}
            />
          }
        />
        ...
        {activeSeries.map((s) => {
          ...
          return (
            <Area
              ...
              stroke={style.color}
              strokeWidth={2.5}
              strokeDasharray={style.dashArray === "0" ? undefined : style.dashArray}
              fill={s.method === "minip" ? "url(#fill-minip)" : "transparent"}

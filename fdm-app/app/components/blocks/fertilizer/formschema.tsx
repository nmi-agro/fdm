...
export const FormSchema = z
  .object({
    p_name_nl: z
      .string({
        error: (issue) => (issue.input === undefined ? "Naam is verplicht" : "Ongeldige waarde"),
      })
      .trim()
      .min(1, {
    ...
    p_type_rvo: z
      .string({
        error: (issue) =>
          issue.input === undefined ? "RVO mestcode is verplicht" : "Ongeldige waarde",
      })
      .min(1, {
    ...
    p_dm: z.preprocess(
      ...
      z.coerce
        .number({
          error: (issue) => (issue.input === undefined ? undefined : "Ongeldige waarde"),
        })
        .min(0, {
    ...
    p_app_method_options: z
      .array(z.string(), {
        error: (issue) => (issue.input === undefined ? "Selecteer minimaal 1 methode" : undefined),
      })
      .prefault([])
      .refine((value) => value.some((item) => item), {
        error: "Selecteer minimaal 1 methode",
      }),
    p_app_amount_unit: z.enum(["kg/ha", "l/ha", "m3/ha", "ton/ha"]).default("kg/ha"),
  })
  .refine(
    (data) => {
      if (data.p_app_amount_unit === "m3/ha" || data.p_app_amount_unit === "l/ha") {

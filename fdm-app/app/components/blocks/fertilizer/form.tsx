...
export function FertilizerForm({
      ...
      <Controller
        ...
        render={({ field, fieldState }) => (
          <Field data-invalid={fieldState.invalid} className="gap-1.5">
                ...
                <SelectContent>
                  {param.options?.map((option) => (
                    <SelectItem key={option.value ?? option.label} value={option.value ?? ""}>
                      {param.parameter === "p_app_amount_unit"
                        ? option.label
            ...
            {param.description && (
              <FieldDescription title={param.description} className="line-clamp-1 text-[10px]">
                {param.description}
              </FieldDescription>
            )}
  ...
  const getAppMethodInput = (param: FertilizerParameterDescriptionItem | undefined) => {
  ...
  const appMethodParam = fertilizerParameters.find((p) => p.parameter === "p_app_method_options")
          ...
          {!editable && (
            <Alert className="bg-muted/50 border-primary/20">
              ...
              <AlertDescription className="mt-2 flex flex-col justify-between gap-6 sm:flex-row sm:items-center">
                ...
                {p_id && (
                  <Button variant="outline" size="sm" className="shrink-0 font-semibold" asChild>
                    <NavLink
              ...
              <Card key={category.name} className="shadow-sm">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {groupedParameters[category.name]?.map((param) => getParameterInput(param))}
                  </div>

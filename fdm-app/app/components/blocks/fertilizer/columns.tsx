...
export const columns: ColumnDef<Fertilizer>[] = [
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-2">
          <span className="font-medium">{row.original.p_name_nl}</span>
          {isCustom && (
            <TooltipProvider>
              <Tooltip>
  ...
  },
  {
    accessorKey: "p_cr_vi",
    header: ({ column }) => {
      return <DataTableColumnHeader column={column} title="Cr-VI (mg/kg)" />
    },
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const isRenure = isRenureRvoCode(fertilizer.p_type_rvo)

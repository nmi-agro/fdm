...
function groupAndOrderHarvests(fields: FieldRow[]) {
  ...
  for (const group of grouped) {
    group.sort((a, b) => compareDates(a.b_lu_harvest_date, b.b_lu_harvest_date))
  }

  return grouped
    .map((group, i) => ({
      id: i.toString(),
      dates: group.map((harvest) => harvest.b_lu_harvest_date).filter((date) => date !== null),
      harvests: group,
    }))
    .sort((a, b) =>
      compareDates(a.dates.length > 0 ? a.dates[0] : null, b.dates.length > 0 ? b.dates[0] : null),
    )
}
...
function combineRecords(records: HarvestRecordItem[]): HarvestRecordItem {
  const timestamps = [
    ...new Set(records.flatMap((record) => record.dates.map((date) => date.getTime()))),
  ]
...
function HarvestDatesDisplayButton({
  ...
  const setActiveForm = useActiveTableFormStore((store) => store.setActiveForm)
...
export const HarvestDatesDisplay: React.FC<HarvestDatesDisplayProps> = ({ row }) => {

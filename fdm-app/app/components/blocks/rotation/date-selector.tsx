            ...
            <DatePicker
              ...
              field={{
                ...
                onChange: (value) => {
                  ...
                  const shouldSubmit = row.original.type === "crop" || formValues[name] !== value
                  if (shouldSubmit) {
                    const fieldIds = encodeURIComponent(
                      row.original.type === "field"
                        ? row.original.b_id
                        : (row.subRows ?? [])
                            .map((fieldRow) => (fieldRow.original as FieldRow).b_id)
                            .join(","),
                    )
...
export function TableDateSelector({
  ...
  return (
    ...
    >
      <DateRangeDisplay range={value} emptyContent="Geen" className={cn(showForm && "invisible")} />
      {showForm && (

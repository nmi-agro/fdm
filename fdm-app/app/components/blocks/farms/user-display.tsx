      ...
      <DropdownMenuTrigger asChild>
        <AvatarGroup>
          {users.slice(0, TRUNCATE_AFTER).map((user, i) => (
            <UserAvatar key={`${i}_${user.displayUserName}`} user={user} />
          ))}
          {users.length > TRUNCATE_AFTER && (
            <AvatarGroupCount>+{users.length - TRUNCATE_AFTER}</AvatarGroupCount>
          )}
        </AvatarGroup>
      </DropdownMenuTrigger>
      <DropdownMenuContent>
...
export function UserAvatar({ user }: { user: DisplayUser }) {
  return (
    <Avatar className="h-6 w-6 rounded-lg">
      <AvatarImage src={user.image ?? undefined} alt={user.displayUserName ?? undefined} />
...
export function AvatarGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("[&>*]:ring-background flex -space-x-2 [&>*]:ring-2", className)}
      {...props}
    />
  )
}

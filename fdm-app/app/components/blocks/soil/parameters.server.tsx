...
export function getSoilParametersForSoilAnalysisType(
  ...
  soilParameterDescription = soilParameterDescription.filter((item: { parameter: string }) =>
    soilParameters.includes(item.parameter),
  )

  // Order soilParameterDescription based on selected soil parameters
  soilParameterDescription.sort((a: { parameter: string }, b: { parameter: string }) => {
    return soilParameters.indexOf(a.parameter) - soilParameters.indexOf(b.parameter)
  })

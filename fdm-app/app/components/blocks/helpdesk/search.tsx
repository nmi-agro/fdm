...
type FilterConfig<T extends BaseFilter> =
  ...
  | {
      type: "custom"
      render: (props: { filters: T; setFilters: (value: any) => void }) => ReactNode
    }
type EnumOption = { value: string; label: string; icon?: ReactNode }

type FilterConfigArray<T extends BaseFilter> = (FilterCommon<T> & FilterConfig<T>)[]
            ...
            <Field key={field.name as string} className="space-y-0">
              ...
              <DatePicker
                ...
                field={{
                  ...
                  ref: (_instance: HTMLInputElement): void => {},
                }}
                fieldState={{
                  invalid: false,
                  isTouched: false,
            ...
            <Field key={field.name as string} className="space-y-0">
              ...
              <AutoComplete
                ...
                onSelectedValueChange={(value) =>
                  setFilters({
                    ...filters,
                    [field.name]: value ? [value] : undefined,
                  })
                }
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  {activeOption && (
                    <DropdownMenuItem
                      onSelect={() => {
                        const { [field.name]: _removed, ...newFilters } = filters
                  ...
                  {field.options.map((item) => (
                    <DropdownMenuCheckboxItem
                      key={item.value}
                      checked={item.value === filters[field.name]}
                      onClick={() => {
                        if (item.value === filters[field.name]) {
                          const { [field.name]: _removed, ...newFilters } = filters
                          setFilters(newFilters as T)
                        } else {
                          setFilters({

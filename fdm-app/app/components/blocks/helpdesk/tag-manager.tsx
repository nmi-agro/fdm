...
import { DEFAULT_TAG_COLOR, DEFAULT_TAG_COLOR_LABEL, SWATCH, TagCreator } from "./tag-creator"
    ...
    <Card className="mx-auto max-w-5xl">
      {canModify && (
        <CardHeader className="flex flex-row items-center gap-2">
          ...
          <Button onClick={() => setTagCreateDialogOpen(true)}>Nieuwe tag aanmaken</Button>
          <TagCreator
...
export function TagRow({ tag, canModify, activeTableCell, setActiveTableCell }: TagRowProps) {
        ...
        {canModify ? (
          ...
          >
            <SelectTrigger className="text-start gap-2" title="Kleur">
              ...
              />
              <div className="grow">{activeColorOption.label}</div>
            </SelectTrigger>
            <SelectContent>
              {SWATCH.map((color) => (
          ...
          (SWATCH.find((color) => color.value === tag.color)?.label ?? tag.color)
        )}
      </TableCell>
      {canModify && (
        <TableCell className="text-end align-middle">
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button type="button" variant="ghost" disabled={isSubmitting} title="Verwijderen">
                <Trash2 />
              </Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Deze actie kan niet ongedaan worden gemaakt. De tag wordt van alle tickets
                  verwijderd.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
...
function TagNameCell({ tag, canModify, activeTableCell, setActiveTableCell }: TagRowProps) {
  const fetcher = useFetcher()

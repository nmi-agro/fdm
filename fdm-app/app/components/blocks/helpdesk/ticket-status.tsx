...
export const TICKET_STATUS_DESCRIPTIONS: Record<string, string> = {
  ...
  waiting_on_customer: "We wachten op een reactie van u. Voeg een bericht hieronder toe.",
...
export function TicketStatusSelector({
  ...
  const allowedStatusTransitions = ALLOWED_TICKET_STATUS_TRANSITIONS[status] ?? []
  ...
  return (
    <DropdownMenu>
      <div className="flex flex-row items-center gap-2">
        <DropdownMenuTrigger asChild>
          <Button
            ...
            disabled={
              !canModify || fetcher.state !== "idle" || allowedStatusTransitions.length === 0
            }
          >
      ...
      <DropdownMenuContent align="start">
        {TICKET_STATUS.filter((item) => allowedStatusTransitions.includes(item.value)).map(
          (item) => (
            <DropdownMenuItem
              key={item.value}
              className="gap-2"

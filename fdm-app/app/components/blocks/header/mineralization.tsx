...
export function HeaderMineralization({
  ...
  const selectedField = b_id ? fieldOptions.find((f) => f.b_id === b_id) : undefined

  return (
    <>
      <BreadcrumbSeparator className="hidden xl:block" />
      <BreadcrumbItem className="hidden xl:block">
        <BreadcrumbLink href={`/farm/${b_id_farm}/${calendar}/mineralization`}>
          Mineralisatie
        </BreadcrumbLink>
      </BreadcrumbItem>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start">
                {fieldOptions.map((option) => (
                  ...
                  >
                    <NavLink to={`/farm/${b_id_farm}/${calendar}/mineralization/${option.b_id}`}>
                      <span className="truncate">{option.b_name}</span>
                      {b_id === option.b_id && <Check className="h-4 w-4 shrink-0" />}
                    </NavLink>
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
      ...
      {isDyna ? (
        <>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink href={`/farm/${b_id_farm}/${calendar}/mineralization/${b_id}/dyna`}>
              DYNA
            </BreadcrumbLink>
          </BreadcrumbItem>
        </>
        ...
        b_id && (
          <>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink href={`/farm/${b_id_farm}/${calendar}/mineralization/${b_id}`}>
                Bodem N-levering
              </BreadcrumbLink>
            </BreadcrumbItem>
          </>

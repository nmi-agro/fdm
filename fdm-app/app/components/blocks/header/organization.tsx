...
export function HeaderOrganization({
  ...
  const isMembersRoute = !!matches.find((match) => match.id === "routes/organization.$slug.members")
  ...
  const isInvitationRespondRoute = !!matches.find(
    (match) => match.id === "routes/organization.invitations.$invitation_id.respond",
  )
  const isNewOrganizationRoute = !!matches.find((match) => match.id === "routes/organization.new")
  const typesOfBalanceRoutes = ["nitrogen", "organic-matter"] as const
  const farmBalanceRouteType = typesOfBalanceRoutes.find((type) =>
    matches.find(
      (match) => match.id === `routes/organization.$slug.$calendar.balance.${type}._index`,
    ),
  )
  ...
  const isIndicatorsRoute = !!matches.find(
    (match) => match.id === "routes/organization.$slug.$calendar.indicators",
  )
  const isIndicatorsAtlasRoute = !!matches.find(
    (match) => match.id === "routes/organization.$slug.$calendar.atlas.indicators",
  )

  return (
    <>
      <BreadcrumbItem className="hidden xl:block">
        <BreadcrumbLink href="/organization">Organisaties</BreadcrumbLink>
      </BreadcrumbItem>
      {isNewOrganizationRoute ? (
        <>
          <BreadcrumbSeparator className="hidden xl:block" />
          <BreadcrumbItem>
            <BreadcrumbLink href="/organization/new">Nieuwe organisatie</BreadcrumbLink>
          </BreadcrumbItem>
        </>
        ...
        <>
          <BreadcrumbSeparator className="hidden xl:block" />
          <BreadcrumbItem>
            <BreadcrumbLink href="/invitations">Uitnodiging</BreadcrumbLink>
          </BreadcrumbItem>
        </>
      ) : (
        <>
                ...
                <span className="truncate">
                  {selectedOrganizationSlug && organizationOptions
                    ? (organizationOptions.find(
                        (option) => option.slug === selectedOrganizationSlug,
              ...
              {organizationOptions && organizationOptions.length > 0 ? (
                <DropdownMenuContent align="start">
                  {organizationOptions.map((option) => (
                    <DropdownMenuCheckboxItem
                      checked={selectedOrganizationSlug === option.slug}
                      key={option.slug}
                    >
                      <NavLink
                        to={
                          selectedOrganizationSlug
                            ? currentPath.replace(selectedOrganizationSlug, option.slug)
            ...
            <>
              ...
              <BreadcrumbItem>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="start">
                    <DropdownMenuCheckboxItem
                      checked={farmBalanceRouteType === "nitrogen"}
                      key={"nitrogen"}
                    >
                      <NavLink
                    ...
                    <DropdownMenuCheckboxItem
                      checked={farmBalanceRouteType === "organic-matter"}
                      key={"organic-matter"}
                    >
                      <NavLink

...
function buildChartDataAndLegend({
  ...
  const nitrateEmission =
    type === "farm" ? balanceData.emission.nitrate : balanceData.emission.nitrate.total
  const chartData: Record<string, string | number | undefined> = {
    ...
    deposition: Math.abs(
      type === "farm" ? balanceData.supply.deposition : balanceData.supply.deposition.total,
    ),
    mineralization: Math.abs(
      type === "farm" ? balanceData.supply.mineralisation : balanceData.supply.mineralisation.total,
    ),
    emissionAmmonia:
      balanceData.emission.ammonia.total === undefined
        ? undefined
        : Math.abs(balanceData.emission.ammonia.total),
    emissionNitrate: nitrateEmission === undefined ? undefined : Math.abs(nitrateEmission),
  }
  ...
  if (type === "farm") {
    ...
    chartData.residueAmmonia = Math.abs(balanceData.emission.ammonia.residues)
    ...
    fertilizerTypes.forEach((p_type) => {
      ...
      chartData[`${p_type}FertilizerSupply`] = Math.abs(balanceData.supply.fertilizers[p_type])
  ...
  if (type === "field") {
    ...
    function addFertilizerApplication(
      ...
      const application = fieldInput.fertilizerApplications.find(
        (fa: { p_app_id: string }) => fa.p_app_id === applicationResult.id,
      )
    ...
    fertilizerTypes.forEach((p_type) => {
      ...
      balanceData.supply.fertilizers[p_type].applications.forEach((app) => {
      ...
      balanceData.emission.ammonia.fertilizers[p_type].applications.forEach((app) => {
        addFertilizerApplication(
          fieldInput,
    ...
    balanceData.supply.fixation.cultivations.forEach((cultivationResult) => {
      addCultivation(
        fieldInput,
        "fixation",
        "Fixatie",
    ...
    balanceData.removal.harvests.harvests.forEach((harvestResult) => {
      ...
      const cultivationDetails = harvestDetails
        ? fieldInput.cultivations.find((lu) => lu.b_lu === harvestDetails.b_lu)
        : undefined
    ...
    balanceData.removal.residues.cultivations.forEach((cultivationResult) => {
      addCultivation(
        fieldInput,
        "removalResidue",
    ...
    balanceData.emission.ammonia.residues.cultivations.forEach((cultivationResult) => {
      addCultivation(
        fieldInput,
        "residueAmmonia",
        "Ammoniakemissie",
  ...
  // Bar Graph Layout
  const supplyBar = ["deposition", ...fixationBar, "mineralization", ...fertilizerSupplyBar]
  const allRemovalBar = [
    ...removalBar,
...
export function NitrogenBalanceChart(
  ...
  const barRadius = 5
  const barRadiusStart: [number, number, number, number] = [barRadius, 0, 0, barRadius]
  const barRadiusEnd: [number, number, number, number] = [0, barRadius, barRadius, 0]
    ...
    >
      ...
      >
        ...
        <ChartTooltip
          shared={false}
          content={({ payload }) => {
            const validPayloadItem = payload.find((item) => item && item.type !== "none")

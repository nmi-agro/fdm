      ...
      <DialogContent>
        ...
        <form ref={formRef} className="space-y-4 max-h-50 overflow-y-scroll">
          {farms.flatMap((farm) => {
            ...
            return (
              <div key={farm.b_id_farm} className="flex flex-row items-center gap-4">
                <Checkbox name={b_id_farm} defaultChecked={!!currentValue} />
                <div className="grow">{farm.b_name_farm ?? "Onbekend"}</div>
          ...
          <DialogClose
            ...
            onClick={() => {
              ...
              if (form) {
                ...
                for (const [b_id_farm, selected] of formData.entries()) {
                  if (selected) {
                    newlySelectedFarmIds.push(b_id_farm)
                  }
                }
              }
              const sortedDefaultSelectedFarmIds = [...defaultSelectedFarmIds].sort()
              newlySelectedFarmIds.sort()
              if (
                sortedDefaultSelectedFarmIds.length !== newlySelectedFarmIds.length ||
                newlySelectedFarmIds.some(
                  (selected_id, index) => selected_id !== sortedDefaultSelectedFarmIds[index],
                )
              ) {
                setSearchParams((searchParams) => {
                  const newSearchParams = new URLSearchParams(searchParams)
                  if (newlySelectedFarmIds.length > 0) {
                    newSearchParams.set("farmIds", newlySelectedFarmIds.join(","))
                  } else {

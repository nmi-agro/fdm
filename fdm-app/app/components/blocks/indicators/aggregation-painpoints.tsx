...
export function AggregationPainpoints({
    ...
    const [selectedAggId, setSelectedAggId] = useState<AggregationId | null>(
        null,
    )
    ...
    const rankedLeaves = useMemo(() => {
        const list = LEAF_AGGREGATION_IDS.map((aggId) => {
            const score01 = computeAreaWeightedAggregation(
                fieldScores,
                fields,
                aggId,
            )
    ...
    const rankedIndicators = useMemo(() => {
        ...
        return INDICATORS.map((ind) => {
            ...
            for (const s of fieldScores) {
                const val = s.score?.indicators.find(
                    (i) => i.indicator_id === ind.id,
                )?.score
    ...
    const worstFieldsForTheme = useMemo(() => {
        ...
        return fields
            .map((field) => {
                ...
                const score01 = fs
                    ? getFieldAggregationScore(fs.score, activeAggId)
                    : null
            ...
            .filter(
                (f): f is NonNullable<typeof f> & { impact: number } =>
    ...
    const worstFieldsForIndicator = useMemo(() => {
        ...
        return fields
            .map((field) => {
                const fs = fsById.get(field.b_id)
                const val = fs?.score?.indicators.find(
                    (i) => i.indicator_id === activeIndId,
                )?.score
    ...
    const activeIndName =
        rankedIndicators.find((i) => i.id === activeIndId)?.name ?? activeIndId
    ...
    const worstFields =
        leftMode === "themes" ? worstFieldsForTheme : worstFieldsForIndicator
    const rightHeading =
        leftMode === "themes"
                    ...
                    {criticalCount > 0 ? (
                        <>
                            ...
                            <span className="font-semibold text-foreground">
                                {criticalCount} kritieke en {warningCount}{" "}
                                matige thema's gedetecteerd.
                            </span>
                        </>
                    ) : warningCount > 0 ? (
                        <>
                            ...
                            <span className="font-semibold text-foreground">
                                {warningCount} matige thema's gedetecteerd. Geen
                                kritieke knelpunten.
                            </span>
                        </>
                    ) : (
                                            ...
                                            return (
                                                <Button
                                                    ...
                                                    className={cn(
                                                        ...
                                                        isSelected &&
                                                            "border-border shadow-sm font-semibold",
                                                    )}
                                                >

  ...
  <Page size="A4" style={pdfStyles.frontPage}>
    ...
    >
      ...
      >
        ...
        <View style={[pdfStyles.frontTitleContainer, { alignItems: "center" }]}>
          <Text style={[pdfStyles.frontTitle, { textAlign: "center" }]}>Bemestingsplan</Text>
          <Text style={[pdfStyles.frontSubtitle, { textAlign: "center", color: "#FFFFFF" }]}>
            {data.year}
          </Text>
        </View>
      </View>
      ...
      <View style={pdfStyles.frontFooter}>
        ...
        <View style={{ marginTop: 10, opacity: 0.8 }}>
          ...
          >
            <Text style={pdfStyles.frontInfo}>KvK: {data.farm.kvk || "-"}</Text>
...
const TableOfContents = ({ data }: { data: BemestingsplanData }) => (
  <Page size="A4" style={pdfStyles.page}>
    ...
    >
      ...
      >
        De berekeningen van de gebruiksruimte en het bemestingsadvies in dit document zijn gebaseerd
        op de door de gebruiker verstrekte gegevens en de op het moment van genereren geldende wet-
        en regelgeving. Deze getallen zijn uitsluitend bedoeld voor informatieve doeleinden en
        dienen als indicatie. Hoewel {data.config.name} streeft naar maximale nauwkeurigheid, kunnen
        er geen rechten worden ontleend aan de gepresenteerde waarden. De uiteindelijke
        verantwoordelijkheid voor de naleving van de mestwetgeving ligt bij de landbouwer. Raadpleeg
        bij twijfel altijd de officiële publicaties van de Rijksdienst voor Ondernemend Nederland
        (RVO) en uw adviseur.
      </Text>
    </View>
  ...
  <Document title={`Bemestingsplan ${data.year} - ${data.farm.name}`}>
    ...
    <Page size="A4" style={pdfStyles.page} id="farm-summary">
      <View style={pdfStyles.header}>
        ...
        <View style={{ alignItems: "flex-end" }}>
          <Text style={pdfStyles.title}>Samenvatting {data.year}</Text>
        </View>
      </View>
        ...
        <PdfCard>
          <View style={pdfStyles.grid}>
            ...
            <View style={[pdfStyles.gridCol, { width: "30%" }]}>
              ...
              <Text style={pdfStyles.value}>{data.totalArea.toFixed(1)} ha</Text>
              <Text style={[pdfStyles.label, { marginTop: 4 }]}>productieve opp.</Text>
              <Text style={pdfStyles.value}>{data.productiveArea.toFixed(1)} ha</Text>
            </View>
          </View>
        </PdfCard>
      </View>
      ...
      <View style={{ marginTop: 10 }}>
        ...
        <View style={[pdfStyles.grid, { marginTop: 5 }]}>
          ...
          <View style={{ width: "50%", paddingLeft: 5 }}>
            ...
            <PdfCard style={{ padding: 8 }}>
              {(() => {
                const productiveFields = data.fields.filter((f) => !f.isBufferstrip)
                const bufferFields = data.fields.filter((f) => f.isBufferstrip)

                const getCrops = (fields: typeof data.fields) => {
                ...
                const productiveCrops = getCrops(productiveFields)
                const bufferCrops = getCrops(bufferFields)
                const bufferTotal = bufferFields.reduce((sum, f) => sum + f.area, 0)
                const farmTotal = data.fields.reduce((sum, f) => sum + f.area, 0)
                ...
                const renderCropList = (
                  ...
                  >
                    ...
                    {Object.entries(crops)
                      ...
                      .map(([crop, area]) => (
                        ...
                        >
                          ...
                          <Text style={[pdfStyles.value, { fontSize: 9 }]}>
                            {area.toFixed(1)} ha ({((area / farmTotal) * 100).toFixed(1)}
                            %)
                          </Text>
                        </View>
                      ))}
                    ...
                    >
                      ...
                      <Text style={[pdfStyles.value, { fontSize: 9 }]}>
                        {total.toFixed(2)} ha ({((total / farmTotal) * 100).toFixed(1)}
                        %)
                      </Text>
                    </View>
                  </View>
                ...
                return (
                  <View>
                    {renderCropList(
                      productiveCrops,
                      data.productiveArea,
                      Object.keys(bufferCrops).length > 0 ? "Productief" : undefined,
                    )}
                    {Object.keys(bufferCrops).length > 0 && (
                      <>
                        ...
                        />
                        {renderCropList(bufferCrops, bufferTotal, "Bufferstroken")}
                      </>
                    )}
                  </View>
                )
                  ...
                  .reduce(
                    (acc, f) => {
                      f.applications.forEach((app) => {
                        if (app.quantity === null || app.quantity_display === null) {
                          return
                        }
                        ...
                        if (!acc[app.product]) {
                          acc[app.product] = {
                            ...
                            amount_display: 0,
                            amount_unit:
                              getApplicationAmountTotalUnitLabel(app.quantity_unit) ?? "kg",
                            n: 0,
                            nw: 0,
                            p: 0,
                            k: 0,
                        ...
                        acc[app.product].amount_display += app.quantity_display * f.area
                        acc[app.product].n += app.p_dose_n * f.area
                        acc[app.product].nw += app.p_dose_nw * f.area
                        acc[app.product].p += app.p_dose_p * f.area
                        acc[app.product].k += app.p_dose_k * f.area
                      })
                      return acc
                    },
                    {} as Record<
                  ...
                  .map(([name, stats], i) => (
                    <View
                      ...
                      style={[
                        pdfStyles.tableRow,
                        {
                          borderBottomWidth: i === Object.keys(fertilizers).length - 1 ? 0 : 1,
                        },
                      ]}
                    >
                      <PdfTableCell weight={2}>
                      ...
                      <PdfTableCell style={{ textAlign: "right" }}>
                        <Text>
                          {Math.round(stats.amount_display).toLocaleString("nl-NL")}{" "}
                          {stats.amount_unit}
                        </Text>
                      </PdfTableCell>
                      <PdfTableCell style={{ textAlign: "right" }}>
                        <Text>{Math.round(stats.n).toLocaleString("nl-NL")}</Text>
                      </PdfTableCell>
                      <PdfTableCell style={{ textAlign: "right" }}>
                        <Text>{Math.round(stats.nw).toLocaleString("nl-NL")}</Text>
                      </PdfTableCell>
                      <PdfTableCell style={{ textAlign: "right" }}>
                        <Text>{Math.round(stats.p).toLocaleString("nl-NL")}</Text>
                      </PdfTableCell>
                      <PdfTableCell style={{ textAlign: "right" }}>
                        <Text>{Math.round(stats.k).toLocaleString("nl-NL")}</Text>
                      </PdfTableCell>
                    </View>
                  ))
              })()}
    ...
    <Page size="A4" orientation="landscape" style={pdfStyles.page}>
      <View style={pdfStyles.header} id="fields-overview">
        <Text style={pdfStyles.title}>Overzicht percelen {data.year}</Text>
      </View>
      <PdfTable>
        <View fixed>
          <View
        ...
        {data.fields
          ...
          .map((field) => (
            <View key={field.id} wrap={false} style={pdfStyles.tableRow}>
              ...
              <PdfTableCell style={{ textAlign: "right" }}>
                <Text>
                  {Math.round(field.normsFilling.nitrogen)} / {Math.round(field.norms.nitrogen)}
                </Text>
              </PdfTableCell>
              <PdfTableCell style={{ textAlign: "right" }}>
                <Text>
                  {Math.round(field.normsFilling.manure)} / {Math.round(field.norms.manure)}
                </Text>
              </PdfTableCell>
              <PdfTableCell style={{ textAlign: "right" }}>
                <Text>
                  {Math.round(field.normsFilling.phosphate)} / {Math.round(field.norms.phosphate)}
                </Text>
              </PdfTableCell>
              <PdfTableCell style={{ textAlign: "right" }}>
                <Text>
                  {Math.round(field.planned.p_dose_nw)} / {Math.round(field.advice.d_n_req)}
                </Text>
              </PdfTableCell>
              <PdfTableCell style={{ textAlign: "right" }}>
                <Text>
                  {Math.round(field.planned.p_dose_p)} / {Math.round(field.advice.d_p_req)}
                </Text>
              </PdfTableCell>
              <PdfTableCell style={{ textAlign: "right" }}>
                <Text>
                  {Math.round(field.planned.p_dose_k)} / {Math.round(field.advice.d_k_req)}
                </Text>
              </PdfTableCell>
            </View>
          ))}
      ...
      .map((field) => (
        <Page key={field.id} size="A4" style={pdfStyles.page}>
          ...
          <View style={pdfStyles.header} id={`field-${field.id}`}>
            ...
            {data.config.logo && <Image src={data.config.logo} style={{ width: 40 }} />}
          </View>
            ...
            <View style={{ width: "50%", paddingRight: 5 }}>
              ...
              <PdfCard style={{ padding: 8 }}>
                <View style={{ gap: 3 }}>
                  ...
                  >
                    <Text style={{ fontSize: 8 }}>Organische stof (%)</Text>
                    <Text style={[pdfStyles.value, { fontSize: 9 }]}>
                      {field.soil.a_som_loi !== undefined ? field.soil.a_som_loi.toFixed(1) : "-"}
                    </Text>
                  </View>
                  <View
                    style={{
                  ...
                  >
                    <Text style={{ fontSize: 8 }}>Bodemtype</Text>
                    <Text style={[pdfStyles.value, { fontSize: 9 }]}>
                      {field.soil.b_soiltype_agr
                        ? soilTypeLabels[field.soil.b_soiltype_agr] || field.soil.b_soiltype_agr
                        : "-"}
                    </Text>
                  </View>
                  <View
                  ...
                  >
                    <Text style={{ fontSize: 8 }}>Klei / Silt / Zand (%)</Text>
                    <Text style={[pdfStyles.value, { fontSize: 9 }]}>
                      {field.soil.a_clay_mi ?? "-"} / {field.soil.a_silt_mi ?? "-"} /{" "}
                  ...
                  >
                    <Text style={{ fontSize: 8 }}>Fosfaat (P-Al / P-CaCl)</Text>
                    <Text style={[pdfStyles.value, { fontSize: 9 }]}>
                      {field.soil.a_p_al ?? "-"} / {field.soil.a_p_cc ?? "-"}
                    </Text>
                  </View>
                  <View
                    style={{
                  ...
                  >
                    <Text style={{ fontSize: 8 }}>Kalium (K-CaCl)</Text>
                    <Text style={[pdfStyles.value, { fontSize: 9 }]}>
                      {field.soil.a_k_cc ?? "-"}
                    </Text>
                  </View>
                  ...
                  >
                    <Text style={{ fontSize: 8 }}>Zuurgraad (pH-CaCl)</Text>
                    <Text style={[pdfStyles.value, { fontSize: 9 }]}>
                      {field.soil.a_ph_cc !== undefined ? field.soil.a_ph_cc.toFixed(1) : "-"}
                    </Text>
                  </View>
                </View>
              </PdfCard>
            ...
            <View style={{ width: "50%", paddingLeft: 5 }}>
              ...
              <PdfCard style={{ padding: 8 }}>
                <View style={{ gap: 3 }}>
                  ...
                  >
                    <Text style={{ fontSize: 8 }}>Hoofdteelt</Text>
                    <Text
                      style={[
                        pdfStyles.value,
                        {
                  ...
                  >
                    <Text style={{ fontSize: 8 }}>Vanggewas</Text>
                    <Text
                      style={[
                        pdfStyles.value,
                        {
                  ...
                  <PdfCard style={{ padding: 8 }}>
                    <View style={{ gap: 2 }}>
                      ...
                      >
                        <Text style={{ fontSize: 8 }}>Aanvoer (EOS)</Text>
                        <Text style={[pdfStyles.value, { fontSize: 9 }]}>
                          {Math.round(field.omBalance.supply)} kg/ha
                        </Text>
                      </View>
                      <View
                        style={{
                      ...
                      >
                        <Text style={{ fontSize: 8 }}>Afbraak (OS)</Text>
                        <Text style={[pdfStyles.value, { fontSize: 9 }]}>
                          {Math.round(field.omBalance.degradation)} kg/ha
                        </Text>
                      </View>
                      <View
                        style={{
                      ...
                      >
                        ...
                        <Text
                          style={[
                            ...
                            {
                              fontSize: 9,
                              color: field.omBalance.balance >= 0 ? "#22c55e" : "#ef4444",
                            },
                          ]}
                        >
                  ...
                  {[
                    ...
                    >
                      <UsageBar
                        label={n.label}
                        planned={
                          (field.planned[n.plannedKey as keyof typeof field.planned] || 0) *
                          n.factor
                        }
                        limit={
                          (field.advice[n.adviceKey as keyof typeof field.advice] || 0) * n.factor
                        }
                        unit={n.unit}
                      />
                    </View>
            ...
            <PdfTable>
              ...
              {field.applications.length > 0 ? (
                field.applications.map((app, idx) => (
                  <PdfTableRow key={`${app.date}-${app.product}-${idx}`}>
                    <PdfTableCell weight={1.2}>
                      <Text>{app.date}</Text>
                      <Text
                        style={{
                    ...
                    <PdfTableCell weight={0.8}>
                      <Text>
                        {app.quantity_display ? Math.round(app.quantity_display) : "?"}{" "}
                        {getApplicationAmountUnitLabel(app.quantity_unit)}
                      </Text>
                    </PdfTableCell>
                    <PdfTableCell>
                      <Text>
                        {Math.round(app.p_dose_n)} / {Math.round(app.p_dose_nw)}
                      </Text>
                    </PdfTableCell>
                    <PdfTableCell>
                      <Text>{Math.round(app.p_dose_p)}</Text>
                    </PdfTableCell>
                    <PdfTableCell>
                      <Text>{Math.round(app.p_dose_k)}</Text>
                    </PdfTableCell>
                  </PdfTableRow>
                ))
              ) : (
                <PdfTableRow>
                  <PdfTableCell weight={5}>
                    <Text>Geen geplande bemestingen gevonden.</Text>
                  </PdfTableCell>
                </PdfTableRow>
              )}
            </PdfTable>
                ...
                <PdfTableRow key={field.id}>
                  ...
                  <PdfTableCell weight={3}>
                    {field.applications.length > 0 ? (
                      ...
                      >
                        Waarschuwing: bemesting op bufferstrook!
                      </Text>
                    ) : (

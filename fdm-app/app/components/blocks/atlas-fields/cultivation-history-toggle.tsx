...
export function CultivationHistoryToggle({
  ...
  if (!advancedCultivationHistory) {
    return <CultivationHistoryCard cultivationHistory={cultivationHistory} />
  }
  ...
  const handleCopy = () => {
    if (view === "simple") {
      ...
      navigator.clipboard.writeText([header, ...rows].join("\n")).then(
        () => toast.success("Uitgebreide gewashistorie gekopieerd naar klembord"),
        () => toast.error("Kopiëren naar klembord mislukt"),
      )
    }
  }
    ...
    <Card className="col-span-1 lg:row-span-2 overflow-hidden">
      ...
      <CardContent className="pt-6 text-sm">
        {view === "simple" ? (
          ...
          <AdvancedCultivationFlow data={advancedCultivationHistory} currentYear={currentYear} />
        )}
      </CardContent>
    </Card>
  )

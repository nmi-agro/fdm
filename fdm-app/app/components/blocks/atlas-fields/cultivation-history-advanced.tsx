  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="w-14 shrink-0 text-right">
        {isActive ? (
          <Badge variant="default" className="h-5 px-1.5 text-[10px] font-black tabular-nums">
            {yearEntry.year}
          </Badge>
        ) : (
              ...
              <TooltipContent className="space-y-0.5">
                ...
                <p className="text-xs">
                  ...
                  <span className="font-medium">
                    {Math.round(field.overlap_pct_of_selected * 100)}% overlap
                  </span>
                </p>
              </TooltipContent>
            </Tooltip>
    ...
    <>
      <div className="space-y-0">
        {data.history.map((yearEntry, index) => {
          ...
          return (
            <div key={yearEntry.year} className={cn(isHiddenOnMobile && "hidden lg:block")}>
              <YearRow yearEntry={yearEntry} currentYear={currentYear} />

              {/* Connector between rows.
                                Hidden on mobile when the next row is also hidden. */}
              {hasNext && (
                <div className={cn(!isExpanded && index >= mobileLimit - 1 && "hidden lg:block")}>
                  <YearConnector />
                </div>
              )}
            </div>
          ...
          >
            {isExpanded ? (
              ...
              <>
                ...
                Meer jaren tonen ({data.history.length - mobileLimit})
              </>
            )}
          </Button>
        </div>

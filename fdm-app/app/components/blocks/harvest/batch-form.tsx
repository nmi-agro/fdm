...
type HarvestRow = Partial<Record<keyof typeof columnInfos, number | null | undefined>> & {
...
function createNewRow(
  ...
  if (lastRow?.b_lu_harvest_date) {
    ...
    try {
      ...
      if (!Number.isFinite(monthOrdinal) || monthOrdinal > 12 || monthOrdinal < 1) {
        throw new Error("Expected month to be in range 01-12")
      }
      b_lu_harvest_date = getContextualDate(calendar, monthOrdinal, day).toISOString()
    } catch (err) {
      console.error(new Error("Failed to parse default date", { cause: err }))
    }
  }
  return {
...
function getColumnTitle(
  ...
  return columnName === "cutting"
    ...
    : columnName === "b_lu_harvest_date"
      ? getHarvestDateTerm(croprotation, harvestable)
      : columnName === "delete"
        ? null
...
interface BatchHarvestFormProps {
  ...
  /** Effective harvestability for the cultivation */
  b_lu_harvestable: HarvestableType
...
function useBatchHarvestRemixForm({
  ...
  return useRemixForm<BatchHarvestFormValues>({
    ...
    resolver: zodResolver(SchemaWithIntent) as Resolver<BatchHarvestFormValues>,
    defaultValues: {
      intent: "batch_harvest",
      harvests:
    ...
    <Controller
      ...
      render={({ field, fieldState }) => (
        <DatePicker
          ...
          placeholder={
            exampleRow?.[columnName] !== undefined && exampleRow[columnName] !== null
...
function BatchHarvestFormRow({
  ...
  croprotation?: string | null
  /** Harvestability for context-aware terminology */
  harvestable?: HarvestableType | null
}) {
      ...
      {columnNames.map((columnName) => {
        const columnTitle =
          columnName === "delete" || columnName === "cutting"
            ? undefined
            : getColumnTitle(columnName, croprotation, harvestable)

        return (
          <TableCell key={columnName} title={columnTitle ?? undefined} className="align-top">
            {columnName === "cutting" ? (
              ...
              <Button
                ...
                onKeyDown={(e) => {
                  if (e.key === "Tab") {
                    if (rowRef.current?.matches(":last-child")) {
                      e.preventDefault()
                      onAdd()
                    }
                  }
    ...
    <Card className="space-y-2 p-2">
      <CardHeader className="flex flex-row items-center p-0">
        <CardTitle className="grow">
          {index + 1}e {getHarvestTerm(croprotation, false, harvestable)}
        </CardTitle>
        <Button
          type="button"
          variant="ghost"
      ...
      <CardContent className="grid grid-cols-1 gap-2 p-0 sm:grid-cols-2">
        {columnNames.map((columnName) => (
          <BatchHarvestDataCell
            key={columnName}
            index={index}
            label={getColumnTitle(columnName, croprotation, harvestable) ?? undefined}
            columnName={columnName}
            harvestRow={harvestRow}
            exampleRow={exampleRow}
          />
...
function BatchHarvestFormFields({
  ...
  const addRow = useCallback(() => {
    fieldArray.append(
      createNewRow(
        ...
        b_lu_start,
        b_lu_end,
        harvests.length > 0 ? harvests[fieldArray.fields.length - 1] : undefined,
        defaultHarvest,
        b_date_harvest_default,
      ),
    )
  ...
  }, [calendar, b_lu_start, b_lu_end, fieldArray, harvests, defaultHarvest, b_date_harvest_default])
  ...
  if (isMobile) {
    const columnNames: CardColumnName[] = ["b_lu_harvest_date", ...harvestParameterColumns]
    ...
    return (
      <div className="space-y-2">
        ...
        >
          <Plus />
          Nieuwe {getHarvestTerm(b_lu_croprotation, false, b_lu_harvestable)} toevoegen
        </Button>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-2">
          ...
          <TableRow>
            {columnNames.map((columnName) => (
              <TableHead key={columnName}>
                {getColumnTitle(columnName, b_lu_croprotation, b_lu_harvestable)}
              </TableHead>
            ))}
          </TableRow>
        </TableHeader>
      ...
      <Button ref={addButtonRef} type="button" variant="secondary" onClick={addRow}>
        <Plus />
        Extra {getHarvestTerm(b_lu_croprotation, false, b_lu_harvestable)} toevoegen
      </Button>
    </div>
  )
}
  ...
  return (
    <Dialog open={true} onOpenChange={() => navigate("..")}>
      <DialogContent className="flex max-h-svh max-w-2xl flex-col">
        <DialogHeader>
          <DialogTitle>
            Meerdere {getHarvestTerm(props.b_lu_croprotation, true, props.b_lu_harvestable)}{" "}
            toevoegen
          </DialogTitle>
          <DialogDescription>
        ...
        <RemixFormProvider {...form}>
          <fetcher.Form method="post" onSubmit={form.handleSubmit} className="grow overflow-y-auto">
            <fieldset
              className="space-y-4"
...
export function BatchHarvestForm(props: BatchHarvestFormProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <RemixFormProvider {...form}>
        <fetcher.Form id="formHarvest" onSubmit={form.handleSubmit} method="post">
          <fieldset
            disabled={form.formState.isSubmitting || fetcher.state !== "idle"}
            className="space-y-8"
          >

...
export function BcsWizard({
  ...
  const currentGuide = currentIndicator
    ? (BCS_GUIDES.find((guide) => guide.key === currentIndicator.key) ?? null)
    : null
  ...
  const handleAddAnnotation = (
    ...
    const indicator = (bcsIndicator as BcsVisualKey | undefined) ?? currentIndicator?.key
    ...
    <Card>
      <CardHeader className="space-y-3 pb-2">
        ...
        <div>
          {currentStep === 0 ? (
            <>
              <CardTitle>BodemConditieScore voor {fieldName}</CardTitle>
              <CardDescription className="mt-1.5">
                Leg eerst de beoordelingsdatum vast en voeg optioneel foto&apos;s toe die, waar je
                later notities aan kunt toevoegen.
              </CardDescription>
            </>
          ) : currentIndicator ? (
            <>
              <CardTitle>{currentIndicator.name}</CardTitle>
              <CardDescription className="mt-1.5">{currentIndicator.description}</CardDescription>
            </>
          ) : (
            <>
              <CardTitle>Controleer en bevestig</CardTitle>
              <CardDescription className="mt-1.5">
                Bekijk het totaal, de deelindicatoren en de foto&apos;s voordat je opslaat.
              </CardDescription>
            </>
          )}
        </div>
        ...
        {currentStep === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 md:items-start">
              ...
              <div className="bg-muted/30 text-muted-foreground space-y-1 rounded-xl border p-4 text-sm">
                ...
                {phCc != null || somLoi != null ? (
                  ...
                  <p>Geen recente labanalyse gevonden — pH en organische stof blijven onbekend.</p>
                )}
              </div>
            </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-4">
              <div className="text-sm font-medium">Kies de score</div>
              <div className="space-y-3">
                {currentGuide.criteria.map((criterion) => {
                  const selected = scores[currentIndicator.key] === criterion.score
                  const bcsColor = indicatorScoreColor(criterion.score, currentIndicator.direction)
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        setScores((previous) => ({
                          ...previous,
                          [currentIndicator.key]: criterion.score,
                        }))
                      }
                      className={cn(
          ...
          <div className="grid gap-6 lg:grid-cols-[3fr_2fr]">
            <div className="space-y-4">
                ...
                {hasAnyVisualScore ? (
                  <div className="flex items-start gap-2">
                    ...
                    <span>
                      ...
                      {BCS_VISUAL_KEYS.filter((key) => scores[key] != null).length} van de{" "}
                      {BCS_VISUAL_KEYS.length} visuele indicatoren ingevuld.
                    </span>
                  </div>
                ) : (
                  <span>Vul minimaal één visuele indicator in voordat je opslaat.</span>
                )}
              </div>
              ...
              >
                {isSubmitting ? "Opslaan..." : "Bevestigen en opslaan"}
              </Button>
            </div>

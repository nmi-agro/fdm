...
function getAnnotationCentroid(annotation: GalleryAnnotation): { x: number; y: number } | null {
  const c = annotation.coordinates
  if (annotation.type === "pin" && isPinCoords(c)) return { x: c.x, y: c.y }
  if (annotation.type === "circle" && isCircleCoords(c)) return { x: c.cx, y: c.cy }
  if (annotation.type === "arrow" && isArrowCoords(c))
    return { x: (c.x1 + c.x2) / 2, y: (c.y1 + c.y2) / 2 }
  if (annotation.type === "freehand" && isFreehandCoords(c) && c.points.length > 0) {
    ...
    <>
      {annotations.map((annotation, index) => {
        ...
        const indicatorLabel = annotation.bcsIndicator
          ? BCS_VISUAL_INDICATOR_MAP[annotation.bcsIndicator as BcsVisualKey]?.name
          : null
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  onMouseEnter={() => onHoverAnnotation(index)}
              ...
              <TooltipContent className="max-w-xs text-left">
                <div className="space-y-1">
                  {indicatorLabel ? <div className="font-semibold">{indicatorLabel}</div> : null}
                  <div>{annotation.text || "Notitie zonder tekst"}</div>
                </div>
              </TooltipContent>
            </Tooltip>
          </div>
...
export function ImageGallery({
  ...
  const [annotationIndicator, setAnnotationIndicator] = useState<string>(defaultIndicator ?? "none")
  ...
  const getContainerRect = () => imageContainerRef.current?.getBoundingClientRect()
  ...
  const handlePointerMove = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (drawingState.tool === "circle") {
      setDrawingState((prev) => (prev ? { ...prev, current: { x, y } } : prev))
  ...
  const handleDeleteAnnotationClick = (imageId: string, annotationIndex: number) => {
      ...
      return (
        <g>
          <line
            ...
            vectorEffect="non-scaling-stroke"
          />
          <circle cx={drawingState.current.x} cy={drawingState.current.y} r={1} fill="white" />
        </g>
      )
    }
    if (drawingState.tool === "freehand" && drawingState.points && drawingState.points.length > 1) {
      const pts = drawingState.points.map((p) => `${p.x},${p.y}`).join(" ")
      return (
        <polyline
          points={pts}
          fill="none"
      ...
      >
        <DialogContent className="max-h-[95dvh] w-full max-w-5xl overflow-y-auto p-4 sm:p-6">
          {selectedImage ? (
            <>
              <DialogHeader>
                <DialogTitle>{selectedImage.caption || "BCS foto"}</DialogTitle>
              </DialogHeader>
                  ...
                  <div className="overflow-auto rounded-xl border bg-black/5 p-2">
                    ...
                    >
                      <img
                        ...
                        alt={selectedImage.caption ?? "BCS foto"}
                      ...
                      >
                        {selectedImage.annotations.map((annotation, index) => {
                          ...
                          const color = isHovered ? "#facc15" : "white"
                          const shapeFilter = {
                            filter: "drop-shadow(0 0 2px rgba(0,0,0,0.8))",
                          }
                          const strokeWidth = isHovered ? "1" : "0.5"
                          ...
                          if (annotation.type === "arrow" && isArrowCoords(c)) {
                            ...
                            if (imgSize && imgSize.w > 0 && imgSize.h > 0) {
                              const sdx = ((c.x2 - c.x1) * imgSize.w) / 100
                              const sdy = ((c.y2 - c.y1) * imgSize.h) / 100
                              const slen = Math.sqrt(sdx * sdx + sdy * sdy)
                              if (slen > 3) {
                                const sux = sdx / slen
                                const suy = sdy / slen
                                const arrowPx = Math.min(slen * 0.35, 14)
                                const wingPx = arrowPx * 0.45
                                const spx = -suy
                                const spy = sux
                                const sbsx = (c.x2 * imgSize.w) / 100 - sux * arrowPx
                                const sbsy = (c.y2 * imgSize.h) / 100 - suy * arrowPx
                                const sw1x = sbsx + spx * wingPx
                                const sw1y = sbsy + spy * wingPx
                                const sw2x = sbsx - spx * wingPx
                                const sw2y = sbsy - spy * wingPx
                                const toX = (sx: number) => (sx * 100) / imgSize.w
                                const toY = (sy: number) => (sy * 100) / imgSize.h
                            ...
                            return (
                              <g key={index}>
                                ...
                                />
                                {headPoints ? (
                                  <polygon points={headPoints} fill={color} style={shapeFilter} />
                                ) : (
                                  <circle cx={c.x2} cy={c.y2} r={1.5} fill={color} />
                                )}
                              </g>
                            )
                          }
                          if (annotation.type === "freehand" && isFreehandCoords(c)) {
                            return (
                              <polyline
                                key={index}
                          ...
                          return (
                            <li
                              ...
                              className={cn(
                                ...
                                isHovered && "bg-yellow-50 dark:bg-yellow-950/20",
                              )}
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="break-words">
                                  {annotation.text || "Zonder tekst"}
                                </div>
                              </div>
                              {editMode && onRemoveAnnotation ? (
                                <Button
                                  type="button"
                                  variant="ghost"
                                  size="icon"
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium" htmlFor="annotation-title">
                Korte titel
              </label>
              <Input
                id="annotation-title"
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium" htmlFor="annotation-indicator">
                Indicator
              </label>
              <Select value={annotationIndicator} onValueChange={setAnnotationIndicator}>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium" htmlFor="annotation-text">
                Toelichting
              </label>
              <Textarea
                id="annotation-text"
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
              <Button type="button" variant="outline" onClick={resetAnnotationDialog}>
                Annuleren
              </Button>
              <Button type="button" onClick={handleSaveAnnotation}>
                Notitie opslaan
              </Button>
            </div>
          </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Deze Notitie wordt definitief verwijderd en kan niet worden teruggehaald.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

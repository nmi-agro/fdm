            ...
            <CollapsibleContent className="mt-3 space-y-2">
              ...
              <div className="space-y-3">
                {doses.applications.map((app) => {
                  const dose = app[description.doseParameter as keyof Dose] as number
                  if (dose === 0) {
                    return null
                  }
                  const fertilizerApplication = fertilizerApplications.find(
                    (x) => x.p_app_id === app.p_app_id,
                  )
                  const fertilizer = fertilizers.find(
                    (x) => x.p_id_catalogue === fertilizerApplication?.p_id_catalogue,
                  )
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="text-sm font-bold">
                          {dose > 1
                            ? Math.round(dose).toLocaleString()
                            : dose > 0
                              ? dose.toPrecision(2).toLocaleString()
                              : 0}{" "}
                          {description.unit}
                        </p>
                        <p className="text-muted-foreground text-xs">

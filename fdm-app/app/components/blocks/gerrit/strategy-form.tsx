...
export function StrategyForm({
    ...
    const currentSelected =
        (form.watch("selectedFertilizerIds") as string[] | undefined) ?? allIds
    const selectedSet = new Set(currentSelected)

    const groupedByType = fertilizerOptions.reduce<
        Record<string, FertilizerOption[]>
    >((acc, f) => {
    ...
    const toggleGroup = (
        ...
        const allGroupIds = (groupedByType[type] ?? []).map(
            (f) => f.p_id_catalogue,
        )
    ...
    const fertError = (form.formState.errors as any)?.selectedFertilizerIds

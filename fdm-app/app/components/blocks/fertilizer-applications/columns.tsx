...
export const columns: ColumnDef<FertAppRecordItem>[] = [
  {
    id: "p_app_date",
    header: "Datum",
    cell: ({ row }) => formatDateRange(row.original.applications.map((app) => app.p_app_date)),
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const fieldNames = Object.entries(
        Object.fromEntries(row.original.applications.map((app) => [app.b_id, app.b_name])),
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="-ms-4">
              <span className="text-muted-foreground">{fieldNames.length} percelen</span>
      ...
      [
        ...new Set(
          row.original.applications
            .map((application) => application.p_app_method_name ?? application.p_app_method)
  ...
  },
  {
    ...
    header: "",
    cell: (ctx) =>
      ctx.row.original.applications.some((app) => app.canModify) && <ModifyCell {...ctx} />,
  },
]
...
function ModifyCell({ row, table }: CellContext<FertAppRecordItem, unknown>) {
  ...
  const modifiableApps = row.original.applications.filter((app) => app.canModify)
  const modifiableAppIds = modifiableApps.map((app) => `${app.b_id}:${app.p_app_id}`).join(",")

  return (
    <div className="flex items-center justify-end gap-2">
      <Spinner className={cn("h-4 w-4", fetcher.state === "idle" && "invisible")} />
      <Button asChild>
        <NavLink

...
export async function computePlanMetrics(
  ...
  const [omInput, nInput] = await Promise.all([
    collectInputForOrganicMatterBalance(fdm, principalId, b_id_farm, timeframe).catch(() => null),
    collectInputForNitrogenBalance(fdm, principalId, b_id_farm, timeframe).catch(() => null),
  ])
      ...
      .map(async (field) => {
        ...
        try {
          ...
          const [manureResult, nitrogenResult, phosphateResult] = await Promise.all([
            ...
            fillingFuncs.calculateFertilizerApplicationFillingForNitrogen(fdm, fillingInput),
            Promise.resolve(
        ...
        if (nmiApiKey && isValidDutchCropCatalogue(field.b_lu_catalogue)) {
          try {
        ...
        if (omInput) {
          const fieldOmInput = omInput.fields.find((f: any) => f.field.b_id === field.b_id)
          if (fieldOmInput) {
            try {
              ...
              console.warn(`[computePlanMetrics] OM calc failed for ${field.b_id}:`, e)
            }
          }
        }
        ...
        if (nInput) {
          const fieldNInput = nInput.fields.find((f: any) => f.field.b_id === field.b_id)
          if (fieldNInput) {
            try {
              ...
              console.warn(`[computePlanMetrics] N calc failed for ${field.b_id}:`, e)
            }
          }
        }
  ...
  const farmNormsKg = aggregateNormsToFarmLevel(
    allFarmFieldNorms
      .filter(
        (field): field is NonNullable<(typeof allFarmFieldNorms)[number]> =>

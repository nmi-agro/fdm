...
const initializeRuntimeEnvMap = (): RuntimeConfig => {
  ...
  for (const key of keysToProcess) {
    ...
    if (typeof process !== "undefined" && process.env && process.env[stringKey] !== undefined) {
...
const sentryConfig =
  sentryDsn && sentryOrg && sentryProject
    ? {
        ...
        trace_sample_rate: Number(getConfigValue("PUBLIC_SENTRY_TRACE_SAMPLE_RATE", 1)),
        replay_sample_rate: Number(getConfigValue("PUBLIC_SENTRY_REPLAY_SAMPLE_RATE", 0)),
        replay_sample_rate_on_error: Number(
          getConfigValue("PUBLIC_SENTRY_REPLAY_SAMPLE_RATE_ON_ERROR", 1),
        ),
        profile_sample_rate: Number(getConfigValue("PUBLIC_SENTRY_PROFILE_SAMPLE_RATE", 1)),
        security_report_uri: String(getConfigValue("PUBLIC_SENTRY_SECURITY_REPORT_URI", "")),
      }
    : null
...
export const clientConfig: ClientConfig = {
  ...
  integrations: {
    map: {
      provider: getConfigValue("PUBLIC_MAP_PROVIDER", "osm") as "maptiler" | "osm",

...
export async function renderMagicLinkEmail(
  ...
  const emailTimestamp: string = format(timeZone ? TZDate.tz(timeZone) : new Date(), "Pp", {
    locale: nl,
  })
...
function getTimeZoneFromUrl(url: string): string | undefined {
  try {
    ...
    const callbackUrlCandidate = parsedMagicLinkUrl.searchParams.get("callbackURL")
    ...
    const parsedCallbackUrl = new URL(
      ...
      callbackUrlCandidate.startsWith("http") ? undefined : "http://example.com",
    )
...
export async function renderHelpdeskNewMessageEmail(
  ...
  const helpdeskSenderName =
    serverConfig.mail?.postmark.helpdesk_sender_name ?? serverConfig.mail?.postmark.sender_name
